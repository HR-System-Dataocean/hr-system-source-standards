            ...
            If txtEmployee.Text <> "" Then
                ...
                If clsEmp.Find("Code='" & txtEmployee.Text & "'") Then
                    If clsEmpVac.Find("ID=" & lbVactionID.Text) Then
                        ...
                        lblTotalDays.Text = Math.Round(Convert.ToDecimal(clsEmpVac.RemainingDays), 0) + Math.Round(Convert.ToDecimal(clsEmpVac.vactiondays), 0)
                        lbTotalVal.Text = Math.Round(Convert.ToDecimal(clsEmpVac.TotalDays), 0)
                        lbRemainVal.Text = Math.Round(Convert.ToDecimal(clsEmpVac.RemainingDays), 0)
                        txtVactiondays.Text = Math.Round(Convert.ToDecimal(clsEmpVac.vactiondays), 0)
                        txtRemarks.Text = ""
    ...
    Protected Sub txtVactiondays_ValueChange(sender As Object, e As System.EventArgs) Handles txtVactiondays.TextChanged
        If txtVactiondays.Text <> "" Then
            ...
            If ClsVacations.RoundAnnualVacBalance = True Then
                lbRemainVal.Text = (RoundVacationBalance(Math.Round(lblTotalDays.Text - txtVactiondays.Text, 2))).ToString()
            Else
                lbRemainVal.Text = Math.Round(lblTotalDays.Text - txtVactiondays.Text, 0)
            ...
            Dim Vacationdays As Integer = CInt(txtVactiondays.Text) + OfficialVacationDays
            WebDateChooser3.Value = vl.Date.AddDays(IIf(Vacationdays < 1, 0, Vacationdays))
            WebDateChooser2.Value = vl.Date.AddDays(IIf(Vacationdays < 1, 0, Vacationdays))
        ...
        For Each row As DataRow In previousPeriods.Tables(0).Rows
            ...
            If CInt(row("VacationTypeID")) <> 1 Then
                If (newStart <= periodEnd AndAlso newEnd >= periodStart) Then
                    ...
                    Dim typeQuery As String = "SELECT TOP 1 EngName FROM   hrs_VacationsTypes where ID =" & row("VacationTypeID").ToString()
                    Dim typeName As Object = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteScalar(ClsEmployees.ConnectionString, CommandType.Text, typeQuery)

                    Dim remarks = "Request Serial =" & row("VacationRequestID").ToString() & "  ..  vacationtype =" & Convert.ToString(typeName) & "   .. startdate =" & periodStart.ToString() & " .. Number of days = " & row("ConsumDays").ToString()
        ...
        If ClsContracts.ID > 0 Then
            ...
            Try
                If WebDateChooser2.Value <> Nothing Then
                    ...
                    Dim OfficialVacationDays As Integer = 0
                    OfficialVacationDays = GetOverlappingOfficialVacationDays(WebDateChooser1.Value, WebDateChooser2.Value)
                    Diffe = Diffe - OfficialVacationDays
            ...
            If lbVactionID.Text = "" Then
                If ClsVacations.RoundAnnualVacBalance = True Then
                    lblTotalDays.Text = (RoundVacationBalance(Math.Round(dblVacationDays, 2))).ToString()
                    ...
                    lbTotalVal.Text = RoundVacationBalance(Math.Round(dblVacationDays, 2))
                    txtVactiondays.Text = Math.Round(Diffe, 0)
                    lbRemainVal.Text = RoundVacationBalance(Math.Round(dblVacationDays - Diffe, 2))
                ...
                Else
                    lblTotalDays.Text = Math.Round(dblVacationDays, 0)
                    lblNewRemain.Text = Math.Round(Balance, 0)
                    lblTransferBalance.Text = Math.Round(Convert.ToDecimal(TransferBalance), 0)
                    lblNewBalance.Text = Math.Round(NewBalance, 0)
                    lblNewConsumed.Text = Math.Round(NewBalance - Balance, 0)

                    lblTransferRemain.Text = Math.Round(remain, 0)
                    lblTransferConsumed.Text = Math.Round(TransferBalance - remain, 0)
                    lbTotalVal.Text = Math.Round(dblVacationDays, 0)
                    txtVactiondays.Text = Math.Round(Diffe, 0)
                    lbRemainVal.Text = Math.Round(dblVacationDays - Diffe, 0)
    ...
    Protected Sub LinkButton_Payments_Click(sender As Object, e As System.EventArgs) Handles LinkButton_Payments.Click
        ...
        If CheckEmployee() Then
            ...
            If (ClsCompanies.VacSettlement) Then
                If (lbVactionID.Text <> "") Then
                    url = "OpenModal1('frmEmployeesVacationsAdvanceSettlementDependonVacation.aspx?Fisical=" & WebDateChooser1.Value & "&ID=" & ClsEmployees.ID & "&TrnsID=" & IIf(lbVactionID.Text = "", 0, lbVactionID.Text) & "&Dys=" & IIf(lbVactionID.Text = "", 0, lblTotalDays.Text) & "',600,900,false,'');"
                Else
