...
Partial Class frmOtherVacationsRequestNursing
    Inherits MainPage
...
#Region "Private Functions"
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            Dim str1 As String = "select ID,VacationType, RequestSerial ,EmployeeID ," & EmpName & " as EmployeeName,RequestDate , " & RequestName & " as RequestType, FormCode from SS_VFollowup where  EmployeeID= " & ClsEmployees.ID & "  and FormCode='SS_0036' Order By RequestDate desc"
    ...
    Public Function SaveAnnualVacationRequest() As Boolean
        ...
        Try
            ...
            '''''''''''''''''''''''''''''عدد المرات
            Dim strNoOfTimes As String
            strNoOfTimes = "select NoOfTimes from SS_RequestTypes where RequestCode='SS_0036'"
            Dim NoOfTimes As Integer
            ...
            strTimesPeriodPerMonth = "select TimesPeriodPerMonth from SS_RequestTypes where RequestCode='SS_0036'"
            ...
            If TimesPeriodPerMonth > 0 And NoOfTimes > 0 Then
                ...
                strCOuntNoOfimes = "Select count(id) from SS_VacationRequest where VacationType='SS_0036' and RequestDate<=GETDATE() and RequestDate>= DATEADD(month, -" & TimesPeriodPerMonth & ", getdate()) and EmployeeID='" & ClsEmployees.ID & "'"
            ...
            WebHandler.GetCookies(Page, "UserID", User)
            InsertCommand = "Insert Into SS_VacationRequest Values(''," & ClsEmployees.ID & "," & DdlVacationType.SelectedItem.Value & ",'SS_0036',convert(datetime, '" & ddlRequestDate.Value & "',103),convert(datetime, '" & WebDateChooser1.Value & "',103) ,convert(datetime, '" & WebDateChooser2.Value & "',103),0, " & lbRemainVal.Text & " ,'" & txtContactNo.Text & "', ' ','" & TxtRemarks.Text & "'," & User & " ,convert(datetime,  '" & ClsEmployeesVacations.GetHigriDate(Date.Now) & "' ,103),'" & txtRequesterUser.Text & "',3)"
    ...
    Public Function SaveFirstLeveNotification() As Boolean
        Try
            ...
            strselect = "select * from SS_Configuration where FormCode='SS_0036'and Rank=1"
            ...
            If DsFirstLevel.Tables(0).Rows.Count > 0 Then
                For Each Row As Data.DataRow In DsFirstLevel.Tables(0).Rows
                    ...
                    If Row("UserTypeID") = 1 Then
                        ...
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & DirectManagerID & ",'SS_0036'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                    ...
                    If Row("UserTypeID") = 2 Then
                        ...
                        If DsPositionEmployees.Tables(0).Rows.Count > 0 Then
                            Dim strinsert As String
                            Dim strMaxRequestSerial As String
                            strMaxRequestSerial = "select isnull(Max (ID),1) from SS_VacationRequest where EmployeeID=" & ClsEmployees.ID & " and AnnualVacationType='SS_0036' "
                            Dim MaxSerial As Integer
                            SqlCommand = New SqlClient.SqlCommand
                            SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                            SqlCommand.CommandType = CommandType.Text
                            ...
                            For Each RW In DsPositionEmployees.Tables(0).Rows
                                ...
                                End If
                                strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & RW("EmployeeID") & ",'SS_0036'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                                SqlCommand = New SqlClient.SqlCommand
                                SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                                SqlCommand.CommandType = CommandType.Text
                                SqlCommand.CommandText = strinsert
                    ...
                    If Row("UserTypeID") = 3 Then
                        ...
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & Row("EmployeeID") & ",'SS_0036'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                        SqlCommand.CommandText = strinsert
    ...
    Public Function GenerateRequestCode() As String
        ...
        MaxSerial = CInt(SqlCommand.ExecuteScalar())
        SqlCommand.Connection.Close()
        RequestCode = "V_OTH_NR_ " & MaxSerial & ""
        Return RequestCode
    End Function
