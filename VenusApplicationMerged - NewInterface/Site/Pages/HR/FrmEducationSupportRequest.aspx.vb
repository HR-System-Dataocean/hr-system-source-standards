...
#Region "Private Functions"
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            Dim str1 As String = "select ID,VacationType, RequestSerial ,EmployeeID ," & EmpName & " as EmployeeName,RequestDate , " & RequestName & " as RequestType, FormCode from SS_VFollowup where  EmployeeID= " & ClsEmployees.ID & "  and FormCode='SS_001912' Order By RequestDate desc"
    ...
    Private Function SavePart() As Boolean
        ...
        Try
            ...
            Dim ServiceDays As Integer = DateDiff(DateInterval.Day, cls_Employee.JoinDate, DateTime.Now)

            If ServiceDays < 90 Then
                Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "Sorry...System Can't Proceed with your requrst Please ensure 90 days from the hire date have passed./عفوا..،هذاالإجراء غير متاح حتى اكتمال 90 يومًا من التعيين ."))
                Exit Function
            End If
    ...
    Public Function SaveRequest() As Boolean
        ...
        Try
            ...
            strNoOfTimes = "select NoOfTimes from SS_RequestTypes where RequestCode='SS_001912'"
            ...
            strTimesPeriodPerMonth = "select TimesPeriodPerMonth from SS_RequestTypes where RequestCode='SS_001912'"
            ...
            If TimesPeriodPerMonth > 0 And NoOfTimes > 0 Then
                ...
                strCOuntNoOfimes = "Select count(id) from SS_EducationSupportRequest where RequestDate <= GETDATE() and RequestDate>= DATEADD(month, -" & TimesPeriodPerMonth & ", getdate()) and EmployeeID='" & ClsEmployees.ID & "'"
            ...
            Dim WebHandler As New Venus.Shared.Web.WebHandler
            WebHandler.GetCookies(Page, "UserID", User)
            InsertCommand = "Insert Into SS_EducationSupportRequest Values(''," & ClsEmployees.ID & ",convert(datetime, '" & ddlRequestDate.Value & "',103),'" & TxtRemarks.Text & "','" & txtRequesterUser.Text & "',3)"
            ...
            Dim UpdateCommand = "Update  SS_EducationSupportRequest  set Code ='" & GenerateRequestCode() & "' where ID=" & GetInertedID() & " "
    ...
    Public Function SaveFirstLeveNotification() As Boolean
        Try
            ...
            strselect = "select * from SS_Configuration where FormCode='SS_001912' and Rank=1"
            ...
            strMaxRequestSerial = "select isnull(Max(ID),1) from SS_VacationRequest where EmployeeID=" & ClsEmployees.ID & " and VacationType='SS_001912'"
            ...
            For Each Row As Data.DataRow In DsFirstLevel.Tables(0).Rows
                ...
                If Row("UserTypeID") = 1 Then
                    ...
                    If Not String.IsNullOrEmpty(DirectManagerID) AndAlso DirectManagerID <> "0" AndAlso DirectManagerID <> "null" Then
                        ...
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID) values(" & MaxSerial & " , " & DirectManagerID & ",'SS_001912'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                ...
                If Row("UserTypeID") = 2 Then
                    ...
                    If DsPositionEmployees.Tables(0).Rows.Count > 0 Then
                        For Each RW In DsPositionEmployees.Tables(0).Rows
                            ...
                            Dim strinsert As String
                            strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID) values(" & MaxSerial & " , " & RW("EmployeeID") & ",'SS_001912'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                            SqlCommand = New SqlClient.SqlCommand
                            SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                            SqlCommand.CommandType = CommandType.Text
                            SqlCommand.CommandText = strinsert
                ...
                If Row("UserTypeID") = 3 Then
                    ...
                    Dim strinsert As String
                    strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID) values(" & MaxSerial & " , " & EmpID & ",'SS_001912'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                    SqlCommand = New SqlClient.SqlCommand
                    SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                    SqlCommand.CommandType = CommandType.Text
                    SqlCommand.CommandText = strinsert
            ...
            If DirectManagerSkipped AndAlso Not Processed Then
                ...
                strSecondLevel = "select * from SS_Configuration where FormCode='SS_001912' and Rank=2"
    ...
    Public Function GetInertedID() As Integer
        ...
        strMaxRequestSerial = "select isnull(Max (ID),1) from SS_EducationSupportRequest where EmployeeID=" & ClsEmployees.ID & ""
