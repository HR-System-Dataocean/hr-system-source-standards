...
Partial Class FrmAdvanceHousingRequest
    Inherits MainPage
        ...
        Select Case e.CommandArgument
            ...
            Case "SaveNew"
                If String.IsNullOrWhiteSpace(InstallmentDate.Text) Then
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, " Please Enter Installment Date /برجاءإدخال تاريخ بداية الاستقطاع"))
                    Exit Sub
                End If
                If String.IsNullOrWhiteSpace(txtInstallmentsNo.Text) Then
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, " Please Enter Installments Count /برجاءإدخال عدد الأقساط"))
                    Exit Sub
                End If
                If CheckDates(CDate(DateTime.Now.Date), CDate(InstallmentDate.Value)) = False Then
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, " Error in Installment Date should be after today  /تاريخ بداية الإستقطاع يجب ان يكون أكبر من تاريخ اليوم"))
                    Exit Sub
                End If
                If CheckAdvanceHousingContract() Then
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, " Housing allowance is not provided for in the current employee's contract terms  /لا يتوفر بدل السكن في بنود عقد الموظف الحالي"))
                    Exit Sub
                End If
...
#Region "Private Functions"
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            Dim str1 As String = "select ID,VacationType, RequestSerial ,EmployeeID ," & EmpName & " as EmployeeName,RequestDate , " & RequestName & " as RequestType, FormCode from SS_VFollowup where  EmployeeID= " & ClsEmployees.ID & "  and FormCode='SS_001913' Order By RequestDate desc"
    ...
    Private Function SavePart() As Boolean
        ...
        End Try
    End Function
    Public Function SaveRequest() As Boolean
        ...
        Try
            ...
            strNoOfTimes = "select NoOfTimes from SS_RequestTypes where RequestCode='SS_001913'"
            ...
            strTimesPeriodPerMonth = "select TimesPeriodPerMonth from SS_RequestTypes where RequestCode='SS_001913'"
            ...
            If TimesPeriodPerMonth > 0 And NoOfTimes > 0 Then
                ...
                strCOuntNoOfimes = "Select count(id) from SS_AdvanceHousingRequest where RequestDate <= GETDATE() and RequestDate>= DATEADD(month, -" & TimesPeriodPerMonth & ", getdate()) and EmployeeID='" & ClsEmployees.ID & "'"
            ...
            Dim WebHandler As New Venus.Shared.Web.WebHandler
            WebHandler.GetCookies(Page, "UserID", User)
            InsertCommand = "Insert Into SS_AdvanceHousingRequest Values(''," & ClsEmployees.ID & ",convert(datetime, '" & ddlRequestDate.Value & "',103),'" & TxtRemarks.Text & "','" & txtRequesterUser.Text & "',3,convert(datetime, '" & InstallmentDate.Value & "',103)," & txtInstallmentsNo.Text & ")"
            ...
            Dim UpdateCommand = "Update  SS_AdvanceHousingRequest  set Code ='" & GenerateRequestCode() & "' where ID=" & GetInertedID() & " "
    ...
    Public Function SaveFirstLeveNotification() As Boolean
        Try
            ...
            strselect = "select * from SS_Configuration where FormCode='SS_001913'and Rank=1"
            ...
            If DsFirstLevel.Tables(0).Rows.Count > 0 Then
                For Each Row As Data.DataRow In DsFirstLevel.Tables(0).Rows
                    ...
                    If Row("UserTypeID") = 1 Then
                        ...
                        strMaxRequestSerial = "select isnull(Max (ID),1) from SS_AdvanceHousingRequest where EmployeeID=" & ClsEmployees.ID & " "
                    ...
                    If Row("UserTypeID") = 2 Then
                        ...
                        If DsPositionEmployees.Tables(0).Rows.Count > 0 Then
                            ...
                            For Each RW In DsPositionEmployees.Tables(0).Rows
                                ...
                                End If
                                strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID,OvertimeDate,HoursCount)  values(" & MaxSerial & " , " & RW("EmployeeID") & ",'SS_001913'," & ClsEmployees.ID & ",0," & Row("ID") & ",convert(datetime, '" & InstallmentDate.Value & "',103)," & txtInstallmentsNo.Text & ")"
                                SqlCommand = New SqlClient.SqlCommand
                                SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                                SqlCommand.CommandType = CommandType.Text
                                SqlCommand.CommandText = strinsert
                    ...
                    If Row("UserTypeID") = 3 Then
                        ...
                        End If
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID,OvertimeDate,HoursCount)  values(" & MaxSerial & " , " & Row("EmployeeID") & ",'SS_001913'," & ClsEmployees.ID & ",0," & Row("ID") & ",convert(datetime, '" & InstallmentDate.Value & "',103)," & txtInstallmentsNo.Text & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                        SqlCommand.CommandText = strinsert
                    ...
                    End If
                Next
                ClsEmployees.SendEmail("SSRequests", Me.Page, 1, "SS_AdvanceHousingRequest", PrintRequestSerial)
