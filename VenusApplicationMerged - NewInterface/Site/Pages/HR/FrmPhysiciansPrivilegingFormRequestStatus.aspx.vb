...
#Region "Protected Sub"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                ClsObjects.Find(" Code='SS_PhysiciansPrivilegingFormRequest'")
    ...
    Protected Sub ImageButton_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs) Handles ImageButton_Save.Command
        ...
        Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "../../Interfaces/frmReportsGridViewer.aspx?Language=false&Criteria=RequestSerial&preview=1&ReportCode=CF_128&sq0=''&v=" & Session("RequestSerial"), 700, 490, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "wWindO", False, True, False, False, False, False, False, False, False)
...
#Region "Private Functions"
    ...
    Private Sub FillEmployeeVacations(ByVal EmployeeID As Integer, Optional ByVal showFirstRecord As Boolean = False)
        ...
        If Type = 2 Then
            Try
                ...
                strselect = "select SS_PhysiciansPrivilegingFormRequest.Code as RequestSerial ,Convert(date,RequestDate) as RequestDate,Remarks from SS_PhysiciansPrivilegingFormRequest  where  SS_PhysiciansPrivilegingFormRequest.ID=" & RequestSerial & ""
        ...
        Else
            Try
                ...
                Dim str1 As String
                'str1 = "select " & EmpName & " as EmployeeName,( case when SS_UserActions.ActionAraName is not null then SS_UserActions.ActionAraName else 'Pending ...' end) As Action ,convert(varchar, ActionDate,101) as ActionDate,ActionRemarks  from SS_PhysiciansPrivilegingFormRequest join SS_RequestActions on SS_PhysiciansPrivilegingFormRequest.ID=SS_RequestActions.RequestSerial and SS_PhysiciansPrivilegingFormRequest.EmployeeID=SS_RequestActions.EmployeeID join hrs_Employees on hrs_Employees.ID= SS_RequestActions.SS_EmployeeID left join SS_UserActions on SS_RequestActions.ActionID=SS_UserActions.ID where RequestSerial=" & RequestSerial & " And SS_RequestActions.FormCode='" & FormCode & "' and ( SS_RequestActions.IsHidden is null or SS_RequestActions.IsHidden=0 ) and SS_PhysiciansPrivilegingFormRequest.EmployeeID=" & ClsEmployees.ID & " "
                str1 = "select " & EmpName & " as EmployeeName,( case when " & ActionName & " is not null then " & ActionName & " else 'Pending ...' end) As Action ,convert(varchar, ActionDate,101) as ActionDate,ActionRemarks  from SS_PhysiciansPrivilegingFormRequest join SS_RequestActions on SS_PhysiciansPrivilegingFormRequest.ID=SS_RequestActions.RequestSerial and SS_PhysiciansPrivilegingFormRequest.EmployeeID=SS_RequestActions.EmployeeID join hrs_Employees on hrs_Employees.ID= SS_RequestActions.SS_EmployeeID left join SS_UserActions on SS_RequestActions.ActionID=SS_UserActions.ID where RequestSerial=" & RequestSerial & " And SS_RequestActions.FormCode='" & FormCode & "' and ( SS_RequestActions.IsHidden is null or SS_RequestActions.IsHidden=0 ) "
    ...
    Private Sub PerformCancelRequest(FormCode As String, RequestSerial As String, ConfigID As Integer, ConnStr As String, includeCancelReason As Boolean)
        ...
        If includeCancelReason Then
            ...
            Try
                Dim strRequestEmp As String = " select EmployeeID from SS_PhysiciansPrivilegingFormRequest where ID=" & RequestSerial & ""
        ...
        Dim UpdateCommandRank As String = "UPDATE SS_PhysiciansPrivilegingFormRequest SET [RequestStautsTypeID] = 5 WHERE ID=" & RequestSerial & ""
