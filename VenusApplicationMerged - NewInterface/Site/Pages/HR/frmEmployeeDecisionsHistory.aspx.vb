...
Partial Class frmEmployeeDecisionsHistory
    Inherits MainPage
    ...
    Public Sub GetAllEmployeeRequests(ByVal filter As String)
        Try
            ...
            Else
                ...
                sqlStr += " Select " & EmpName & " as EmpName,'" & colName & "' as DecisionName ," & current & " as CurrentDecision ," & previous & " as PreviousDecision,hrs_EmployeesDecisions.RegDate," & regUser & " as RegUser FROM hrs_EmployeesDecisions INNER JOIN sys_Users ON hrs_EmployeesDecisions.RegUserID = sys_Users.ID where 1=1 " + filter
            End If
            Dim User As String = String.Empty
            Dim WebHandler As New Venus.Shared.Web.WebHandler
            WebHandler.GetCookies(Page, "UserID", User)
