...
#Region "Protected Sub"
    ...
    Protected Sub ImageButton_Save_Click(sender As Object, e As System.Web.UI.ImageClickEventArgs) Handles ImageButton_Save.Click
        Try
            ...
            cmdstring = "set dateformat dmy; update hrs_VacationsBalance set EndServiceDate = '" & DteEndOfServiceDate & "' where EmployeeID = " & IntEmployeeId
            Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteNonQuery(ClsEmployee.ConnectionString, CommandType.Text, cmdstring)
            '==========================================================================
...
#Region "Private Function"
    ...
    Private Sub SetData(ByVal IntEmployeeId As Integer, ByVal BaseDate As Date)
        ...
        If ClsEmployeeClass.AdvanceBalance Then
            ...
            'remain = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteScalar(ClsContracts.ConnectionString, Data.CommandType.Text, strAdv)

            'strAdv = "SELECT    Balance FROM hrs_VacationsBalance where hrs_VacationsBalance.EndServiceDate IS NULL and BalanceTypeID<>2 and EmployeeID=" & ClsEmployees.ID & " and ExpireDate >='" & CDate(wdcEndOfServiceDate.Value).ToString("yyyy-MM-dd") & "' and DueDate<='" & CDate(wdcEndOfServiceDate.Value).ToString("yyyy-MM-dd") & "'"
            'Dim Balance As Decimal
            'Balance = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteScalar(ClsContracts.ConnectionString, Data.CommandType.Text, strAdv)


            Dim StrSelectCommand = "SELECT    Balance, Consumed, Remaining, BalanceTypeID, ExpireDate, DueDate FROM hrs_VacationsBalance where hrs_VacationsBalance.EndServiceDate IS NULL and EmployeeID=" & ClsEmployees.ID & " and ExpireDate >='" & CDate(wdcEndOfServiceDate.Value).ToString("yyyy-MM-dd") & "' and DueDate<='" & CDate(wdcEndOfServiceDate.Value).ToString("yyyy-MM-dd") & "' and (CancelDate is null or CancelDate>'" & CDate(wdcEndOfServiceDate.Value).ToString("yyyy-MM-dd") & "') and ISNULL(Posted,0)=0"

            StrSelectCommand = StrSelectCommand '& orderByStr
            Dim mSqlDataAdapter As New SqlClient.SqlDataAdapter
            mSqlDataAdapter = New SqlClient.SqlDataAdapter(StrSelectCommand, ClsContracts.ConnectionString)
            ...
            If DataSet.Tables(0).Rows.Count > 0 Then
                For i = 0 To DataSet.Tables(0).Rows.Count - 1
                    ...
                    If CInt(DataSet.Tables(0).Rows(i).Item("BalanceTypeID")) = 1 Then
                        lblNewBalance.Text = Math.Round(myBallance, 2).ToString()

                    End If

                    If CInt(DataSet.Tables(0).Rows(i).Item("BalanceTypeID")) = 3 Then
                        lblCompData.Text = Math.Round(myBallance, 2).ToString()
            ...
            lblTransferBalance.Visible = True
            lblNewYear.Visible = True
            lblTransferYear.Visible = True

            lblCompLbl.Visible = True
            lblCompData.Visible = True
        Else
            lblNewBalance.Visible = False
            lblTransferBalance.Visible = False
            lblNewYear.Visible = False
            lblTransferYear.Visible = False

            lblCompLbl.Visible = False
            lblCompData.Visible = False
        End If
        If ProfileCls.CurrentLanguage = "Ar" Then
            lblNewYear.Text = "جديد" & " :"
            lblTransferYear.Text = "مرحل" & " :"
