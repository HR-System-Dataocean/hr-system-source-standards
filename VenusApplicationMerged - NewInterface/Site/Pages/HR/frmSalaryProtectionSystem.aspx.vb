                        ...
                        For Each ObjRow In UwgSearchEmployees.Rows
                            Try
                                ...
                                If ObjRow.Cells.FromKey("Select").Value Then
                                    ...
                                    insertCommand = "INSERT INTO [dbo].[hrs_ProtectionSalaryPrepared]([FisicalPeriod],[EmployeeCode],[Prepared])VALUES(" & period & "," & EmployeeCode & ",1)"
    ...
    Private Function LoadData(ByRef dt As DataTable, Optional ByVal command As String = "") As Boolean
        Try
            ...
            If command = "Special" Then
                ...
                If ConfigurationManager.AppSettings("overTimeWPSystem")=10 Then
                    strCommand = "Set DateFormat DMY;" &
                    "select B.EngName AS 'Bank Name', Char(39) + isnull(e.BankAccountNumber,'') AS 'Account Number'," &
                    "dbo.fn_GetEmpName(e.Code,0) AS 'Employee EngName',dbo.fn_GetEmpName(e.Code,1) AS 'Employee ArbName'," &
                    "e.Code AS 'Employee Number',isnull(e.SSnNo,'') AS 'National ID Number'," &
                    "dbo.fn_GetOverTime(e.ID," & ClsFisicalPeriods.ID & ") AS 'salary'," &
                     "dbo.fn_GetBasicSalarywps(e.ID," & ClsFisicalPeriods.ID & ") AS 'Basic salary'," &
                    "'0' AS 'Housing','0' AS 'Other Earning'," &
                    "dbo.fn_GetOverTimeDeduction(e.ID," & ClsFisicalPeriods.ID & ") 'Deductions'," &
                    "isnull(e.Mobile,'') AS 'Mobile' from hrs_EmployeesTransactions " &
                    "et inner join hrs_Employees e on e.ID = et.EmployeeID " &
                    "INNER JOIN sys_FiscalYearsPeriodsModules as m ON m.FiscalYearPeriodID=et.FiscalYearPeriodID " &
                    "INNER JOIN sys_Banks B on B.ID = e.BankID " &
                    "where  m.ModuleID=" & GetModuleID("frmPrepareSalaries") &
                    " And IsNull(m.CloseDate,'')='' And et.FiscalYearPeriodID=" & ClsFisicalPeriods.ID &
                    " And et.PrepareType ='N' and e.ID In " &
                    "(select * from dbo.GetEmpOnBankTransfeer('B','EWA',et.FiscalYearPeriodID)) " &
                    "And dbo.fn_GetOverTime(e.ID," & ClsFisicalPeriods.ID & ")>0"
                End If
                'Dim dsEmployee1 As New Data.DataSet
                'Dim SqlConn As New System.Data.SqlClient.SqlConnection(ConfigurationManager.AppSettings(0).ToString())
                'Dim adapter As System.Data.SqlClient.SqlDataAdapter = New System.Data.SqlClient.SqlDataAdapter()
        ...
        Try
            ...
            If ddlStatues.SelectedValue > 0 Then
                If ddlStatues.SelectedValue = 1 Then
                    strFilter &= " And isnull(pr.Prepared,0) = 1 "
                End If
                If ddlStatues.SelectedValue = 2 Then
                    strFilter &= " And pr.Prepared is null "
                End If
            End If
            Dim strCommand As String
            'If ConfigurationManager.AppSettings("WPSystem") = 1 Then
            strCommand = "Set DateFormat DMY; select distinct et.ID,et.EmployeeID,e.Code as Code," & "dbo.fn_GetEmpName(e.Code," & ClsNavigationHandler.SetLanguage(Page, "0/1") & ") AS FullName, round(dbo.fn_GetNetSalary(e.ID," & ClsFisicalPeriods.ID & "),0) AS NetSalary,ISNULL(pr.Prepared,0) as Prepared  from hrs_EmployeesTransactions et inner join hrs_Employees e on e.ID = et.EmployeeID INNER JOIN sys_FiscalYearsPeriodsModules as m ON m.FiscalYearPeriodID=et.FiscalYearPeriodID left outer join hrs_ProtectionSalaryPrepared as pr on et.FiscalYearPeriodID=pr.FisicalPeriod and e.Code=pr.EmployeeCode where  m.ModuleID=" & GetModuleID("frmPrepareSalaries") & " And IsNull(m.CloseDate,'')='' And et.FiscalYearPeriodID=" & ClsFisicalPeriods.ID & " And et.PrepareType ='N' and round(dbo.fn_GetNetSalary(e.ID," & ClsFisicalPeriods.ID & "),0) > 0 and e.ID In (select * from dbo.GetEmpOnBankTransfeer('B','EWA',et.FiscalYearPeriodID))"
            strCommand &= strFilter
            'Else
...
#End Region
    ...
    Private Sub ddlBankAccountType_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlBankAccountType.SelectedIndexChanged
