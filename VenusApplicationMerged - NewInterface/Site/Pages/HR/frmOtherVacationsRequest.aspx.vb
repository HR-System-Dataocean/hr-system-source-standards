...
#Region "Protected Sub"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                ClsVacationTypes.GetDropDownList(DdlVacationType, True, "IsAnnual=0 and ISNULL(ExcludedFromSSRequests,0)=0")
                ...
                'mousa
                Dim strRequiredAttach As String
                strRequiredAttach = "select isnull(RequiredAttach,0) from SS_RequestTypes where RequestCode='SS_0013'"
                Dim RequiredAttach As Boolean
                RequiredAttach = CBool(Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteScalar(ClsEmployees.ConnectionString, Data.CommandType.Text, strRequiredAttach))
                If RequiredAttach Then
                    AttachPnl.Visible = True
                Else
                    AttachPnl.Visible = False
                End If
        ...
        End Try
    End Sub
    Protected Sub AddButton_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs) Handles AddButton.Command

        Dim newRow As New Infragistics.WebUI.UltraWebGrid.UltraGridRow()
        UwgEmployeeDocuments.Rows.Add(newRow)

        Dim DocNumber As Integer
        If Not String.IsNullOrEmpty(txtDocumentNumber0.Text) Then
            DocNumber = CInt(txtDocumentNumber0.Text)
        End If

        newRow.Cells.FromKey("DocumentNumber").Value = DocNumber
        newRow.Cells.FromKey("FileName").Value = txtAttachedFile.FileName
        ' newRow.Cells.FromKey("ReferenceNumber").Value = txtReferanceNumber.Text
        newRow.Cells.FromKey("FileBytes").Value = txtAttachedFile.FileBytes

        'newRow.Cells.("Actions").Value = "Delete"
        SaveFileTemp(txtAttachedFile.FileName, txtAttachedFile.FileBytes)

        txtDocumentNumber0.Text = ""
    End Sub
    Private Sub SaveFileTemp(fileName As String, fileBytes As Byte())
        If Session("TempFiles") Is Nothing Then
            Session("TempFiles") = New Dictionary(Of String, Byte())()
        End If

        Dim tempFiles As Dictionary(Of String, Byte()) = CType(Session("TempFiles"), Dictionary(Of String, Byte()))
        tempFiles(fileName) = fileBytes
    End Sub
    ...
    '    Dim ClsDocumentsInformations As Clssys_DocumentsInformations
    ...
    Private Function SaveAttachment(RequestID As Integer, FileName As String, DocumentNumber As String) As Boolean
        Dim ClsObjects As New Clssys_Objects(Me.Page)
        Dim ClsEmployeesVacations As New Clshrs_EmployeesVacations(Me.Page)
        ...
        Dim ClsDocumentAttachment As Clssys_DocumentsInformationAttachments
        ClsDocumentAttachment = New Clssys_DocumentsInformationAttachments(Page)


        Dim ObjNavigationHandler As New Venus.Shared.Web.NavigationHandler(ClsDocumentsInformations.ConnectionString)
        Dim TableName As String = "SS_VacationRequest"
        Dim ItemID As Integer
        ...
        Dim IntRecordId As Integer
        ClsObjects.Find(" Code ='" & TableName & "'")
        Dim ObjectID As Integer = ClsObjects.ID
        ClsObjects.Find(" Code ='sys_DocumentsDetails'")
        Dim DocumentObjectID As Integer = ClsObjects.ID
        ...
        If Not DocumentsInformationsAssignValue(ClsDocumentsInformations) Then
            Exit Function
        End If
        ...
        If FileName <> "" Then
            ...
            End If
            Dim arrTempFileName As String() = FileName.Split(".")
            ...
            ClsDocumentsInformations.Record = RequestID
            ClsDocumentsInformations.DocumentNumber = DocumentNumber
            ...
            If FileName <> "" Then
                If Not ClsDocumentAttachment.Find("Recordid=" & IntRecordId & " And ObjectID=" & ObjectID & " And IsImageView Is Null ") Then
                    AssignValueAttachment(ClsDocumentAttachment, FileName)
                    ...
                    ClsDocumentAttachment.DbObject = DocumentObjectID
    ...
    Private Function AssignValueAttachment(ByRef ClsDocumentAttachment As Clssys_DocumentsInformationAttachments, ByRef FileName As String) As Boolean
        ClsDocumentAttachment.DbObject = GetDoc_DetailsObjectId()
        'ClsDocumentAttachment.RecordId = RequestID

        ClsDocumentAttachment.FolderName = "Uploads"
        ClsDocumentAttachment.FileName = FileName
        ClsDocumentAttachment.EngName = FileName.ToString.Substring(0, FileName.ToString.IndexOf(".") - 1)
        ClsDocumentAttachment.ArbName = FileName.ToString.Substring(0, FileName.ToString.IndexOf(".") - 1)
        Return True
    End Function
    ...
    Protected Sub lbRemainVal_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles lbRemainVal.TextChanged
        ...
        End Try
    End Sub
    Protected Sub DdlVacationType_SelectedIndexChanged(sender As Object, e As EventArgs) Handles DdlVacationType.SelectedIndexChanged
...
#Region "Private Functions"
    Private Function SetNew() As Boolean
        ...
        Try
            ...
            UwgEmployeeDocuments.DataSource = Nothing
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            Dim str1 As String = "select SS_VacationRequest.ID,SS_VacationRequest.VacationType ,SS_VacationRequest.Code as RequestSerial ,SS_VacationRequest.EmployeeID ," & EmpName & " as EmployeeName,SS_VacationRequest.RequestDate,SS_VacationRequest.VacationType ,case when  SS_VacationRequest.VacationType='SS_0011' and SS_VacationRequest.VacationTypeID =1 then 'اجازة سنوي اداري' when SS_VacationRequest.VacationType='SS_0012' and SS_VacationRequest.VacationTypeID =1 then 'اجازة سنوي طبي' else  hrs_VacationsTypes.ArbName  end as RequestType ,case when  SS_VacationRequest.VacationType='SS_0011' and SS_VacationRequest.VacationTypeID =1 then 'SS_0011' when SS_VacationRequest.VacationType='SS_0012' and SS_VacationRequest.VacationTypeID =1 then 'SS_0012' else  'SS_0013'  end as FormCode from SS_VacationRequest join hrs_Employees on SS_VacationRequest.EmployeeID=hrs_Employees.id join hrs_VacationsTypes on SS_VacationRequest.VacationTypeID= hrs_VacationsTypes.ID where  EmployeeID= " & ClsEmployees.ID & "  and VacationType='SS_0013' Order By SS_VacationRequest.RequestDate desc"
    ...
    Private Function SavePart() As Boolean
        ...
        Try
            ...
            Dim ClsVacations As New Clshrs_VacationsTypes(Page)
            ClsVacations.Find(" ID=" & DdlVacationType.SelectedItem.Value)
            If ClsVacations.ConsiderAllowedDays And CInt(lbRemainVal.Text) > ClsVacations.AllowedDaysNo Then
    ...
    Public Function SaveAnnualVacationRequest() As Boolean
        ...
        Try
            ...
            If ClsEmployeesVacations2.GetEmployeeLastVacation(ClsEmployees.ID) Then
                ...
                End If
            End If
            If UwgEmployeeDocuments.Rows.Count < 1 And AttachPnl.Visible Then
                Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "The application files must be attached. /يجب ارفاق الملفات الخاصة بالطلب "))
                Exit Function
            End If
            ...
            DteVacationDate = WebDateChooser2.Value
            dsPrevVac = ClsEmployeeVacation.GetEmployeeVacationPerYear(CDate(WebDateChooser2.Value).Year, ClsEmployees.ID, -1)
            If dsPrevVac.Tables(0).Rows.Count > 0 Then
                For index = 0 To dsPrevVac.Tables(0).Rows.Count - 1
                    If (dsPrevVac.Tables(0).Rows(index).Item("ActualEndDate") >= WebDateChooser1.Value And dsPrevVac.Tables(0).Rows(index).Item("ActualStartDate") <= WebDateChooser1.Value) Or (dsPrevVac.Tables(0).Rows(index).Item("ActualEndDate") >= WebDateChooser2.Value And dsPrevVac.Tables(0).Rows(index).Item("ActualStartDate") <= WebDateChooser2.Value) Or (WebDateChooser2.Value >= dsPrevVac.Tables(0).Rows(index).Item("ActualStartDate") And WebDateChooser1.Value <= dsPrevVac.Tables(0).Rows(index).Item("ActualStartDate")) Or (WebDateChooser2.Value >= dsPrevVac.Tables(0).Rows(index).Item("ActualEndDate") And WebDateChooser1.Value <= dsPrevVac.Tables(0).Rows(index).Item("ActualEndDate")) Then
                        Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "There is a vacation in this period / يوجد اجازة مسجلة بالفعل فى هذه الفترة  "))
            ...
            '''''''''''''''''''''''''''''عدد المرات
            Dim strNoOfTimes As String
            strNoOfTimes = "select NoOfTimes from SS_RequestTypes where RequestCode='SS_0013'"
            Dim NoOfTimes As Integer
            ...
            strTimesPeriodPerMonth = "select TimesPeriodPerMonth from SS_RequestTypes where RequestCode='SS_0013'"
            ...
            'Dim strselect = "select SS_VacationRequest.ID as noActionsId from SS_VacationRequest inner join SS_RequestActions on SS_VacationRequest.VacationType=SS_RequestActions.FormCode and SS_VacationRequest.ID=SS_RequestActions.RequestSerial where SS_RequestActions.ActionID is null and (SS_RequestActions.FormCode='SS_0011' or SS_RequestActions.FormCode='SS_0012' or SS_RequestActions.FormCode='SS_0013' or SS_RequestActions.FormCode='SS_0018') and SS_VacationRequest.EmployeeID=" & ClsEmployees.ID
            'Dim DsRequestNoAction As DataSet = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(ClsEmployees.ConnectionString, CommandType.Text, strselect)
            ...
            WebHandler.GetCookies(Page, "UserID", User)
            InsertCommand = "Insert Into SS_VacationRequest Values(''," & ClsEmployees.ID & "," & DdlVacationType.SelectedItem.Value & ",'SS_0013',convert(datetime, '" & ddlRequestDate.Value & "',103),convert(datetime, '" & WebDateChooser1.Value & "',103) ,convert(datetime, '" & WebDateChooser2.Value & "',103),0, " & lbRemainVal.Text & " ,'" & txtContactNo.Text & "', ' ','" & TxtRemarks.Text & "'," & User & " ,convert(datetime,  '" & ClsEmployeesVacations.GetHigriDate(Date.Now) & "' ,103),'" & txtRequesterUser.Text & "',3) ; Select SCOPE_IDENTITY()"
            ...
            RequestID = CInt(SqlCommand.ExecuteScalar())
            SqlCommand.Connection.Close()
            For Each row In UwgEmployeeDocuments.Rows
                If Not String.IsNullOrEmpty(row.Cells.FromKey("DocumentNumber").Value) Then
                    SaveAttachment(RequestID, row.Cells.FromKey("FileName").Value, row.Cells.FromKey("DocumentNumber").Value)
                End If
            Next
                ...
                For Each Row As Data.DataRow In DsFirstLevel.Tables(0).Rows
                    ...
                    If Row("UserTypeID") = 2 Then
                        ...
                        If DsPositionEmployees.Tables(0).Rows.Count > 0 Then
                            Dim strinsert As String
                            Dim strMaxRequestSerial As String
                            strMaxRequestSerial = "select isnull(Max (ID),1) from SS_VacationRequest where EmployeeID=" & ClsEmployees.ID & " and VacationType='SS_0013' "
                            Dim MaxSerial As Integer
                            SqlCommand = New SqlClient.SqlCommand
                            SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                            SqlCommand.CommandType = CommandType.Text
                            ...
                            For Each RW In DsPositionEmployees.Tables(0).Rows
                                ...
                                End If
                                strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & RW("EmployeeID") & ",'SS_0013'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                                SqlCommand = New SqlClient.SqlCommand
                                SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                                SqlCommand.CommandType = CommandType.Text
                                SqlCommand.CommandText = strinsert
                    ...
                    If Row("UserTypeID") = 3 Then
                        ...
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & Row("EmployeeID") & ",'SS_0013'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                        SqlCommand.CommandText = strinsert
    ...
    Private Sub setOverDueDays(ClsVacationTypes As Clshrs_VacationsTypes, dat_NEW_RETURN As Date)
        ...
        End If
    End Sub
    Private Function DocumentsInformationsAssignValue(ByRef ClsDocumentsInformations As Clssys_DocumentsInformations) As Boolean
        Dim TableName As String = "SS_VacationRequest"
        Dim ItemID As Integer
...
#End Region
    ...
    'Protected Sub LinkButton1_Click(sender As Object, e As EventArgs) Handles LinkButtgon1.Click
    '    Dim ObjNavigationHandler As New Venus.Shared.Web.NavigationHandler(ClsEmployees.ConnectionString)
