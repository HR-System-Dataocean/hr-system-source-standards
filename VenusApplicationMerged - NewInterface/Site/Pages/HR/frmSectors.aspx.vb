...
#Region "Protected Sub"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If ClsObjects.Find(" Code='" & ClsSectors.Table.Trim & "'") Then
            ...
            If Not IsPostBack Then
                Page.Session.Add("ConnectionString", ClsSectors.ConnectionString)
                ClsSectors.AddOnChangeEventToControls("frmSectors", Page, UltraWebTab1)
                ...
                ClsSectors.GetDropDownList(ddl_MainDepartment, True)
                uwgDepartment.Rows.Add(New Infragistics.WebUI.UltraWebGrid.UltraGridRow)
            End If
            '================================== Add DateUpdateSchedules [Start]
            Dim IntrecordID As Integer
            If (txtCode.Text <> "") Then
                ClsSectors.Find(" Code='" & txtCode.Text & "'")
                IntrecordID = ClsSectors.ID
        ...
        Catch ex As Exception
            ...
            mErrorHandler.RecordExceptions_DataBase("", ex, Err.Number, ClsSectors.RegUserID, Venus.Shared.ErrorsHandler.eRecordingType.System_DataBase)
    ...
    Protected Sub ImageButton_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs) Handles ImageButton_Save.Command, ImageButton_SaveN.Command, LinkButton_SaveN.Command, ImageButton_New.Command, ImageButton_Print.Command, ImageButton_Properties.Command, LinkButton_Properties.Command, ImageButton_Remarks.Command, LinkButton_Remarks.Command, ImageButton_Last.Command, ImageButton_Next.Command, ImageButton_Back.Command, ImageButton_First.Command, ImageButton_Delete.Command
        ...
        Dim ObjNavigationHandler As New Venus.Shared.Web.NavigationHandler(ClsSectors.ConnectionString)
        Select Case e.CommandArgument
            ...
            Case "New"
                AfterOperation()
            Case "Delete"
                ClsSectors.Delete("Code='" & txtCode.Text & "'")
                AfterOperation()
            Case "Property"
                ClsSectors.Find("Code='" & txtCode.Text & "'")
                Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmPropertyScreen.aspx?ID=" & ClsSectors.ID & "&TableName=" & ClsSectors.Table, 477, 313, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Property", False)
            Case "Remarks"
                ClsSectors.Find("Code='" & txtCode.Text & "'")
                Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmRemarks.aspx?ID=" & ClsSectors.ID & "&TableName=" & ClsSectors.Table, 410, 210, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Remarks", False)
            Case "Print"
            ...
            Case "First"
                ClsSectors.Find("Code='" & txtCode.Text & "'")
                ClsSectors.FirstRecord()
                GetValues(ClsSectors)
            Case "Previous"
                ClsSectors.Find("Code='" & txtCode.Text & "'")
                If Not ClsSectors.previousRecord() Then
                    ClsSectors.Find("Code='" & txtCode.Text & "'")
            ...
            Case "Next"
                ClsSectors.Find("Code='" & txtCode.Text & "'")
                If Not ClsSectors.NextRecord() Then
                    ClsSectors.Find("Code='" & txtCode.Text & "'")
...
#Region "Private Functions"
    Private Function SavePart() As Boolean
        Dim StrMode As String = Request.QueryString.Item("Mode")
        ClsSectors = New ClsSys_Sectors(Page)

        Try
            ClsSectors.Find("Code='" & txtCode.Text & "'")

            If ClsSectors.ID > 0 Then
                If Not AssignValue(ClsSectors) Then
                    Exit Function
                End If
                ClsSectors.Update("Code='" & txtCode.Text & "'")
            ...
            ClsSectors.Find("Code='" & txtCode.Text & "'")
            ClsSectors.SaveAllDepartment(ClsSectors.ID, uwgDepartment)

            clsMainOtherFields = New clsSys_MainOtherFields(Me.Page)
            clsMainOtherFields.CollectDataAndSave(value.Text, ClsSectors.Table, ClsSectors.ID)
            value.Text = ""
            Return True
        Catch ex As Exception
            Return False
        End Try
    End Function
    Private Function AssignValue(ByRef ClsSectors As ClsSys_Sectors) As Boolean
        Try
            With ClsSectors
                .Code = txtCode.Text
        ...
        End Try
    End Function
    Private Function GetValues(ByVal ClsSectors As ClsSys_Sectors) As Boolean
        ...
        Try
            ...
            With ClsSectors
                ...
                uwgDepartment.DataSource = ClsSectors.GetAllDepartment(.ID).Tables(0).DefaultView
                uwgDepartment.DataBind()
                ...
                Dim str As String = ""
                GetChildSectors(.ID, .ConnectionString, str)
                str = str.Trim(",")
                ...
                If Page.IsPostBack Then
                    CreateOtherFields(ClsSectors.ID)
                End If
            End With
            Return True
        Catch ex As Exception
        End Try
    End Function
    Private Function GetChildSectors(ByVal SectorID As Integer, ByVal Connection As String, ByRef RetStr As String) As Boolean
        Dim strSelect As String = " Select ID from sys_Sectors where ParentID = " & SectorID
        ...
        If (DS.Tables(0).Rows.Count > 0) Then
            For Each row As Data.DataRow In DS.Tables(0).Rows
                ...
                GetChildSectors(row("ID"), Connection, RetStr)
            Next
        Else
            Return True
        End If
    ...
    Private Function CheckCode() As Boolean
        ...
        Try
            ...
            If ClsSectors.ID > 0 Then
                ...
                StrMode = "E"
            Else
                If ClsSectors.CheckRecordExistance(" Code='" & txtCode.Text & "'") Then
                    txtCode.Text = ""
                    txtCode.Focus()
                End If
                uwgDepartment.DataSource = ClsSectors.GetAllDepartment(ClsSectors.ID).Tables(0).DefaultView
                uwgDepartment.DataBind()
                Clear()
                ImageButton_Delete.Enabled = False
                ClsSectors.GetDropDownList(ddl_MainDepartment, True)
            ...
            End If
            SetToolBarDefaults()
            SetToolBarPermission(Me, ClsSectors.ConnectionString, ClsSectors.DataBaseUserRelatedID, ClsSectors.GroupID, StrMode)
            SetToolBarRecordPermission(Me, ClsSectors.ConnectionString, ClsSectors.DataBaseUserRelatedID, ClsSectors.GroupID, ClsSectors.Table, IntId)
    ...
    Private Sub LoadDataUpdateSchedules(ByVal formName As String)
        ...
        ClsSectors = New ClsSys_Sectors(Me.Page)
        ClsSectors.Find(" code = '" & txtCode.Text & "'")
        Dim recordID As Integer = ClsSectors.ID
        If (recordID > 0) Then
    ...
    Private Function CreateOtherFields(ByVal IntRecordID As Integer)
        ...
        clsMainOtherFields = New clsSys_MainOtherFields(Me.Page)
        If clsMainOtherFields.CheckOtherFieldsButton(Me.Page, ClsSectors.Table) = True Then
            Dim StrTablename As String
            ClsSectors = New ClsSys_Sectors(Me)
            StrTablename = ClsSectors.Table
