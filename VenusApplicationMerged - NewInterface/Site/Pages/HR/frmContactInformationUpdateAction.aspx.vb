...
Partial Class frmContactInformationUpdateAction
    Inherits MainPage
...
#Region "Protected Sub"
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            'lll
            Dim str1 As String
            str1 = "select " & EmpName & " as EmployeeName,( case when " & ActionName & " is not null then " & ActionName & " else 'Pending ...' end) As Action ,convert(varchar, ActionDate,103) as ActionDate,ActionRemarks  from SS_ContactInformationUpdate join SS_RequestActions on SS_ContactInformationUpdate.ID=SS_RequestActions.RequestSerial and SS_ContactInformationUpdate.EmployeeID=SS_RequestActions.EmployeeID join hrs_Employees on hrs_Employees.ID= SS_RequestActions.SS_EmployeeID left join SS_UserActions on SS_RequestActions.ActionID=SS_UserActions.ID where RequestSerial=" & RequestSerial & " and ( SS_RequestActions.IsHidden is null or SS_RequestActions.IsHidden=0 ) and FormCode='SS_001922' "
            ...
            strselect = "select SS_ContactInformationUpdate.ID as RequestSerial,SS_ContactInformationUpdate.EmployeeID,hrs_employees.Code as EmpCode," & EmpName & " as EmployeeName ,Convert(date,RequestDate) as RequestDate,SS_ContactInformationUpdate.Remarks, " & Position & " As Position , " & Department & " As Department,SS_ContactInformationUpdate.E_Mail,SS_ContactInformationUpdate.Phone,SS_ContactInformationUpdate.Mobile,SS_ContactInformationUpdate.AddressAsPerContract,hrs_employees.E_Mail as OldE_Mail,hrs_employees.Phone as OldPhone,hrs_employees.Mobile as OldMobile,hrs_employees.AddressAsPerContract as OldAddressAsPerContract FROM            dbo.SS_ContactInformationUpdate INNER JOIN dbo.hrs_Employees ON dbo.SS_ContactInformationUpdate.EmployeeID = dbo.hrs_Employees.ID INNER JOIN dbo.hrs_Contracts ON dbo.hrs_Employees.ID = dbo.hrs_Contracts.EmployeeID and( hrs_Contracts.EndDate>=getdate() or hrs_Contracts.EndDate is null)  INNER JOIN dbo.hrs_Positions ON dbo.hrs_Contracts.PositionID = dbo.hrs_Positions.ID INNER JOIN dbo.sys_Departments ON dbo.hrs_Employees.DepartmentID = dbo.sys_Departments.ID   where  SS_ContactInformationUpdate.ID=" & RequestSerial & ""
            ...
            txtEmail.Text = DS2.Tables(0).Rows(0)("E_Mail").ToString()
            txtPhone.Text = DS2.Tables(0).Rows(0)("Phone").ToString()
            txtMobile.Text = DS2.Tables(0).Rows(0)("Mobile").ToString()
            txtAddress.Text = DS2.Tables(0).Rows(0)("AddressAsPerContract").ToString()

            txtOldEmail.Text = DS2.Tables(0).Rows(0)("OldE_Mail").ToString()
            txtOldPhone.Text = DS2.Tables(0).Rows(0)("OldPhone").ToString()
            txtOldMobile.Text = DS2.Tables(0).Rows(0)("OldMobile").ToString()
            txtOldAddress.Text = DS2.Tables(0).Rows(0)("OldAddressAsPerContract").ToString()
            End If
    ...
    Protected Sub btnSave_Click(sender As Object, e As Infragistics.WebUI.WebDataInput.ButtonEventArgs) Handles btnSave.Click
        ...
        If ddlAction.SelectedValue > 0 Then
            ...
            If ddlAction.SelectedValue = 2 Then   'رفض
                ...
                UpdateCommand = "update SS_RequestActions set  seen=1 , ActionID=" & ddlAction.SelectedValue & " ,ActionDate= GETDATE(),ActionRemarks='" + TxtRemarks.Text + "'  where ConfigID=" & ConfigID & " And FormCode='SS_001922' and RequestSerial=" & RequestSerial & " and SS_EmployeeID=" & ClsEmployees_SS.ID & " And ActionID Is Null "
                ...
                UpdateCommandRank = "UPDATE SS_ContactInformationUpdate SET [RequestStautsTypeID] = 2 WHERE ID=" & RequestSerial & ""
                ...
                ClsEmployees.SendEmail("frmContactInformationUpdateAction", Me.Page, 1, "SS_RequestActions", actionSerial)
            ...
            If ddlAction.SelectedValue = 1 Then
                ...
                UpdateCommandRank1 = "UPDATE SS_ContactInformationUpdate SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
                ...
                If dsconfig.Tables(0).Rows.Count > 0 Then
                    ...
                    If CBool(dsconfig.Tables(0).Rows(0)("IsFinal")) Then
                        ...
                        Dim SqlCommandRank As Data.SqlClient.SqlCommand
                        Dim UpdateCommandRank As String = ""
                        UpdateCommandRank = "UPDATE SS_ContactInformationUpdate SET [RequestStautsTypeID] = 1 WHERE ID=" & RequestSerial & ""
                        SqlCommandRank = New SqlClient.SqlCommand
                        SqlCommandRank.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                        SqlCommandRank.CommandType = CommandType.Text
                        SqlCommandRank.CommandText = UpdateCommandRank
...
#End Region
    ...
    Private Function ProcessForwardForReview(ByVal RequestSerial As Integer, ByVal ConfigID As Integer, ByVal ReviewerCode As String, ByVal ActionRemarks As String, ByVal ConfirmedNoOfDays As String) As Boolean
        Try
            ...
            Dim UpdateStatus As String = "UPDATE SS_ContactInformationUpdate SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
