...
Partial Class frmBusinessORTrainingTravelAction
    Inherits MainPage
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            If ProfileCls.CurrentLanguage = "Ar" Then
                ...
                TravelReason = "SS_TravelReasons.ArbName"
            Else
                ...
                TravelReason = "SS_TravelReasons.EngName"

            End If

            'lll
            Dim str1 As String
            str1 = "select " & EmpName & " as EmployeeName,( case when " & ActionName & " is not null then " & ActionName & " else 'Pending ...' end) As Action ,convert(varchar, ActionDate,103) as ActionDate,ActionRemarks  from SS_BusinessORTrainingTravel join SS_RequestActions on SS_BusinessORTrainingTravel.ID=SS_RequestActions.RequestSerial and SS_BusinessORTrainingTravel.EmployeeID=SS_RequestActions.EmployeeID join hrs_Employees on hrs_Employees.ID= SS_RequestActions.SS_EmployeeID left join SS_UserActions on SS_RequestActions.ActionID=SS_UserActions.ID where RequestSerial=" & RequestSerial & " and ( SS_RequestActions.IsHidden is null or SS_RequestActions.IsHidden=0 ) and FormCode='SS_001927' "
            ...
            strselect = "select SS_BusinessORTrainingTravel.ID as RequestSerial,SS_BusinessORTrainingTravel.EmployeeID,hrs_employees.Code as EmpCode," & EmpName & " as EmployeeName ,Convert(date,RequestDate) as RequestDate,SS_BusinessORTrainingTravel.Remarks,CONVERT(varchar, SS_BusinessORTrainingTravel.FromDate,103) as FromDate,  CONVERT(varchar, SS_BusinessORTrainingTravel.ToDate, 103) as ToDate, " & Position & " As Position , " & Department & " As Department, CONVERT(varchar, hrs_Employees.JoinDate, 103) as JoinDate," & TravelReason & " as TravelReason FROM            dbo.SS_BusinessORTrainingTravel INNER JOIN dbo.hrs_Employees ON dbo.SS_BusinessORTrainingTravel.EmployeeID = dbo.hrs_Employees.ID INNER JOIN dbo.hrs_Contracts ON dbo.hrs_Employees.ID = dbo.hrs_Contracts.EmployeeID and( hrs_Contracts.EndDate>=getdate() or hrs_Contracts.EndDate is null)  INNER JOIN dbo.hrs_Positions ON dbo.hrs_Contracts.PositionID = dbo.hrs_Positions.ID INNER JOIN dbo.sys_Departments ON dbo.hrs_Employees.DepartmentID = dbo.sys_Departments.ID left join SS_TravelReasons on SS_TravelReasons.ID=SS_BusinessORTrainingTravel.TravelReasonID  where  SS_BusinessORTrainingTravel.ID=" & RequestSerial & ""
            ...
            TxtRequestSerial.Text = DS2.Tables(0).Rows(0)("RequestSerial").ToString()
                ...
                txtDromDate.Text = DS2.Tables(0).Rows(0)("FromDate").ToString()
                txtToDate.Text = DS2.Tables(0).Rows(0)("ToDate").ToString()
                TxtTravelReason.Text = DS2.Tables(0).Rows(0)("TravelReason").ToString()
                TxtRequestRemarks.Text = DS2.Tables(0).Rows(0)("Remarks").ToString()
                'TxtJoinDate.Text = DS2.Tables(0)("JoinDate").ToString()
                Dim joinDateValue As Object = DS2.Tables(0).Rows(0)("JoinDate").ToString()
    ...
    Protected Sub btnSave_Click(sender As Object, e As Infragistics.WebUI.WebDataInput.ButtonEventArgs) Handles btnSave.Click
        ...
        If ddlAction.SelectedValue > 0 Then
            ...
            If ddlAction.SelectedValue = 2 Then   'رفض
                ...
                UpdateCommand = "update SS_RequestActions set  seen=1 , ActionID=" & ddlAction.SelectedValue & " ,ActionDate= GETDATE(),ActionRemarks='" + TxtRemarks.Text + "'  where ConfigID=" & ConfigID & " And FormCode='SS_001927' and RequestSerial=" & RequestSerial & " and SS_EmployeeID=" & ClsEmployees_SS.ID & " And ActionID Is Null"
                ...
                UpdateCommandRank = "UPDATE SS_BusinessORTrainingTravel SET [RequestStautsTypeID] = 2 WHERE ID=" & RequestSerial & ""
                ...
                ClsEmployees.SendEmail("frmBusinessORTrainingTravelAction", Me.Page, 1, "SS_RequestActions", actionSerial)
            ...
            If ddlAction.SelectedValue = 1 Then
                ...
                UpdateCommandRank1 = "UPDATE SS_BusinessORTrainingTravel SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
                ...
                If dsconfig.Tables(0).Rows.Count > 0 Then
                    ...
                    If CBool(dsconfig.Tables(0).Rows(0)("IsFinal")) Then
                        ...
                        Dim SqlCommandRank As Data.SqlClient.SqlCommand
                        Dim UpdateCommandRank As String = ""
                        UpdateCommandRank = "UPDATE SS_BusinessORTrainingTravel SET [RequestStautsTypeID] = 1 WHERE ID=" & RequestSerial & ""
                        SqlCommandRank = New SqlClient.SqlCommand
                        SqlCommandRank.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                        SqlCommandRank.CommandType = CommandType.Text
                        SqlCommandRank.CommandText = UpdateCommandRank
...
#End Region
    ...
    Private Function ProcessForwardForReview(ByVal RequestSerial As Integer, ByVal ConfigID As Integer, ByVal ReviewerCode As String, ByVal ActionRemarks As String, ByVal ConfirmedNoOfDays As String) As Boolean
        Try
            ...
            Dim UpdateStatus As String = "UPDATE SS_BusinessORTrainingTravel SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
