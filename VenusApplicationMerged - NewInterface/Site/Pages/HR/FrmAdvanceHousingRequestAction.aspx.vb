...
#Region "Protected Sub"
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            'lll
            Dim str1 As String
            str1 = "select " & EmpName & " as EmployeeName,( case when " & ActionName & " is not null then " & ActionName & " else 'Pending ...' end) As Action ,convert(varchar, ActionDate,103) as ActionDate,ActionRemarks,SS_RequestActions.OvertimeDate  from SS_AdvanceHousingRequest join SS_RequestActions on SS_AdvanceHousingRequest.ID=SS_RequestActions.RequestSerial and SS_AdvanceHousingRequest.EmployeeID=SS_RequestActions.EmployeeID join hrs_Employees on hrs_Employees.ID= SS_RequestActions.SS_EmployeeID left join SS_UserActions on SS_RequestActions.ActionID=SS_UserActions.ID where RequestSerial=" & RequestSerial & " and ( SS_RequestActions.IsHidden is null or SS_RequestActions.IsHidden=0 ) and FormCode='SS_001913' "
            ...
            strselect = "select SS_AdvanceHousingRequest.ID as RequestSerial,SS_AdvanceHousingRequest.EmployeeID,hrs_employees.Code as EmpCode," & EmpName & " as EmployeeName ,Convert(date,RequestDate) as RequestDate,SS_AdvanceHousingRequest.Remarks, " & Position & " As Position , " & Department & " As Department,SS_AdvanceHousingRequest.InstallmentDate,SS_AdvanceHousingRequest.InstallmentsCount FROM            dbo.SS_AdvanceHousingRequest INNER JOIN dbo.hrs_Employees ON dbo.SS_AdvanceHousingRequest.EmployeeID = dbo.hrs_Employees.ID INNER JOIN dbo.hrs_Contracts ON dbo.hrs_Employees.ID = dbo.hrs_Contracts.EmployeeID and( hrs_Contracts.EndDate>=getdate() or hrs_Contracts.EndDate is null)  INNER JOIN dbo.hrs_Positions ON dbo.hrs_Contracts.PositionID = dbo.hrs_Positions.ID INNER JOIN dbo.sys_Departments ON dbo.hrs_Employees.DepartmentID = dbo.sys_Departments.ID   where  SS_AdvanceHousingRequest.ID=" & RequestSerial & ""
            ...
            Else
                ...
                If Not String.IsNullOrWhiteSpace(Convert.ToString(DS2.Tables(0).Rows(0)("InstallmentDate"))) Then
                    InstallmentDate.Value = CDate(DS2.Tables(0).Rows(0)("InstallmentDate").ToString())
                End If
                If Not String.IsNullOrWhiteSpace(Convert.ToString(DS2.Tables(0).Rows(0)("InstallmentsCount"))) Then
                    txtInstallmentsNo.Text = DS2.Tables(0).Rows(0)("InstallmentsCount").ToString()
                Else
                ...
                requesttypestr = "Select " & SelectRequestType & " from SS_RequestTypes where RequestCode='SS_001913'"
                Dim RequestType As String = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteScalar(ClsEmployees.ConnectionString, Data.CommandType.Text, requesttypestr)
    ...
    Protected Sub btnSave_Click(sender As Object, e As Infragistics.WebUI.WebDataInput.ButtonEventArgs) Handles btnSave.Click
        ...
        If ddlAction.SelectedValue > 0 Then
            ...
            If ddlAction.SelectedValue = 2 Then   'رفض
                ...
                UpdateCommandRank = "UPDATE SS_AdvanceHousingRequest SET [RequestStautsTypeID] = 2 WHERE ID=" & RequestSerial & ""
                ...
                ClsEmployees.SendEmail("FrmAdvanceHousingRequestAction", Me.Page, 1, "SS_RequestActions", actionSerial)
            ...
            If ddlAction.SelectedValue = 1 Then
                If String.IsNullOrWhiteSpace(InstallmentDate.Text) Then
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, objNav.SetLanguage(Page, " Please Enter Installment Date /برجاءإدخال تاريخ بداية الاستقطاع"))
                    Exit Sub
                End If
                If CheckDates(CDate(DateTime.Now.Date), CDate(InstallmentDate.Value)) = False Then
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, objNav.SetLanguage(Page, " Error in Installment Date should be after today  /تاريخ بداية الإستقطاع يجب ان يكون أكبر من تاريخ اليوم"))
                    Exit Sub
                End If
                If CheckAdvanceHousingContract() Then
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, objNav.SetLanguage(Page, " Housing allowance is not provided for in the current employee's contract terms  /لا يتوفر بدل السكن في بنود عقد الموظف الحالي"))
                    Exit Sub
                End If
                ...
                If CBool(dsconfig.Tables(0).Rows(0)("IsFinal")) Then
                    ...
                    ClsFisicalYearsPeriods.Find("FromDate <= '" & ClsEmployees.SetHigriDate(InstallmentDate.Value) & "' and ToDate >='" & ClsEmployees.SetHigriDate(InstallmentDate.Value) & "'")
                    ...
                    If Not String.IsNullOrWhiteSpace(Convert.ToString(clsFiscalYearsPeriodsModules.CloseDate)) Then
                        Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, objNav.SetLanguage(Page, "Installment Start Date in Closed Period !/!تاريخ الإستقطاع في فترة مغلقة"))
                        Exit Sub
                    End If
                    ...
                    If periodPrepared > 0 Then
                        Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, objNav.SetLanguage(Page, "Installment Start Date in Prepared Period !/!تاريخ الإستقطاع في فترة مجهزة"))
                        Exit Sub
                    End If
                End If
                Dim SqlCommandRank1 As Data.SqlClient.SqlCommand
                Dim UpdateCommandRank1 As String = ""
                UpdateCommandRank1 = "UPDATE SS_AdvanceSalaryRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
                ...
                If dsconfig.Tables(0).Rows.Count > 0 Then
                    ...
                    If CBool(dsconfig.Tables(0).Rows(0)("IsFinal")) Then
                        ...
                        Dim SqlCommandRank As Data.SqlClient.SqlCommand
                        Dim UpdateCommandRank As String = ""
                        UpdateCommandRank = "UPDATE SS_AdvanceHousingRequest SET [RequestStautsTypeID] = 1 WHERE ID=" & RequestSerial & ""
                        SqlCommandRank = New SqlClient.SqlCommand
                        SqlCommandRank.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                        SqlCommandRank.CommandType = CommandType.Text
                        SqlCommandRank.CommandText = UpdateCommandRank
                        ...
                        If dsRank.Tables(0).Rows.Count > 0 Then
                            For Each Row In dsRank.Tables(0).Rows
                                ...
                                If Row("UserTypeID") = 1 Then
                                    ...
                                    strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID,OvertimeDate,HoursCount)  values(" & TxtRequestSerial.Text & " , " & DirectManagerID & ",'" & dsconfig.Tables(0).Rows(0)("FormCode") & "'," & ClsEmployees.ID & ",0," & Row("ID") & ",convert(datetime, '" & InstallmentDate.Value & "',103)," & txtInstallmentsNo.Text & ")"
                                    SqlCommand = New SqlClient.SqlCommand
                                    SqlCommand.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                                    SqlCommand.CommandType = CommandType.Text
                                ...
                                If Row("UserTypeID") = 2 Then
                                    ...
                                    If DsPositionEmployees.Tables(0).Rows.Count > 0 Then
                                        For Each RW In DsPositionEmployees.Tables(0).Rows
                                            ...
                                            End If
                                            Dim strinsert As String
                                            strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID,OvertimeDate,HoursCount)  values(" & TxtRequestSerial.Text & " , " & RW("EmployeeID") & ",'" & dsconfig.Tables(0).Rows(0)("FormCode") & "'," & ClsEmployees.ID & ",0," & Row("ID") & ",convert(datetime, '" & InstallmentDate.Value & "',103)," & txtInstallmentsNo.Text & ")"
                                            SqlCommand = New SqlClient.SqlCommand
                                            SqlCommand.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                                            SqlCommand.CommandType = CommandType.Text
                                            SqlCommand.CommandText = strinsert
                                ...
                                If Row("UserTypeID") = 3 Then
                                    ...
                                    End If
                                    Dim strinsert As String
                                    strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID,OvertimeDate,HoursCount)  values(" & TxtRequestSerial.Text & " , " & Row("EmployeeID") & ",'" & dsconfig.Tables(0).Rows(0)("FormCode") & "'," & ClsEmployees.ID & ",0," & Row("ID") & ",convert(datetime, '" & InstallmentDate.Value & "',103)," & txtInstallmentsNo.Text & ")"
                                    SqlCommand = New SqlClient.SqlCommand
                                    SqlCommand.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                                    SqlCommand.CommandType = CommandType.Text
                                    SqlCommand.CommandText = strinsert
            ...
            If ddlAction.SelectedValue = 3 Then
                ...
                Dim SqlCommandRank As Data.SqlClient.SqlCommand
                Dim UpdateCommandRank As String = ""
                UpdateCommandRank = "UPDATE SS_AdvanceHousingRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
                SqlCommandRank = New SqlClient.SqlCommand
                SqlCommandRank.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                SqlCommandRank.CommandType = CommandType.Text
                SqlCommandRank.CommandText = UpdateCommandRank
                ...
                If dsconfig.Tables(0).Rows.Count > 0 Then
                    ...
                    strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID,OvertimeDate,HoursCount)  values(" & TxtRequestSerial.Text & " , " & clsDelegatedEmp.ID & ",'" & dsconfig.Tables(0).Rows(0)("FormCode") & "'," & ClsEmployees.ID & ",0," & dsconfig.Tables(0).Rows(0)("ID") & ",convert(datetime, '" & InstallmentDate.Value & "',103)," & txtInstallmentsNo.Text & ")"
                    SqlCommand = New SqlClient.SqlCommand
                    SqlCommand.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                    SqlCommand.CommandType = CommandType.Text
                    SqlCommand.CommandText = strinsert
    ...
    Private Function CheckAdvanceHousingContract() As Boolean
        ...
        ClsContractTransactions.Find(" ContractID = " & ClsContracts.ID & " AND  TransactionTypeID = " & ClsTransactionsType.ID)
        If ClsContractTransactions.ID > 0 Then
            If ClsContractTransactions.Amount < 1 Then
                Return True
            End If
        Else
            Return True
    ...
    Private Function SaveOverTimePayabilities(ByVal employeeId As Integer) As Boolean
        ...
        Dim ClsTransactionsType As New Clshrs_TransactionsTypes(Page)
        ClsTransactionsType.Find(" Code=247")

        ClsEmployeesPayability.TransactionTypeID = ClsTransactionsType.ID
        ClsEmployeesPayability.Number = StrSerial
        ClsEmployeesPayability.TransactionDate = ClsEmployeesPayability.SetHigriDate(DateTime.Now.Date)
    ...
    Private Function SaveInstalment(ByVal IntId As Integer, ByVal empId As Integer) As Boolean
        ...
        Try
            ...
            Dim ClsContracts As New Clshrs_Contracts(Me.Page)
            ClsContracts.Find(" EmployeeId = " & empId & " And Isnull(CancelDate,'')= '' and (EndDate is null or EndDate > getdate()) order by ID desc")

            Dim ClsContractTransactions = New Clshrs_ContractsTransactions(Page)
            ClsContractTransactions.Find(" ContractID = " & ClsContracts.ID & " AND  TransactionTypeID = " & ClsTransactionsType.ID)
            Dim OriginalLaonAmount As Double = LoanAmount
            Dim settlemntAmount As Double
            settlemntAmount = Convert.ToDouble(0)
            If ClsContractTransactions.ID > 0 Then
                settlemntAmount = Math.Round(Convert.ToDouble(ClsContractTransactions.Amount), 2)
            End If
            ...
            If LoanAmount > 0 Then
                ...
                Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteScalar(clsEmployeesPayability.ConnectionString, System.Data.CommandType.Text, StrSqlCommand)
            End If
            Dim StallCount As Integer = 6
            If txtInstallmentsNo.Text <> "" Then
                StallCount = CInt(txtInstallmentsNo.Text)
            End If

            If OriginalLaonAmount = 0 Then
                Dim currDate As Date

                For index = 1 To StallCount

                    currDate = clsEmployeesPayability.SetHigriDate(InstallmentDate.Value.AddMonths(index))
                    ...
                    StrSqlCommand &= "Insert Into hrs_EmployeesPayabilitiesSchedules(" &
                        ...
                        ")values(" &
                        IntId & ",'" &
                        currDate.ToString("dd/MM/yyyy") & "'," & settlemntAmount & "," &
                        clsEmployeesPayability.DataBaseUserRelatedID & "," &
                        clsEmployeesPayability.MainCompanyID &
                        ");" & vbNewLine
...
#End Region
    Private Function ProcessForwardForReview(ByVal RequestSerial As Integer, ByVal ConfigID As Integer, ByVal ReviewerCode As String, ByVal ActionRemarks As String, ByVal ConfirmedNoOfDays As String) As Boolean
        Try
            ...
            Dim UpdateStatus As String = "UPDATE SS_AdvanceHousingRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
            ...
            Dim strinsert As String
            strinsert = "Insert Into SS_RequestActions (RequestSerial, SS_EmployeeID, FormCode, EmployeeID, Seen, ConfigID, IsForwarded, ForwardedBy, OvertimeDate, HoursCount) " &
                        "values(" & RequestSerial & ", " & clsReviewerEmp.ID & ", '" & dsconfig.Tables(0).Rows(0)("FormCode") & "', " & ClsRequestEmployee.ID & ", 0, " & ConfigID & ", 1, " & _sys_User.RelEmployee & ", convert(datetime, '" & InstallmentDate.Value & "',103), " & txtInstallmentsNo.Text & ")"
    ...
    Private Function ProcessReviewerDecision(ByVal RequestSerial As Integer, ByVal ConfigID As Integer, ByVal ActionID As Integer, ByVal ActionRemarks As String, ByVal ConfirmedNoOfDays As String) As Boolean
        Try
            ...
            ' 5- الحصول على بيانات إضافية (OvertimeDate, HoursCount)
            Dim GetExtraData As String = "SELECT TOP 1 OvertimeDate, HoursCount FROM SS_RequestActions WHERE ConfigID=" & ConfigID & " AND RequestSerial=" & RequestSerial & " AND IsForwarded=1"
            Dim dsExtra As DataSet = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(ClsEmployees.ConnectionString, Data.CommandType.Text, GetExtraData)
            Dim OvertimeDate As Object = DBNull.Value
            Dim HoursCount As Object = DBNull.Value
            If dsExtra.Tables(0).Rows.Count > 0 Then
                OvertimeDate = dsExtra.Tables(0).Rows(0)("OvertimeDate")
                HoursCount = dsExtra.Tables(0).Rows(0)("HoursCount")
            End If
            ...
            ' 7- إدراج سطر جديد للمدير
            Dim SqlCommandInsert As Data.SqlClient.SqlCommand
            Dim InsertManager As String = ""
            If OvertimeDate Is DBNull.Value OrElse OvertimeDate Is Nothing Then
                InsertManager = "Insert Into SS_RequestActions (RequestSerial, SS_EmployeeID, FormCode, EmployeeID, Seen, ConfigID, HoursCount) " &
                                "values(" & RequestSerial & ", " & ForwardedBy & ", '" & FormCode & "', " & EmployeeID & ", 0, " & ConfigID & ", " & IIf(HoursCount Is DBNull.Value OrElse HoursCount Is Nothing, "0", HoursCount) & ")"
            Else
                InsertManager = "Insert Into SS_RequestActions (RequestSerial, SS_EmployeeID, FormCode, EmployeeID, Seen, ConfigID, OvertimeDate, HoursCount) " &
                                "values(" & RequestSerial & ", " & ForwardedBy & ", '" & FormCode & "', " & EmployeeID & ", 0, " & ConfigID & ", convert(datetime, '" & Convert.ToDateTime(OvertimeDate).ToString("dd/MM/yyyy") & "',103), " & IIf(HoursCount Is DBNull.Value OrElse HoursCount Is Nothing, "0", HoursCount) & ")"
            End If
            ...
            ' 8- تحديث حالة الطلب لـ "تحت الاجراء"
            Dim SqlCommandStatus As Data.SqlClient.SqlCommand
            Dim UpdateStatus As String = "UPDATE SS_AdvanceHousingRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
            SqlCommandStatus = New SqlClient.SqlCommand
            SqlCommandStatus.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
