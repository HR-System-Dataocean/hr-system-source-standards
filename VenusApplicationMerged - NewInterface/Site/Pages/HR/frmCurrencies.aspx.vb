...
#Region "Public Decleration"
    Private ClsCurrency As ClsSys_Currencies
...
#Region "Protected Sub"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ClsCurrency = New ClsSys_Currencies(Me.Page)
        Dim ClsObjects As New Clssys_Objects(Page)
        Dim ClsSearchs As New Clssys_Searchs(Page)
        Dim SearchID As Integer = 0
        Try
            Dim clsSearchsColumns As New Clssys_SearchsColumns(Page)
            If ClsObjects.Find(" Code='" & ClsCurrency.Table.Trim & "'") Then
            ...
            If Not IsPostBack Then
                Page.Session.Add("ConnectionString", ClsCurrency.ConnectionString)
                ClsCurrency.AddOnChangeEventToControls("frmCurrencies", Page, UltraWebTab1)
                ...
                Venus.Shared.Web.ClientSideActions.SetLanguage(Page, txtArbSymbol, Venus.Shared.Web.ClientSideActions.LANGUAGE_TYPE.ARABIC)
                Venus.Shared.Web.ClientSideActions.SetLanguage(Page, txtDecimalFarctArb, Venus.Shared.Web.ClientSideActions.LANGUAGE_TYPE.ARABIC)
            End If
            '================================== Add DateUpdateSchedules [Start]
            Dim IntrecordID As Integer
            If (txtCode.Text <> "") Then
                ClsCurrency.Find(" Code='" & txtCode.Text & "'")
                IntrecordID = ClsCurrency.ID
        ...
        Catch ex As Exception
            ...
            mErrorHandler.RecordExceptions_DataBase("", ex, Err.Number, ClsCurrency.RegUserID, Venus.Shared.ErrorsHandler.eRecordingType.System_DataBase)
    ...
    Protected Sub ImageButton_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs) Handles ImageButton_Save.Command, ImageButton_SaveN.Command, LinkButton_SaveN.Command, ImageButton_New.Command, ImageButton_Print.Command, ImageButton_Properties.Command, LinkButton_Properties.Command, ImageButton_Remarks.Command, LinkButton_Remarks.Command, ImageButton_Last.Command, ImageButton_Next.Command, ImageButton_Back.Command, ImageButton_First.Command, ImageButton_Delete.Command
        ...
        Dim ObjNavigationHandler As New Venus.Shared.Web.NavigationHandler(ClsCurrency.ConnectionString)
        Select Case e.CommandArgument
            ...
            Case "Delete"
                ...
                ClsCountries.Find(" CurrencyID= " & ClsCurrency.ID & " And IsNull(CancelDate,'')='' ")
                If ClsCountries.DataSet.Tables(0).Rows.Count > 0 Then
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, " Found countries use this currency")
                    Return
                Else
                    ClsCurrency.Delete("Code='" & txtCode.Text & "'")
                    AfterOperation()
                End If
            Case "Property"
                If ClsCurrency.Find("Code='" & txtCode.Text & "'") Then
                    Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmPropertyScreen.aspx?ID=" & ClsCurrency.ID & "&TableName=" & ClsCurrency.Table, 477, 313, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Property", False)
                End If
            Case "Remarks"
                If ClsCurrency.Find("Code='" & txtCode.Text & "'") Then
                    Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmRemarks.aspx?ID=" & ClsCurrency.ID & "&TableName=" & ClsCurrency.Table, 410, 210, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Remarks", False)
                End If
            ...
            Case "First"
                ClsCurrency.Find("Code='" & txtCode.Text & "'")
                ClsCurrency.FirstRecord()
                GetValues(ClsCurrency)
            Case "Previous"
                ClsCurrency.Find("Code='" & txtCode.Text & "'")
                If Not ClsCurrency.previousRecord() Then
                    ClsCurrency.Find("Code='" & txtCode.Text & "'")
            ...
            Case "Next"
                ClsCurrency.Find("Code='" & txtCode.Text & "'")
                If Not ClsCurrency.NextRecord() Then
                    ClsCurrency.Find("Code='" & txtCode.Text & "'")
...
#Region "Private Functions"
    Private Function SavePart() As Boolean
        Dim StrMode As String = Request.QueryString.Item("Mode")
        ClsCurrency = New ClsSys_Currencies(Page)

        Try
            ClsCurrency.Find("Code='" & txtCode.Text & "'")

            If ClsCurrency.ID > 0 Then
                If Not AssignValue(ClsCurrency) Then
                    Exit Function
                End If
                ClsCurrency.Update("Code='" & txtCode.Text & "'")
            ...
            clsMainOtherFields.CollectDataAndSave(value.Text, ClsCurrency.Table, ClsCurrency.ID)
            value.Text = ""
            Return True
        Catch ex As Exception
            Return False
        End Try
    End Function
    Private Function AssignValue(ByRef ClsCurrency As ClsSys_Currencies) As Boolean
        Try
            With ClsCurrency
                .Code = txtCode.Text
                .EngName = txtEngName.Text
                .ArbName = txtArbName.Text
                .EngSymbol = txtEngSymbol.Text
                .ArbSymbol = txtArbSymbol.Text

                .DecimalFraction = (Convert.ToInt16(txtDecimalFract.Text))
                .DecimalEngName = txtDecimalFarctEng.Text
                .DecimalArbName = txtDecimalFarctArb.Text
                .Amount = CType(txtAmount.Text, Decimal)
                .NoDecimalPlaces = IIf(txtNoOfDecimalPlaces.Text.Trim = "", 0, Convert.ToInt32(txtNoOfDecimalPlaces.Text))
        ...
        End Try
    End Function
    Private Function GetValues(ByVal ClsCurrency As ClsSys_Currencies) As Boolean
        ...
        Try
            ...
            With ClsCurrency
                ...
                txtEngSymbol.Text = .EngSymbol
                txtArbSymbol.Text = .ArbSymbol
                txtDecimalFract.Text = .DecimalFraction
                txtDecimalFarctEng.Text = .DecimalEngName
                txtDecimalFarctArb.Text = .DecimalArbName
                txtAmount.Text = .Amount
                txtNoOfDecimalPlaces.Text = .NoDecimalPlaces
    ...
    Private Function CheckCode() As Boolean
        ...
        Try
            ClsCurrency.Find("Code='" & txtCode.Text & "'")
            IntId = ClsCurrency.ID
            txtEngName.Focus()
            If ClsCurrency.ID > 0 Then
                GetValues(ClsCurrency)
                StrMode = "E"
            Else
                If ClsCurrency.CheckRecordExistance(" Code='" & txtCode.Text & "'") Then
                    txtCode.Text = ""
                    txtCode.Focus()
                End If
            ...
            End If
            SetToolBarDefaults()
            SetToolBarPermission(Me, ClsCurrency.ConnectionString, ClsCurrency.DataBaseUserRelatedID, ClsCurrency.GroupID, StrMode)
            SetToolBarRecordPermission(Me, ClsCurrency.ConnectionString, ClsCurrency.DataBaseUserRelatedID, ClsCurrency.GroupID, ClsCurrency.Table, IntId)
    ...
    Private Function Clear() As Boolean
        ...
        txtEngSymbol.Text = String.Empty
        txtArbSymbol.Text = String.Empty
        txtDecimalFract.Text = 0

        txtDecimalFarctEng.Text = String.Empty
        txtDecimalFarctArb.Text = String.Empty
        txtAmount.Text = 0
        txtNoOfDecimalPlaces.Text = 1
    ...
    Private Sub LoadDataUpdateSchedules(ByVal formName As String)
        ...
        ClsCurrency = New ClsSys_Currencies(Me.Page)
        ClsCurrency.Find(" code = '" & txtCode.Text & "'")
        Dim recordID As Integer = ClsCurrency.ID
        If (recordID > 0) Then
    ...
    Private Function CreateOtherFields(ByVal IntRecordID As Integer)
        ...
        If clsMainOtherFields.CheckOtherFieldsButton(Me.Page, ClsCurrency.Table) = True Then
            Dim StrTablename As String
            ClsCurrency = New ClsSys_Currencies(Me)
            StrTablename = ClsCurrency.Table
