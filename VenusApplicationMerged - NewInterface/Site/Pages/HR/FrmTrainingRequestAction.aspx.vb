...
#Region "Protected Sub"
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            'lll
            Dim str1 As String
            str1 = "select " & EmpName & " as EmployeeName,( case when " & ActionName & " is not null then " & ActionName & " else 'Pending ...' end) As Action ,convert(varchar, ActionDate,103) as ActionDate,ActionRemarks  from SS_TrainingRequest join SS_RequestActions on SS_TrainingRequest.ID=SS_RequestActions.RequestSerial and SS_TrainingRequest.EmployeeID=SS_RequestActions.EmployeeID join hrs_Employees on hrs_Employees.ID= SS_RequestActions.SS_EmployeeID left join SS_UserActions on SS_RequestActions.ActionID=SS_UserActions.ID where RequestSerial=" & RequestSerial & " and ( SS_RequestActions.IsHidden is null or SS_RequestActions.IsHidden=0 ) and FormCode='SS_00195' "
            ...
            strselect = "select SS_TrainingRequest.ID as RequestSerial,SS_TrainingRequest.EmployeeID,hrs_employees.Code as EmpCode," & EmpName & " as EmployeeName ,Convert(date,RequestDate) as RequestDate,SS_TrainingRequest.Remarks, " & Position & " As Position , " & Department & " As Department FROM            dbo.SS_TrainingRequest INNER JOIN dbo.hrs_Employees ON dbo.SS_TrainingRequest.EmployeeID = dbo.hrs_Employees.ID INNER JOIN dbo.hrs_Contracts ON dbo.hrs_Employees.ID = dbo.hrs_Contracts.EmployeeID and( hrs_Contracts.EndDate>=getdate() or hrs_Contracts.EndDate is null)  INNER JOIN dbo.hrs_Positions ON dbo.hrs_Contracts.PositionID = dbo.hrs_Positions.ID INNER JOIN dbo.sys_Departments ON dbo.hrs_Employees.DepartmentID = dbo.sys_Departments.ID   where  SS_TrainingRequest.ID=" & RequestSerial & ""
            ...
            txtRequestDate.Text = CDate(DS2.Tables(0).Rows(0)("RequestDate").ToString()).ToShortDateString()
                ...
                requesttypestr = "Select " & SelectRequestType & " from SS_RequestTypes where RequestCode='SS_00195'"
    ...
    Protected Sub btnSave_Click(sender As Object, e As Infragistics.WebUI.WebDataInput.ButtonEventArgs) Handles btnSave.Click
        ...
        If ddlAction.SelectedValue > 0 Then
            ...
            If ddlAction.SelectedValue = 2 Then   'رفض
                ...
                UpdateCommand = "update SS_RequestActions set  seen=1 , ActionID=" & ddlAction.SelectedValue & " ,ActionDate= GETDATE(),ActionRemarks='" + TxtRemarks.Text + "'  where ConfigID=" & ConfigID & " And FormCode='SS_00195' and RequestSerial=" & RequestSerial & " and SS_EmployeeID=" & ClsEmployees_SS.ID & " And ActionID Is Null "
                ...
                UpdateCommandRank = "UPDATE SS_TrainingRequest SET [RequestStautsTypeID] = 2 WHERE ID=" & RequestSerial & ""
                ...
                ClsEmployees.SendEmail("FrmTrainingRequestAction", Me.Page, 1, "SS_RequestActions", actionSerial)
            ...
            If ddlAction.SelectedValue = 1 Then
                ...
                UpdateCommandRank1 = "UPDATE SS_TrainingRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
                ...
                If dsconfig.Tables(0).Rows.Count > 0 Then
                    ...
                    Else
                        ...
                        NeededactionIdSql = "SELECT [ActionSerial] FROM [dbo].[SS_RequestActions]  where ConfigID=" & ConfigID & " And FormCode='SS_00195' and RequestSerial=" & RequestSerial & " and ActionID is null and SS_EmployeeID<>" & ClsEmployees.ID & ""
                    ...
                    If CBool(dsconfig.Tables(0).Rows(0)("IsFinal")) Then
                        ...
                        Dim SqlCommandRank As Data.SqlClient.SqlCommand
                        Dim UpdateCommandRank As String = ""
                        UpdateCommandRank = "UPDATE SS_TrainingRequest SET [RequestStautsTypeID] = 1 WHERE ID=" & RequestSerial & ""
                        SqlCommandRank = New SqlClient.SqlCommand
                        SqlCommandRank.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                        SqlCommandRank.CommandType = CommandType.Text
                        SqlCommandRank.CommandText = UpdateCommandRank
...
#End Region
    ...
    Private Function ProcessForwardForReview(ByVal RequestSerial As Integer, ByVal ConfigID As Integer, ByVal ReviewerCode As String, ByVal ActionRemarks As String, ByVal ConfirmedNoOfDays As String) As Boolean
        Try
            ...
            Dim UpdateStatus As String = "UPDATE SS_TrainingRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
