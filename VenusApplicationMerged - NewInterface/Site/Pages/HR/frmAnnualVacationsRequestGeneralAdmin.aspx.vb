...
Partial Class frmAnnualVacationsRequestGeneralAdmin
    Inherits MainPage
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                WebDateChooser1.Value = ClsEmployeesVacations.GetHigriDate(DateTime.Now.ToString("dd/MM/yyyy"))
...
#Region "Private Functions"
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            Dim str1 As String = "select ID,VacationType, RequestSerial ,EmployeeID ," & EmpName & " as EmployeeName,RequestDate ,NoOfDays, " & RequestName & " as RequestType, FormCode from SS_VAnnualVacationsFollowup where  EmployeeID= " & ClsEmployees.ID & "  and FormCode='SS_0034' Order By RequestDate desc"
    ...
    Public Function SaveAnnualVacationRequest() As Boolean
        ...
        Try
            ...
            strNoOfTimes = "select NoOfTimes from SS_RequestTypes where RequestCode='SS_0034'"
            ...
            strTimesPeriodPerMonth = "select TimesPeriodPerMonth from SS_RequestTypes where RequestCode='SS_0034'"
            ...
            If TimesPeriodPerMonth > 0 And NoOfTimes > 0 Then
                ...
                strCOuntNoOfimes = "Select count(id) from SS_VacationRequest where VacationType='SS_0034' and RequestDate<=GETDATE() and RequestDate>= DATEADD(month, -" & TimesPeriodPerMonth & ", getdate()) and EmployeeID='" & ClsEmployees.ID & "'"
            ...
            WebHandler.GetCookies(Page, "UserID", User)
            InsertCommand = "Insert Into SS_VacationRequest Values(''," & ClsEmployees.ID & "," & DdlVacationType.SelectedItem.Value & ",'SS_0034',convert(datetime, '" & ddlRequestDate.Value & "',103),convert(datetime, '" & WebDateChooser1.Value & "',103) ,convert(datetime, '" & WebDateChooser2.Value & "',103)," & lbTotalVal.Text & " , " & lbRemainVal.Text & " ,'" & txtContactNo.Text & "', '" & AlternativeEmpID & "','" & TxtRemarks.Text & "'," & User & " ,convert(datetime,  '" & ClsEmployeesVacations.GetHigriDate(Date.Now) & "' ,103),'" & txtRequesterUser.Text & "',3)"
            ...
            PrintRequestSerial = GetInertedID()
            SqlCommand = New SqlClient.SqlCommand
            SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
            SqlCommand.CommandType = CommandType.Text
    ...
    Public Function SaveFirstLeveNotification() As Boolean
        Try
            ...
            strselect = "select * from SS_Configuration where FormCode='SS_0034' and Rank=1"
            ...
            strMaxRequestSerial = "select isnull(Max(ID),1) from SS_VacationRequest where EmployeeID=" & ClsEmployees.ID & " and VacationType='SS_0034'"
            ...
            For Each Row As Data.DataRow In DsFirstLevel.Tables(0).Rows
                ...
                If Row("UserTypeID") = 1 Then
                    ...
                    If Not String.IsNullOrEmpty(DirectManagerID) AndAlso DirectManagerID <> "0" AndAlso DirectManagerID <> "null" Then
                        ...
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID) values(" & MaxSerial & " , " & DirectManagerID & ",'SS_0034'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                ...
                If Row("UserTypeID") = 2 Then
                    ...
                    If DsPositionEmployees.Tables(0).Rows.Count > 0 Then
                        For Each RW In DsPositionEmployees.Tables(0).Rows
                            ...
                            Dim strinsert As String
                            strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID) values(" & MaxSerial & " , " & RW("EmployeeID") & ",'SS_0034'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                            SqlCommand = New SqlClient.SqlCommand
                            SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                            SqlCommand.CommandType = CommandType.Text
                            SqlCommand.CommandText = strinsert
                ...
                If Row("UserTypeID") = 3 Then
                    ...
                    Dim strinsert As String
                    strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID) values(" & MaxSerial & " , " & EmpID & ",'SS_0034'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                    SqlCommand = New SqlClient.SqlCommand
                    SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                    SqlCommand.CommandType = CommandType.Text
                    SqlCommand.CommandText = strinsert
            ...
            If DirectManagerSkipped AndAlso Not Processed Then
                ...
                strSecondLevel = "select * from SS_Configuration where FormCode='SS_0034' and Rank=2"
...
#End Region
    ...
    Public Function CheckOfficialvacationbetweenvacationosrequests() As Boolean
        ...
        Dim strSelect As String = "SELECT MAX(EndDate) FROM SS_VacationRequest " &
                              ...
                              "WHERE FormCode='SS_0034' AND (ActionID=4 OR ActionID=2))"
