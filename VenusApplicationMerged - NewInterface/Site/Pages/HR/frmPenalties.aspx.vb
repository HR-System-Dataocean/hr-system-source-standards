...
#Region "Public Decleration"
    Dim clshrsPenalties As Clshrs_Penalties
...
#Region "Protected Sub"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        clshrsPenalties = New Clshrs_Penalties(Me)
        Dim ClsObjects As New Clssys_Objects(Page)
        Dim ClsSearchs As New Clssys_Searchs(Page)
        Dim SearchID As Integer = 0
        Try
            Dim clsSearchsColumns As New Clssys_SearchsColumns(Page)
            If ClsObjects.Find(" Code='" & clshrsPenalties.Table.Trim & "'") Then
            ...
            If Not IsPostBack Then
                Page.Session.Add("ConnectionString", clshrsPenalties.ConnectionString)
                clshrsPenalties.AddOnChangeEventToControls("Penalties", Page, UltraWebTab1)
                Dim clstransactiontype As New Clshrs_TransactionsTypes(Me)
                clstransactiontype.GetDropDownList(ddlTransactionType, True, "Sign= -1 and InputIsNumeric = 1 and IsPaid = 1 and isnull(Formula,'') = '' and IsDistributable = 0")
            ...
            If (txtCode.Text <> "") Then
                clshrsPenalties.Find(" Code='" & txtCode.Text & "'")
                IntrecordID = clshrsPenalties.ID
        ...
        Catch ex As Exception
            ...
            mErrorHandler.RecordExceptions_DataBase("", ex, Err.Number, clshrsPenalties.RegUserID, Venus.Shared.ErrorsHandler.eRecordingType.System_DataBase)
    ...
    Protected Sub ImageButton_Command(sender As Object, e As System.Web.UI.WebControls.CommandEventArgs) Handles ImageButton_Save.Command, ImageButton_SaveN.Command, LinkButton_SaveN.Command, ImageButton_New.Command, ImageButton_Print.Command, ImageButton_Properties.Command, LinkButton_Properties.Command, ImageButton_Remarks.Command, LinkButton_Remarks.Command, ImageButton_Last.Command, ImageButton_Next.Command, ImageButton_Back.Command, ImageButton_First.Command, ImageButton_Delete.Command
        ...
        Dim ObjNavigationHandler As New Venus.Shared.Web.NavigationHandler(clshrsPenalties.ConnectionString)
        Select Case e.CommandArgument
            Case "SaveNew"
                ...
                If Not AssignValues() Then
                    Exit Sub
                End If
                If clshrsPenalties.ID > 0 Then
                    clshrsPenalties.Update("code='" & txtCode.Text & "'")
                Else
                    clshrsPenalties.Save()
                End If
                clshrsPenalties.Find("Code='" & txtCode.Text & "'")
                clsMainOtherFields = New clsSys_MainOtherFields(Me.Page)
                clsMainOtherFields.CollectDataAndSave(value.Text, clshrsPenalties.Table, clshrsPenalties.ID)
                value.Text = ""
                AfterOperation()
            Case "Save"
                If txtCode.Text = "" Then
            ...
            Case "New"
                AfterOperation()
            Case "Delete"
                clshrsPenalties.Delete("Code='" & txtCode.Text & "'")
                AfterOperation()
            Case "Property"
                clshrsPenalties.Find("Code='" & txtCode.Text & "'")
                Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmPropertyScreen.aspx?ID=" & clshrsPenalties.ID & "&TableName=" & clshrsPenalties.Table, 477, 313, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Property", False)
            Case "Remarks"
                clshrsPenalties.Find("Code='" & txtCode.Text & "'")
                Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmRemarks.aspx?ID=" & clshrsPenalties.ID & "&TableName=" & clshrsPenalties.Table, 410, 210, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Remarks", False)
            Case "Print"
                Venus.Shared.Web.ClientSideActions.PrintWindow(Me)
            Case "Other Fields"
                Dim clsObjOtherFields As New ClsSys_OtherFields(Page)
                Dim clsSysObjects As New Clssys_Objects(Page)
                Dim tablename As String = clshrsPenalties.Table
                clshrsPenalties.Find(" code = '" & txtCode.Text & "'")
                Dim recordID As Integer = clshrsPenalties.ID
                ...
                With clsObjOtherFields
                    If .Find(" sys_OtherFields.ObjectID = " & clsSysObjects.ID) = True Then
                        ...
                        Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmOtherFieldsDynamic.aspx?tableName=" & tablename & "&ObjectId=" & .ObjectID & "&RecordId=" & clshrsPenalties.ID, 602, 306, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "OtherFields", False)
                    End If
                End With
            Case "Exit"
                clshrsPenalties.Find(" Code= '" & txtCode.Text & "'")
                If clshrsPenalties.ID > 0 Then
                    Dim Ds As Data.DataSet = clshrsPenalties.DataSet
                    If Not AssignValues() Then
                        Exit Sub
                    End If
                    If clshrsPenalties.CheckDiff(clshrsPenalties, Ds, "") Then
                ...
                End If
            Case "First"
                clshrsPenalties.FirstRecord()
                GetValues()
            Case "Previous"
                clshrsPenalties.Find("Code='" & txtCode.Text & "'")
                If Not clshrsPenalties.previousRecord() Then
                    clshrsPenalties.Find("Code='" & txtCode.Text & "'")
            ...
            Case "Next"
                clshrsPenalties.Find("Code='" & txtCode.Text & "'")
                If Not clshrsPenalties.NextRecord() Then
                    clshrsPenalties.Find("Code='" & txtCode.Text & "'")
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, " This is the last page /هذه أخر صفحة"))
                End If
                GetValues()
            Case "Last"
                clshrsPenalties.LastRecord()
                GetValues()
        End Select
    End Sub
...
#Region "Private Functions"
    ...
    Private Function GetValues() As Boolean
        ...
        Try
            SetToolBarDefaults()
            txtCode.Text = clshrsPenalties.Code
            txtEngName.Text = clshrsPenalties.EngName
            txtArbName.Text = clshrsPenalties.ArbName
            ddlTransactionType.SelectedValue = clshrsPenalties.TransactionTypeID

            If Not clshrsPenalties.RegUserID = Nothing Then
                ClsUser.Find("ID=" & clshrsPenalties.RegUserID)
            End If
            ...
            End If
            If Convert.ToDateTime(clshrsPenalties.RegDate).Date = Nothing Then
                lblRegDateValue.Text = ""
            Else
                lblRegDateValue.Text = Convert.ToDateTime(clshrsPenalties.RegDate).Date
            End If
            If clshrsPenalties.CancelDate = Nothing Then
                lblCancelDateValue.Text = ""
            Else
                lblCancelDateValue.Text = Convert.ToDateTime(clshrsPenalties.CancelDate).Date
            End If
            If Not clshrsPenalties.CancelDate = Nothing Then
            ...
            Else
                StrMode = "N"
            End If
            SetToolBarPermission(Me, clshrsPenalties.ConnectionString, clshrsPenalties.DataBaseUserRelatedID, clshrsPenalties.GroupID, StrMode)
            SetToolBarRecordPermission(Me, clshrsPenalties.ConnectionString, clshrsPenalties.DataBaseUserRelatedID, clshrsPenalties.GroupID, clshrsPenalties.Table, clshrsPenalties.ID)
            If Not clshrsPenalties.CancelDate = Nothing Then
                ImageButton_Delete.Enabled = False
            End If
            If Page.IsPostBack Then
                CreateOtherFields(clshrsPenalties.ID)
            End If
            Return True
        Catch ex As Exception
        End Try
    ...
    Private Function CheckCode() As Boolean
        ...
        Try
            clshrsPenalties.Find("Code='" & txtCode.Text & "'")
            IntId = clshrsPenalties.ID
            txtEngName.Focus()
            If clshrsPenalties.ID > 0 Then
                GetValues()
                StrMode = "E"
            Else
                If clshrsPenalties.CheckRecordExistance(" Code='" & txtCode.Text & "'") Then
                    txtCode.Text = ""
                    txtCode.Focus()
                End If
                Clear()
            ...
            SetToolBarDefaults()
            SetToolBarPermission(Me, clshrsPenalties.ConnectionString, clshrsPenalties.DataBaseUserRelatedID, clshrsPenalties.GroupID, StrMode)
            SetToolBarRecordPermission(Me, clshrsPenalties.ConnectionString, clshrsPenalties.DataBaseUserRelatedID, clshrsPenalties.GroupID, clshrsPenalties.Table, IntId)
    ...
    Private Sub LoadDataUpdateSchedules(ByVal formName As String)
        ...
        clshrsPenalties = New Clshrs_Penalties(Page)
        clshrsPenalties.Find(" code = '" & txtCode.Text & "'")
        Dim recordID As Integer = clshrsPenalties.ID
        If (recordID > 0) Then
    ...
    Private Function CreateOtherFields(ByVal IntRecordID As Integer)
        ...
        clsMainOtherFields = New clsSys_MainOtherFields(Me.Page)
        If clsMainOtherFields.CheckOtherFieldsButton(Me.Page, clshrsPenalties.Table) = True Then
            Dim StrTablename As String
            clshrsPenalties = New Clshrs_Penalties(Me)
            StrTablename = clshrsPenalties.Table
