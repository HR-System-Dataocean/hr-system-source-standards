...
Partial Class frmOvertimeRequest
    Inherits MainPage
        ...
        Select Case e.CommandArgument
            ...
            Case "SaveNew"
                If txtHoursNo.Text = "" Then
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, " Please Enter Hours count /برجاء إدخال عددالساعات"))
                    Exit Sub
                End If
                ...
                If Convert.ToInt16(txtHoursNo.Text) > 12 Then
                    ...
                    Exit Sub
                End If
                If Convert.ToInt16(txtMinutsNo.Text) > 60 Then
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, " The count of minutes cannot exceed 60 minutes. /لا يمكن زيادة عدد الدقائق عن 60 دقيقة"))
                    Exit Sub
                End If
                SavePart()
...
#Region "Private Functions"
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            Dim str1 As String = "select ID,VacationType, RequestSerial ,EmployeeID ," & EmpName & " as EmployeeName,RequestDate , " & RequestName & " as RequestType, FormCode from SS_VFollowup where  EmployeeID= " & ClsEmployees.ID & "  and FormCode='SS_001919' Order By RequestDate desc"
    ...
    Public Function SaveExecuseRequest() As Boolean
        ...
        Try
            ...
            If ClsEmployees.JoinDate > CDate(OverTimeDate.Text) Then
                Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "Sorry ...Overtime date cannot be before the employee's join date./عفواً... لا يمكن إتمام الطلب. تاريخ الوقت الإضافي لا يمكن أن يكون سابقاً لتاريخ الالتحاق بالعمل. "))
                Exit Function
            End If

            '''''''''''''''''''''''''''''عدد المرات
            Dim strNoOfTimes As String
            strNoOfTimes = "select NoOfTimes from SS_RequestTypes where RequestCode='SS_001919'"
            ...
            strTimesPeriodPerMonth = "select TimesPeriodPerMonth from SS_RequestTypes where RequestCode='SS_001919'"
            ...
            If TimesPeriodPerMonth > 0 And NoOfTimes > 0 Then
                ...
                strCOuntNoOfimes = "Select count(id) from SS_OvertimeRequest where RequestDate <= GETDATE() and RequestDate>= DATEADD(month, -" & TimesPeriodPerMonth & ", getdate()) and EmployeeID='" & ClsEmployees.ID & "'"
            ...
            Dim WebHandler As New Venus.Shared.Web.WebHandler
            WebHandler.GetCookies(Page, "UserID", User)
            If String.IsNullOrEmpty(txtMinutsNo.Text) Then
                txtMinutsNo.Text = "0"
            End If
            InsertCommand = "Insert Into SS_OvertimeRequest Values(''," & ClsEmployees.ID & ",convert(datetime, '" & ddlRequestDate.Value & "',103),'" & TxtRemarks.Text & "','" & txtRequesterUser.Text & "',3," & txtHoursNo.Text & ",convert(datetime, '" & OverTimeDate.Value & "',103)," & ddlOverTimeType.SelectedValue & "," & txtMinutsNo.Text & ")"
            ...
            Dim UpdateCommand = "Update  SS_OvertimeRequest  set Code ='" & GenerateRequestCode() & "' where ID=" & GetInertedID() & " "
    ...
    Public Function SaveFirstLeveNotification() As Boolean
        Try
            ...
            strselect = "select * from SS_Configuration where FormCode='SS_001919'and Rank=1"
            ...
            If DsFirstLevel.Tables(0).Rows.Count > 0 Then
                For Each Row As Data.DataRow In DsFirstLevel.Tables(0).Rows
                    ...
                    If Row("UserTypeID") = 1 Then
                        ...
                        strMaxRequestSerial = "select isnull(Max (ID),1) from SS_OvertimeRequest where EmployeeID=" & ClsEmployees.ID & " "
                        ...
                        SqlCommand.Connection.Close()
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & DirectManagerID & ",'SS_001919'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                    ...
                    If Row("UserTypeID") = 2 Then
                        ...
                        If DsPositionEmployees.Tables(0).Rows.Count > 0 Then
                            ...
                            For Each RW In DsPositionEmployees.Tables(0).Rows
                                ...
                                End If
                                strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & RW("EmployeeID") & ",'SS_001919'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                                SqlCommand = New SqlClient.SqlCommand
                                SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                                SqlCommand.CommandType = CommandType.Text
                                SqlCommand.CommandText = strinsert
                    ...
                    If Row("UserTypeID") = 3 Then
                        ...
                        End If
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & Row("EmployeeID") & ",'SS_001919'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                        SqlCommand.CommandText = strinsert
                    ...
                    End If
                Next
                ClsEmployees.SendEmail("SSRequests", Me.Page, 1, "SS_OvertimeRequest", PrintRequestSerial)
            End If
