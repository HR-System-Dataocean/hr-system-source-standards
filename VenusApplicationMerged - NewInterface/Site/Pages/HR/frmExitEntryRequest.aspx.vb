...
#Region "Private Functions"
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            Dim str1 As String = "select ID,VacationType, RequestSerial ,EmployeeID ," & EmpName & " as EmployeeName,RequestDate , " & RequestName & " as RequestType, FormCode from SS_VFollowup where  EmployeeID= " & ClsEmployees.ID & "  and FormCode='SS_00191' Order By RequestDate desc"
    ...
    Private Function SavePart() As Boolean
        ...
        If txtExitDate.Text = "" Then
            ...
            Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "Please Select Exit Date / عفوا لابد من تحديد تاريخ الخروج "))
            txtExitDate.Focus()
            Exit Function
        ...
        If txtEntryDate.Text = "" Then
            ...
            Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "Please Select Entry Date / عفوا لابد من تحديد تاريخ العودة "))
            txtEntryDate.Focus()
            Exit Function
    ...
    Public Function SaveRequest() As Boolean
        ...
        Try
            ...
            strNoOfTimes = "select NoOfTimes from SS_RequestTypes where RequestCode='SS_00191'"
            ...
            strTimesPeriodPerMonth = "select TimesPeriodPerMonth from SS_RequestTypes where RequestCode='SS_00191'"
            ...
            If TimesPeriodPerMonth > 0 And NoOfTimes > 0 Then
                ...
                strCOuntNoOfimes = "Select count(id) from SS_ExitEntryRequest where RequestDate <= GETDATE() and RequestDate>= DATEADD(month, -" & TimesPeriodPerMonth & ", getdate()) and EmployeeID='" & ClsEmployees.ID & "'"
            ...
            Dim WebHandler As New Venus.Shared.Web.WebHandler
            WebHandler.GetCookies(Page, "UserID", User)
            InsertCommand = "Insert Into SS_ExitEntryRequest (Code,EmployeeID,RequestDate,ExitDate,EntryDate,Remarks,RequesterUser,RequestStautsTypeID)Values(''," & ClsEmployees.ID & ",convert(datetime, '" & ddlRequestDate.Value & "',103),convert(datetime, '" & txtExitDate.Value & "',103),convert(datetime, '" & txtEntryDate.Value & "',103),'" & TxtRemarks.Text & "','" & txtRequesterUser.Text & "',3)"
            ...
            Dim UpdateCommand = "Update  SS_ExitEntryRequest  set Code ='" & GenerateRequestCode() & "' where ID=" & GetInertedID() & " "
    ...
    Public Function SaveFirstLeveNotification() As Boolean
        Try
            ...
            strselect = "select * from SS_Configuration where FormCode='SS_00191'and Rank=1"
            ...
            If DsFirstLevel.Tables(0).Rows.Count > 0 Then
                For Each Row As Data.DataRow In DsFirstLevel.Tables(0).Rows
                    ...
                    If Row("UserTypeID") = 1 Then
                        ...
                        strMaxRequestSerial = "select isnull(Max (ID),1) from SS_ExitEntryRequest where EmployeeID=" & ClsEmployees.ID & " "
                        ...
                        SqlCommand.Connection.Close()
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & DirectManagerID & ",'SS_00191'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                    ...
                    If Row("UserTypeID") = 2 Then
                        ...
                        If DsPositionEmployees.Tables(0).Rows.Count > 0 Then
                            ...
                            For Each RW In DsPositionEmployees.Tables(0).Rows
                                ...
                                End If
                                strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & RW("EmployeeID") & ",'SS_00191'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                                SqlCommand = New SqlClient.SqlCommand
                                SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                                SqlCommand.CommandType = CommandType.Text
                                SqlCommand.CommandText = strinsert
                    ...
                    If Row("UserTypeID") = 3 Then
                        ...
                        End If
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & Row("EmployeeID") & ",'SS_00191'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                        SqlCommand.CommandText = strinsert
                    ...
                    End If
                Next
                ClsEmployees.SendEmail("SSRequests", Me.Page, 1, "SS_ExitEntryRequest", PrintRequestSerial)
            End If
