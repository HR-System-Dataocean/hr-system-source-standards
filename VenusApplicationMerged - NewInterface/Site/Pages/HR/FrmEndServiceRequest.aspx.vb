...
#Region "Protected Sub"
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        Try
            ...
            If Not IsPostBack Then
                ...
                Dim ObjNavigationHandler1 As New Venus.Shared.Web.NavigationHandler(ClsTransactionsTypes.ConnectionString)
                Dim strselectAction As String = "SELECT  [Code] as ID,[ArbName],[EngName] FROM [dbo].[SS_ResignationReason]"
                Dim Item As Global.System.Web.UI.WebControls.ListItem
                Item = New Global.System.Web.UI.WebControls.ListItem
                Item.Value = 0
                Item.Text = ObjNavigationHandler1.SetLanguage(Page, "[Select Your Choise]/[ برجاء الاختيار ]")
                ddlReason.Items.Add(Item)

                Dim dsActions As DataSet = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(ClsTransactionsTypes.ConnectionString, CommandType.Text, strselectAction)
                For Each dr In dsActions.Tables(0).Rows
                    Item = New Global.System.Web.UI.WebControls.ListItem
                    Item.Value = dr("ID")
                    If ProfileCls.CurrentLanguage = "Ar" Then
                        Item.Text = dr("ArbName")
                    Else
                        Item.Text = dr("EngName")
                    End If
                    ddlReason.Items.Add(Item)
                Next
                ...
                Item.Value = 0
                Item.Text = ObjNavigationHandler1.SetLanguage(Page, "[Select Your Choise]/[ برجاء الاختيار ]")
                ddlRateDepartment.Items.Add(Item)
                ddlRateUs.Items.Add(Item)


                dsActions = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(ClsTransactionsTypes.ConnectionString, CommandType.Text, strselectRate)
                For Each dr In dsActions.Tables(0).Rows
                    Item = New Global.System.Web.UI.WebControls.ListItem
                    Item.Value = dr("ID")
                    If ProfileCls.CurrentLanguage = "Ar" Then
                ...
                ClsEndOfService.GetDropDownList(DdlEOSType, False, " ISNULL(ExcludedFromSSRequests,0)=0 ")
    ...
    Private Sub SetData()
        ...
        ClsEmployees.Find("Code='" & txtEmployee.Text & "'")
        Dim IntEmployeeId As Integer = ClsEmployees.ID
        ...
        Dim BaseDate As Date = Clshrs_EmployeesJoins.JoinDate
        Dim ClsContract As New Clshrs_Contracts(Page)
        ...
        Dim IntNotYear As Integer = 0
        Dim IntNotMonth As Integer = 0
        Dim IntNotDays As Integer = 0
        Dim IntUnPaid As Integer = 0
        ...
        Dim DblPayabilityBalance As Double = 0
        Dim DblPayabilities As Double = 0
        Dim DblFinalizedPayabilities As Double = 0
        ...
        Dim IntserviceYear As Integer = 0
        Dim IntserviceMonth As Integer = 0
        Dim IntserviceDays As Integer = 0
        Dim IntTotalServicesDays As Integer = 0
        ...
        DteEndOfServiceDate = ClsEmployees.SetHigriDate(WebDateChooser1.Value)
        ...
        ClsEndOfService.CalculateEndofService(IntEmployeeId, IntTotalDays, DdlEOSType.SelectedItem.Value, DblAmount, DteEndOfServiceDate)

        Dim lblDescworkingDays As Integer = IntTotalDays
        LblYear.Text = IntYear
        LblMonth.Text = IntMonth
        LblDay.Text = IntDays
        ...
        '    ClsSolver.EvaluateExpression(ClsTransactionTypes.Formula)
        ...
        'Dim lblTotalBenefit1 As Double = Math.Round((dblBenefits * IntPaid), Get_NoDecimalPlaces())
        'Dim lblRemainAmount As Double = Math.Round(dblTotalReamin + dblTotalDed, Get_NoDecimalPlaces())



        'TxtEOSAmount.Text = Math.Round(DblAmount, Get_NoDecimalPlaces())
    ...
    Private Function Load_ClsLayers(ByVal BasDate As Date) As Boolean
        Try
            ...
            ClsFisicalPeriod.GetFisicalperiodInfo(ClsEmployees.SetHigriDate(WebDateChooser1.Value), IntFisicalPeriod, FromDate, ToDate)
    ...
    Private Function Get_FromToDate(ByVal Basedate As Date) As Boolean
        Try
            ...
            ToDate = IIf(ClsFisicalPeriods.ToDate > ClsEmployees.SetHigriDate(WebDateChooser1.Value), ClsEmployees.SetHigriDate(WebDateChooser1.Value), ClsFisicalPeriods.ToDate)
...
#Region "Private Functions"
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            Dim str1 As String = "select ID,VacationType, RequestSerial ,EmployeeID ," & EmpName & " as EmployeeName,RequestDate , " & RequestName & " as RequestType, FormCode from SS_VFollowup where  EmployeeID= " & ClsEmployees.ID & "  and FormCode='SS_0015' Order By RequestDate desc"
    ...
    Private Function SavePart() As Boolean
        ...
        End Try
    End Function
    Public Function SaveEOSRequest() As Boolean
        ...
        Try
            ...
            strNoOfTimes = "select NoOfTimes from SS_RequestTypes where RequestCode='SS_0015'"
            ...
            strTimesPeriodPerMonth = "select TimesPeriodPerMonth from SS_RequestTypes where RequestCode='SS_0015'"
            ...
            If TimesPeriodPerMonth > 0 And NoOfTimes > 0 Then
                ...
                strCOuntNoOfimes = "Select count(id) from SS_EndOfServiceRequest where  RequestDate<=GETDATE() and RequestDate>= DATEADD(month, -" & TimesPeriodPerMonth & ", getdate()) and EmployeeID='" & ClsEmployees.ID & "'"
            ...
            Dim WebHandler As New Venus.Shared.Web.WebHandler
            WebHandler.GetCookies(Page, "UserID", User)
            InsertCommand = "Insert Into SS_EndOfServiceRequest Values(''," & ClsEmployees.ID & ",convert(datetime, '" & ddlRequestDate.Value & "',103),'" & DdlEOSType.SelectedValue & "',convert(datetime, '" & WebDateChooser1.Value & "',103) ,'" & TxtEOSReasons.Text & "','" & TxtRemarks.Text & "'," & LblYear.Text & "," & LblMonth.Text & "," & LblDay.Text & ",'SS_0015','" & txtRequesterUser.Text & "'," & ddlReason.SelectedValue & "," & ddlRateDepartment.SelectedValue & ",'" & txtOtherReason.Text & "'," & ddlRateUs.SelectedValue & "," & ddlBack.SelectedValue & ",3)"
            ...
            Dim UpdateCommand = "Update  SS_EndOfServiceRequest  set Code ='" & GenerateRequestCode() & "' where ID=" & GetInertedID() & " "
    ...
    Public Function SaveFirstLeveNotification() As Boolean
        Try
            ...
            strselect = "select * from SS_Configuration where FormCode='SS_0015'and Rank=1"
            ...
            If DsFirstLevel.Tables(0).Rows.Count > 0 Then
                For Each Row As Data.DataRow In DsFirstLevel.Tables(0).Rows
                    ...
                    If Row("UserTypeID") = 1 Then
                        ...
                        strMaxRequestSerial = "select isnull(Max (ID),1) from SS_EndOfServiceRequest where EmployeeID=" & ClsEmployees.ID & " "
                        ...
                        SqlCommand.Connection.Close()
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & DirectManagerID & ",'SS_0015'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                    ...
                    If Row("UserTypeID") = 2 Then
                        ...
                        If DsPositionEmployees.Tables(0).Rows.Count > 0 Then
                            ...
                            For Each RW In DsPositionEmployees.Tables(0).Rows
                                ...
                                End If
                                strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & RW("EmployeeID") & ",'SS_0015'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                                SqlCommand = New SqlClient.SqlCommand
                                SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                                SqlCommand.CommandType = CommandType.Text
                                SqlCommand.CommandText = strinsert
                    ...
                    If Row("UserTypeID") = 3 Then
                        ...
                        End If
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & Row("EmployeeID") & ",'SS_0015'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                        SqlCommand.CommandText = strinsert
                    ...
                    End If
                Next
                ClsEmployees.SendEmail("SSRequests", Me.Page, 1, "SS_EndOfServiceRequest", PrintRequestSerial)
            End If
    ...
    Public Function CheckActingEmployeeAssignment(EmpID As Integer) As Integer
        ...
        Dim actingEmpID As Object = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteScalar(
            CType(HttpContext.Current.Session("ConnectionString"), String), Data.CommandType.Text, sql)
