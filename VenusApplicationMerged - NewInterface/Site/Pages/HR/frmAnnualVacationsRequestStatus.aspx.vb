...
#Region "Private Functions"
    ...
    Private Sub FillEmployeeVacations(ByVal EmployeeID As Integer, Optional ByVal showFirstRecord As Boolean = False)
        ...
        If Type = 2 Then
            Try
                ...
                strselect = "select SS_VacationRequest.ID as RequestSerial,SS_VacationRequest.EmployeeID  ," & EmpName & " as AlternativeEmployeeName,hrs_VacationsTypes.EngName as VacationEngName , hrs_VacationsTypes.ArbName as VacationArabName,Convert(date,RequestDate) as RequestDate,Convert(date,StartDate) as StartDate,Convert(date,EndDate) as EndDate,TotalBalance,NoOfDays,ContactNo,AlternativeUser,SS_VacationRequest.Remarks from SS_VacationRequest join hrs_VacationsTypes on SS_VacationRequest.VacationTypeID=hrs_VacationsTypes.ID Left join hrs_Employees on SS_VacationRequest.AlternativeUser=hrs_Employees.ID where  SS_VacationRequest.ID=" & RequestSerial & ""
                ...
                TxtBalance.Text = DS2.Tables(0).Rows(0)("TotalBalance").ToString()
                TxtNoOfDays.Text = DS2.Tables(0).Rows(0)("NoOfDays").ToString()
                TxtContactNo.Text = DS2.Tables(0).Rows(0)("ContactNo").ToString()
                TxtAlternative.Text = DS2.Tables(0).Rows(0)("AlternativeEmployeeName").ToString()
    ...
    Public Function CheckVacationsOverlapping() As Boolean
        ...
        If Not Integer.TryParse(TxtNoOfDays.Text, confirmedDays) Then Return False
        ...
        Dim ClsVacationTypes As New Clshrs_VacationsTypes(Page)
        ClsVacationTypes.Find("ID=" & ClsEmployeesVacations.VacationTypeID)

        ' Get UserID from cookie

        ' Get employee class & contract
        Dim Cls_Contracts As New Clshrs_Contracts(Page)
        Dim ClsClasses = New Clshrs_EmployeeClasses(Page)
                ...
                If ClsVacationTypes.OverlapWithAnotherVac Then
                    If CInt(row("VacationTypeID")) = 1 AndAlso ClsClasses.AdvanceBalance Then
                        ...
                        If expireDate IsNot Nothing Then
                            ...
                            Dim insertQuery As String = " Update [dbo].[hrs_VacationsBalance] set CancelDate='" & DateTime.Now.Date.ToString("yyyy-MM-dd") &
                            "' where [EmployeeID]= @EmployeeID and [BalanceTypeID]=3 and [Src]='FrmAnnualVacationRequestAction' and DueDate=@DueDate and ISNULL(Posted,0)=0 "
    ...
    Private Sub PerformCancelRequest(FormCode As String, RequestSerial As String, ConfigID As Integer, ConnStr As String, includeCancelReason As Boolean)
        ...
        Dim UpdateCommandRank As String = "UPDATE SS_VacationRequest SET [RequestStautsTypeID] = 5 WHERE ID=" & RequestSerial & ""
    ...
    Protected Sub ImageButton_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs) Handles ImageButton_Save.Command
        ...
        Session("RequestSerial") = TxtRequestSerial.Text
        Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "../../Interfaces/frmReportsGridViewer.aspx?Language=false&Criteria=RequestSerial&preview=1&ReportCode=CF_101&sq0=''&v=" & Session("RequestSerial"), 700, 490, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "wWindO", False, True, False, False, False, False, False, False, False)
