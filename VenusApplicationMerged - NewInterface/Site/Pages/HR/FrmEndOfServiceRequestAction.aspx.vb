...
#Region "Protected Sub"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ClsEndOfService.GetDropDownList(DdlEOSType, False)
        DdlEOSType.Enabled = False
        txtEOSDate.Enabled = False
        TxtEOSReasons.Enabled = False
        ...
        If Not IsPostBack Then
            ...
            Dim ObjNavigationHandler1 As New Venus.Shared.Web.NavigationHandler(clsEmployees.ConnectionString)
            Dim strselectResignation As String = "SELECT  [Code] as ID,[ArbName],[EngName] FROM [dbo].[SS_ResignationReason]"
            Dim ItemResignation As Global.System.Web.UI.WebControls.ListItem
            ItemResignation = New Global.System.Web.UI.WebControls.ListItem
            ItemResignation.Value = 0
            ItemResignation.Text = ObjNavigationHandler1.SetLanguage(Page, "[Select Your Choise]/[ برجاء الاختيار ]")
            ddlReason.Items.Add(ItemResignation)

            Dim dsActionsResignation As DataSet = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(clsEmployees.ConnectionString, CommandType.Text, strselectResignation)
            For Each dr In dsActionsResignation.Tables(0).Rows
                ItemResignation = New Global.System.Web.UI.WebControls.ListItem
                ItemResignation.Value = dr("ID")
                If ProfileCls.CurrentLanguage = "Ar" Then
                    ItemResignation.Text = dr("ArbName")
                Else
                    ItemResignation.Text = dr("EngName")
                End If
                ddlReason.Items.Add(ItemResignation)
            Next

            Dim strselectRate As String = "SELECT  [Code] as ID,[ArbName],[EngName] FROM [dbo].[SS_ExperienceRate]"
            ItemResignation = New Global.System.Web.UI.WebControls.ListItem
            ItemResignation.Value = 0
            ItemResignation.Text = ObjNavigationHandler1.SetLanguage(Page, "[Select Your Choise]/[ برجاء الاختيار ]")
            ddlRateDepartment.Items.Add(ItemResignation)
            Dim ItemRate As Global.System.Web.UI.WebControls.ListItem
            ItemRate = New Global.System.Web.UI.WebControls.ListItem
            ItemRate.Value = 0
            ItemRate.Text = ObjNavigationHandler1.SetLanguage(Page, "[Select Your Choise]/[ برجاء الاختيار ]")
            ddlRateUs.Items.Add(ItemRate)
            dsActionsResignation = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(clsEmployees.ConnectionString, CommandType.Text, strselectRate)
            For Each dr In dsActionsResignation.Tables(0).Rows
                ItemResignation = New Global.System.Web.UI.WebControls.ListItem
                ItemResignation.Value = dr("ID")
                If ProfileCls.CurrentLanguage = "Ar" Then
                ...
                ddlRateDepartment.Items.Add(ItemResignation)
                ItemRate = New Global.System.Web.UI.WebControls.ListItem
                ItemRate.Value = dr("ID")
                If ProfileCls.CurrentLanguage = "Ar" Then
                    ItemRate.Text = dr("ArbName")
                Else
                    ItemRate.Text = dr("EngName")
                End If
                ddlRateUs.Items.Add(ItemRate)
            Next
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            strselect = "select SS_EndOfServiceRequest.ID as RequestSerial,hrs_Employees.Code ,SS_EndOfServiceRequest.EmployeeID," & EmpName & " as EmployeeName ,Convert(date,RequestDate) as RequestDate,EOSDate,EOSTypeID,EOSReasons,EOSRemarks,ServiceYears,SerciveMonths,ServiceDays," & Position & " As Position, " & Department & " As Department,SS_EndOfServiceRequest.ResignationReasonCode, SS_EndOfServiceRequest.DepartmentRateCode,SS_EndOfServiceRequest.OtherResignationReason,SS_EndOfServiceRequest.UsRateCode,SS_EndOfServiceRequest.CanBack , CONVERT(varchar, hrs_Employees.JoinDate, 103) as JoinDate  FROM            dbo.SS_EndOfServiceRequest INNER JOIN dbo.hrs_Employees ON dbo.SS_EndOfServiceRequest.EmployeeID = dbo.hrs_Employees.ID INNER JOIN dbo.hrs_Contracts ON dbo.hrs_Employees.ID = dbo.hrs_Contracts.EmployeeID  INNER JOIN dbo.hrs_Positions ON dbo.hrs_Contracts.PositionID = dbo.hrs_Positions.ID INNER JOIN dbo.sys_Departments ON dbo.hrs_Employees.DepartmentID = dbo.sys_Departments.ID where  SS_EndOfServiceRequest.ID=" & RequestSerial & ""
            ...
            Else
                txtEmployee.Text = DS2.Tables(0).Rows(0)("Code").ToString()
            ...
            DdlEOSType.SelectedValue = DS2.Tables(0).Rows(0)("EOSTypeID")
            txtEOSDate.Text = DS2.Tables(0).Rows(0)("EOSDate")
            TxtEOSReasons.Text = DS2.Tables(0).Rows(0)("EOSReasons")
            TxtRemarks.Text = DS2.Tables(0).Rows(0)("EOSRemarks")
            LblYear.Text = DS2.Tables(0).Rows(0)("ServiceYears")
            LblMonth.Text = DS2.Tables(0).Rows(0)("SerciveMonths")
            LblDay.Text = DS2.Tables(0).Rows(0)("ServiceDays")

            ddlReason.SelectedValue = DS2.Tables(0).Rows(0)("ResignationReasonCode")
            ddlRateDepartment.SelectedValue = DS2.Tables(0).Rows(0)("DepartmentRateCode")
            txtOtherReason.Text = DS2.Tables(0).Rows(0)("OtherResignationReason")
            ddlRateUs.SelectedValue = DS2.Tables(0).Rows(0)("UsRateCode")
            If Convert.ToBoolean(DS2.Tables(0).Rows(0)("CanBack")) Then
                ddlBack.SelectedValue = 1
            Else
                ddlBack.SelectedValue = 2
            End If
                Dim joinDateValue As Object = DS2.Tables(0).Rows(0)("JoinDate")

                If Not IsDBNull(joinDateValue) Then
                    TxtJoinDate.Text = joinDateValue.ToString()
                Else
                    TxtJoinDate.Text = ""
                End If
        ...
        End Try
    End Sub
    'Public Sub CheckEmployeeValidcontract(EmployeeID As Integer)

    '    Dim RequestSerial As Integer = Request.QueryString.Item("RequestSerial")

    '    Dim CLsContracts As New Clshrs_Contracts(Me.Page)
    '    Dim contractid As Integer
    '    contractid = CLsContracts.ContractValidatoinId(EmployeeID, DateTime.Now)
        ...
        If ddlAction.SelectedValue > 0 Then
            ...
            If IsReviewer > 0 Then
                ...
                If ddlAction.SelectedValue = 1 Then  ' موافقة
                    If ProcessReviewerDecision(RequestSerial, ConfigID, 1, TxtActionRemarks.Text, "0") Then
                        Page.ClientScript.RegisterStartupScript(Me.GetType(), "", "CloseMe()", True)
                    End If
                ElseIf ddlAction.SelectedValue = 2 Then  ' رفض
                    If ProcessReviewerDecision(RequestSerial, ConfigID, 2, TxtActionRemarks.Text, "0") Then
                        Page.ClientScript.RegisterStartupScript(Me.GetType(), "", "CloseMe()", True)
                    End If
                Else
            ...
            If ddlAction.SelectedValue = 2 Then   'رفض
                ...
                UpdateCommand = "update SS_RequestActions set  seen=1 , ActionID=" & ddlAction.SelectedValue & " ,ActionDate= GETDATE(),ActionRemarks='" + TxtActionRemarks.Text + "'  where ConfigID=" & ConfigID & " And (FormCode='SS_0015' Or FormCode='SS_0019') and RequestSerial=" & RequestSerial & " and SS_EmployeeID=" & ClsEmployees_SS.ID & " And ActionID is null"
                ...
                UpdateCommandRank = "UPDATE SS_EndOfServiceRequest SET [RequestStautsTypeID] = 2 WHERE ID=" & RequestSerial & ""
                ...
                ClsEmployees.SendEmail("FrmEndOfServiceRequestAction", Me.Page, 1, "SS_RequestActions", actionSerial)
            ...
            If ddlAction.SelectedValue = 1 Then
                ...
                UpdateCommandRank1 = "UPDATE SS_EndOfServiceRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
                ...
                SqlCommand.Connection.Close()
                        Dim SqlCommandRank As Data.SqlClient.SqlCommand
                        Dim UpdateCommandRank As String = ""
                        UpdateCommandRank = "UPDATE SS_EndOfServiceRequest SET [RequestStautsTypeID] = 1 WHERE ID=" & RequestSerial & ""
                        SqlCommandRank = New SqlClient.SqlCommand
                        SqlCommandRank.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                        SqlCommandRank.CommandType = CommandType.Text
                        SqlCommandRank.CommandText = UpdateCommandRank
            ...
            If ddlAction.SelectedValue = 5 Then
                ...
                If ProcessForwardForReview(RequestSerial, ConfigID, txtReviewer.Text, TxtActionRemarks.Text, "0") Then
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "", "CloseMe()", True)
                End If
                Exit Sub
            End If
...
#End Region
    ...
    Private Function ProcessForwardForReview(ByVal RequestSerial As Integer, ByVal ConfigID As Integer, ByVal ReviewerCode As String, ByVal ActionRemarks As String, ByVal ConfirmedNoOfDays As String) As Boolean
        Try
            ...
            Dim UpdateStatus As String = "UPDATE SS_EndOfServiceRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
    ...
    Private Function ProcessReviewerDecision(ByVal RequestSerial As Integer, ByVal ConfigID As Integer, ByVal ActionID As Integer, ByVal ActionRemarks As String, ByVal ConfirmedNoOfDays As String) As Boolean
        Try
            ...
            ' 7- تحديث حالة الطلب لـ "تحت الاجراء"
            Dim SqlCommandStatus As Data.SqlClient.SqlCommand
            Dim UpdateStatus As String = "UPDATE SS_EndOfServiceRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
            SqlCommandStatus = New SqlClient.SqlCommand
            SqlCommandStatus.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
