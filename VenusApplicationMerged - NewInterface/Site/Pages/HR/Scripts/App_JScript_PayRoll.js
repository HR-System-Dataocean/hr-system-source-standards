...
function uwgFiscalYearsPeriods_AfterSelectChangeHandler(gridName, id) {
    ...
    txtCode = window.document.all.item("txtCode");
    txtIsHijri = window.document.all.item("txtIsHijri");
    ...
    if (txtIsHijri.value == "1") {
        FromDate = Row.getCell(11).getValue();
        ToDate = Row.getCell(12).getValue();
    } else {
...
function uwgFiscalYearsPeriods_AfterRowActivateHandler(gridName, rowId) {
    ...
    if (txtIsHijri.value == "1") {
        wdcStartDate = row.getCellFromKey("HFromDate").Element.outerText;
        wdcEndDate = row.getCellFromKey("HToDate").Element.outerText;
    } else {
    ...
    txtRowIndexH = window.document.all.item("txtRowIndex");
    txtRowIndexH.value = rowId.split("_")[2];

    txtRowIDH = window.document.all.item("txtRowID");
    txtRowIDH.value = row.getCellFromKey("ID").getValue();
...
function frmEmpItems_Open_Search_KeyDown(SearchID, ControlName) {
    ...
    if (e.keyCode == 120) {
        if (control.readOnly == false) {
            var winopen = window.open("frmSearchScreen.aspx?TargetControl=" + ControlName + "&SearchID=" + SearchID, "_Parent" + 1, "height=615,width=724,resizable=0,menubar=0,toolbar=0,location=0,directories=0,scrollbars=0,status=0,dependent=No");
            winopen.document.focus();
        }
    }
}
...
function call_backRecordInfo(res) {
    ...
    var reguserItem = window.document.all.item("lblRegUserValue");
    var regdateItem = window.document.all.item("lblRegDateValue");
    var canceldateItem = window.document.all.item("lblCancelDateValue");
