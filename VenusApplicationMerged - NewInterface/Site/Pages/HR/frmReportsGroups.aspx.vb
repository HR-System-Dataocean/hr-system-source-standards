...
Partial Class frmReportsGroups
    Inherits MainPage
#Region "Public Decleration"
    Private ClsReportGroup As ClsSys_ReportsGroups
...
#Region "Protected Sub"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ClsReportGroup = New ClsSys_ReportsGroups(Me.Page)
        Dim ClsModule As New Clssys_Modules(Page)
        Dim ClsObjects As New Clssys_Objects(Page)
        Dim ClsSearchs As New Clssys_Searchs(Page)
        Dim SearchID As Integer = 0
        Try
            Dim clsSearchsColumns As New Clssys_SearchsColumns(Page)
            If ClsObjects.Find(" Code='" & ClsReportGroup.Table.Trim & "'") Then
            ...
            If Not IsPostBack Then
                Page.Session.Add("ConnectionString", ClsReportGroup.ConnectionString)
                ClsReportGroup.AddOnChangeEventToControls("frmReportsGroups", Page, UltraWebTab1)
                '================================= Exit & Navigation Notification [ End ]
                ClsReportGroup.GetDropDownList(DdlParentGroup, True)
                ClsModule.GetDropDownList(DdlModule, False)
            ...
            If (txtCode.Text <> "") Then
                ClsReportGroup.Find(" Code='" & txtCode.Text & "'")
                IntrecordID = ClsReportGroup.ID
        ...
        Catch ex As Exception
            ...
            mErrorHandler.RecordExceptions_DataBase("", ex, Err.Number, ClsReportGroup.RegUserId, Venus.Shared.ErrorsHandler.eRecordingType.System_DataBase)
    ...
    Protected Sub ImageButton_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs) Handles ImageButton_Save.Command, ImageButton_SaveN.Command, LinkButton_SaveN.Command, ImageButton_New.Command, ImageButton_Print.Command, ImageButton_Properties.Command, LinkButton_Properties.Command, ImageButton_Remarks.Command, LinkButton_Remarks.Command, ImageButton_Last.Command, ImageButton_Next.Command, ImageButton_Back.Command, ImageButton_First.Command, ImageButton_Delete.Command
        ...
        Dim ObjNavigationHandler As New Venus.Shared.Web.NavigationHandler(ClsReportGroup.ConnectionString)
        Select Case e.CommandArgument
            ...
            Case "Delete"
                ClsReportGroup.Find("Code='" & txtCode.Text & "'")
                Dim tempRepGroup As New ClsSys_ReportsGroups(Page)
                tempRepGroup.Find(" ParentID=" & ClsReportGroup.ID & " And IsNull(CancelDate,'')='' ")
                If tempRepGroup.DataSet.Tables(0).Rows.Count > 0 Then
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "This Group has child groups / هذه المجموعة مستخدمة من المشاريخ الأخرى"))
                    Return
                Else
                    ClsReportGroup.Delete("Code='" & txtCode.Text & "'")
                    AfterOperation()
                End If
            Case "Property"
                If ClsReportGroup.Find("Code='" & txtCode.Text & "'") Then
                    Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmPropertyScreen.aspx?ID=" & ClsReportGroup.ID & "&TableName=" & ClsReportGroup.Table, 477, 313, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Property", False)
                End If
            Case "Remarks"
                If ClsReportGroup.Find("Code='" & txtCode.Text & "'") Then
                    Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmRemarks.aspx?ID=" & ClsReportGroup.ID & "&TableName=" & ClsReportGroup.Table, 410, 210, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Remarks", False)
                End If
            ...
            Case "First"
                ClsReportGroup.Find("Code='" & txtCode.Text & "'")
                ClsReportGroup.FirstRecord()
                GetValues(ClsReportGroup)
            Case "Previous"
                ClsReportGroup.Find("Code='" & txtCode.Text & "'")
                If Not ClsReportGroup.previousRecord() Then
                    ClsReportGroup.Find("Code='" & txtCode.Text & "'")
            ...
            Case "Next"
                ClsReportGroup.Find("Code='" & txtCode.Text & "'")
                If Not ClsReportGroup.NextRecord() Then
                    ClsReportGroup.Find("Code='" & txtCode.Text & "'")
...
#Region "Private Functions"
    Private Function GetChildGroups(ByVal GroupID As Integer, ByVal Connection As String, ByRef RetStr As String) As Boolean

        Dim strSelect As String = " Select ID from sys_ReportsGroups where ParentID = " & GroupID
        ...
        If (DS.Tables(0).Rows.Count > 0) Then
            For Each row As Data.DataRow In DS.Tables(0).Rows
                ...
                GetChildGroups(row("ID"), Connection, RetStr)
            Next
    ...
    Private Function SavePart() As Boolean
        Try
            ...
            Dim StrMode As String = Request.QueryString.Item("Mode")
            ClsReportGroup = New ClsSys_ReportsGroups(Page)

            ClsReportGroup.Find("Code='" & txtCode.Text & "'")
            If Not AssignValue(ClsReportGroup) Then
                Exit Function
            End If
            If ClsReportGroup.ID > 0 Then

                ClsReportGroup.Update("Code='" & txtCode.Text & "'")
            Else
            ...
            clsMainOtherFields.CollectDataAndSave(value.Text, ClsReportGroup.Table, ClsReportGroup.ID)
            value.Text = ""
        ...
        End Try
    End Function
    Private Function AssignValue(ByRef ClsReportGroup As ClsSys_ReportsGroups) As Boolean
        ...
        Try
            With ClsReportGroup
                ...
                .ParentID = IIf(DdlParentGroup.SelectedItem.Value = 0, Nothing, DdlParentGroup.SelectedItem.Value)
        ...
        End Try
    End Function
    Private Function GetValues(ByVal ClsReportGroup As ClsSys_ReportsGroups) As Boolean
        ...
        Try
            ...
            With ClsReportGroup
                ...
                Dim ClsMod As New Clssys_Modules(Page)
                ClsMod.GetDropDownList(DdlModule, False)

                Dim ObjNavigationHandler As New Venus.Shared.Web.NavigationHandler(.ConnectionString)



                ClsMod.Find(" ID= " & IIf(IsNothing(ClsReportGroup.ModuleID), 0, ClsReportGroup.ModuleID))
                If ClsMod.ID > 0 Then

                    item.Value = .ModuleID
                    item.Text = ObjNavigationHandler.SetLanguage(Page, ClsMod.EngName & "/" & ClsMod.ArbName)

                    If (item.Text.Trim = "") Then
                        item.Text = ObjNavigationHandler.SetLanguage(Page, ClsMod.ArbName & "/" & ClsMod.EngName)
                ...
                Dim str As String = ""
                GetChildGroups(.ID, .ConnectionString, str)
                str = str.Trim(",")
                If str.Trim <> String.Empty Then
                    .GetDropDownList(DdlParentGroup, True, " ID <> " & .ID & " And ID  not in ( " & str & " )")
                Else
                    .GetDropDownList(DdlParentGroup, True, " ID <> " & .ID)
                End If
                '------------------------------=============-----------------------------------------

                DdlParentGroup.SelectedValue = IIf(.ParentID Is Nothing, 0, .ParentID)


                If Not ClsReportGroup.RegUserID = Nothing Then
                    ClsUser.Find("ID=" & ClsReportGroup.RegUserID)
                End If
                ...
                SetToolBarRecordPermission(Me, ClsReportGroup.ConnectionString, ClsReportGroup.DataBaseUserRelatedID, ClsReportGroup.GroupID, ClsReportGroup.Table, IntId)
                If Not .CancelDate = Nothing Then
                    ImageButton_Delete.Enabled = False
                End If
                If Page.IsPostBack Then
                    CreateOtherFields(ClsReportGroup.ID)
                End If
            End With
            Return True
        Catch ex As Exception
    ...
    Private Function CheckCode() As Boolean
        ...
        Try
            ClsReportGroup.Find("Code='" & txtCode.Text & "'")
            IntId = ClsReportGroup.ID
            txtEngName.Focus()
            If ClsReportGroup.ID > 0 Then
                GetValues(ClsReportGroup)
                StrMode = "E"
            Else
                If ClsReportGroup.CheckRecordExistance(" Code='" & txtCode.Text & "'") Then
                    txtCode.Text = ""
                    txtCode.Focus()
                End If
            ...
            End If
            SetToolBarDefaults()
            SetToolBarPermission(Me, ClsReportGroup.ConnectionString, ClsReportGroup.DataBaseUserRelatedID, ClsReportGroup.GroupID, StrMode)
            SetToolBarRecordPermission(Me, ClsReportGroup.ConnectionString, ClsReportGroup.DataBaseUserRelatedID, ClsReportGroup.GroupID, ClsReportGroup.Table, IntId)
    ...
    Private Sub LoadDataUpdateSchedules(ByVal formName As String)
        ...
        ClsReportGroup = New ClsSys_ReportsGroups(Me.Page)
        ClsReportGroup.Find(" code = '" & txtCode.Text & "'")
        Dim recordID As Integer = ClsReportGroup.ID
        If (recordID > 0) Then
    ...
    Private Function CreateOtherFields(ByVal IntRecordID As Integer)
        ...
        clsMainOtherFields = New clsSys_MainOtherFields(Me.Page)
        If clsMainOtherFields.CheckOtherFieldsButton(Me.Page, ClsReportGroup.Table) = True Then
            Dim StrTablename As String
            ClsReportGroup = New ClsSys_ReportsGroups(Page)
            StrTablename = ClsReportGroup.Table
