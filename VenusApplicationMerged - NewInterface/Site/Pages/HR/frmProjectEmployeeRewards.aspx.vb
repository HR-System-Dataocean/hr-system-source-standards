...
Partial Class frmProjectEmployeeRewards
    Inherits MainPage
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Else
                ...
                Msg.Text = ""
            End If
            Dim ClsRewardy As New Clshrs_Rewards(Me)
            ClsRewardy.GetDropDownList(ddlRewardy, True)
        End If
    End Sub
#End Region
    ...
    Protected Sub LinkButton_Absent_Click(sender As Object, e As System.EventArgs) Handles LinkButton_Absent.Click
        ...
        If ddlRewardy.SelectedValue > 0 Then
            With clsProjectEmployeesPen
                ...
                Dim ClsRewardy As New Clshrs_Rewards(Me)
                ClsRewardy.Find("ID =" & ddlRewardy.SelectedValue)
                Dim hrstrans As New Clshrs_TransactionsTypes(Page)
                hrstrans.Find("ID = " & ClsRewardy.TransactionTypeID)

                Dim RewardyVal As Double = 0
                If Fixed.Checked Then
                    RewardyVal = txtAmount.Value
                Else
                    Dim SPH As Double = 0
                    Calculate_SalaryPerHour(SPH, Request.QueryString.Item("ID"), fperiodid, (ToDate.Subtract(FromDate).Days + 1))
                    RewardyVal = txtAmount.Value * SPH
                End If
                Dim strcommand As String = "set dateformat dmy; insert into hrs_EmployeeExtraItems values ((select Code from hrs_Employees where ID = " & Request.QueryString.Item("ID") & "),''," & hrstrans.Code & "," & RewardyVal & "," & fperiodid & ",1,'" & .TrnsDate.ToString("dd/MM/yyyy") & "',3,(select top 1 ID from hrs_ProjectEmployeeRewards where EmployeeID = " & Request.QueryString.Item("ID") & " order by ID desc),'','')"
