...
Partial Class frmIntervals
    Inherits MainPage
#Region "Public Decleration"
    Private ClsInterval As Clshrs_Intervals
...
#Region "Protected Sub"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ClsInterval = New Clshrs_Intervals(Me.Page)
        Dim ClsObjects As New Clssys_Objects(Page)
        Dim ClsSearchs As New Clssys_Searchs(Page)
        Dim SearchID As Integer = 0
        Try
            Dim clsSearchsColumns As New Clssys_SearchsColumns(Page)
            If ClsObjects.Find(" Code='" & ClsInterval.Table.Trim & "'") Then
            ...
            If Not IsPostBack Then
                Page.Session.Add("ConnectionString", ClsInterval.ConnectionString)
                ClsInterval.AddOnChangeEventToControls("frmIntervals", Page, UltraWebTab1)
            ...
            If (txtCode.Text <> "") Then
                ClsInterval.Find(" Code='" & txtCode.Text & "'")
                IntrecordID = ClsInterval.ID
        ...
        Catch ex As Exception
            ...
            mErrorHandler.RecordExceptions_DataBase("", ex, Err.Number, ClsInterval.RegUserID, Venus.Shared.ErrorsHandler.eRecordingType.System_DataBase)
    ...
    Protected Sub ImageButton_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs) Handles ImageButton_Save.Command, ImageButton_SaveN.Command, LinkButton_SaveN.Command, ImageButton_New.Command, ImageButton_Print.Command, ImageButton_Properties.Command, LinkButton_Properties.Command, ImageButton_Remarks.Command, LinkButton_Remarks.Command, ImageButton_Last.Command, ImageButton_Next.Command, ImageButton_Back.Command, ImageButton_First.Command, ImageButton_Delete.Command
        ...
        Dim ObjNavigationHandler As New Venus.Shared.Web.NavigationHandler(ClsInterval.ConnectionString)
        Select Case e.CommandArgument
            ...
            Case "New"
                AfterOperation()
            Case "Delete"
                ClsInterval.Delete("Code='" & txtCode.Text & "'")
                AfterOperation()
            Case "Property"
                If ClsInterval.Find("Code='" & txtCode.Text & "'") Then
                    Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmPropertyScreen.aspx?ID=" & ClsInterval.ID & "&TableName=" & ClsInterval.Table, 477, 313, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Property", False)
                End If
            Case "Remarks"
                If ClsInterval.Find("Code='" & txtCode.Text & "'") Then
                    Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmRemarks.aspx?ID=" & ClsInterval.ID & "&TableName=" & ClsInterval.Table, 410, 210, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Remarks", False)
                End If
            ...
            Case "First"
                ClsInterval.Find("Code='" & txtCode.Text & "'")
                ClsInterval.FirstRecord()
                GetValues(ClsInterval)
            Case "Previous"
                ClsInterval.Find("Code='" & txtCode.Text & "'")
                If Not ClsInterval.previousRecord() Then
                    ClsInterval.Find("Code='" & txtCode.Text & "'")
            ...
            Case "Next"
                ClsInterval.Find("Code='" & txtCode.Text & "'")
                If Not ClsInterval.NextRecord() Then
                    ClsInterval.Find("Code='" & txtCode.Text & "'")
...
#Region "Private Functions"
    Private Function SavePart() As Boolean
        Dim StrMode As String = Request.QueryString.Item("Mode")
        ClsInterval = New Clshrs_Intervals(Page)

        Try
            ClsInterval.Find("Code='" & txtCode.Text & "'")

            If ClsInterval.ID > 0 Then
                If Not AssignValues(ClsInterval) Then
                    Exit Function
                End If
                ClsInterval.Update("Code='" & txtCode.Text & "'")
            ...
            clsMainOtherFields.CollectDataAndSave(value.Text, ClsInterval.Table, ClsInterval.ID)
            value.Text = ""
            Return True
        Catch ex As Exception
            Return False
    ...
    Private Function AssignValues(ByRef ClsBasicFiles As Object) As Boolean
        Try
            ...
            Return True
        Catch ex As Exception
            mErrorHandler = New Venus.Shared.ErrorsHandler(ClsInterval.ConnectionString)
            Page.Session.Add("ErrorValue", ex)
            mErrorHandler.RecordExceptions_DataBase("", ex, Err.Number, ClsInterval.RegUserID, Venus.Shared.ErrorsHandler.eRecordingType.System_DataBase)
            Page.Response.Redirect("ErrorPage.aspx")
            Return False
        End Try
    End Function
    Private Function GetValues(ByVal ClsInterval As Clshrs_Intervals) As Boolean
        ...
        Try
            ...
            With ClsInterval
                ...
                If Not ClsInterval.RegUserID = Nothing Then
                    ClsUser.Find("ID=" & ClsInterval.RegUserID)
                End If
                ...
                SetToolBarRecordPermission(Me, ClsInterval.ConnectionString, ClsInterval.DataBaseUserRelatedID, ClsInterval.GroupID, ClsInterval.Table, IntId)
                If Not .CancelDate = Nothing Then
                    ImageButton_Delete.Enabled = False
                End If
                If Page.IsPostBack Then
                    CreateOtherFields(ClsInterval.ID)
                End If
            End With
            Return True
        Catch ex As Exception
    ...
    Private Function CheckCode() As Boolean
        ...
        Try
            ClsInterval.Find("Code='" & txtCode.Text & "'")
            IntId = ClsInterval.ID
            txtEngName.Focus()
            If ClsInterval.ID > 0 Then
                GetValues(ClsInterval)
                StrMode = "E"
            Else
                If ClsInterval.CheckRecordExistance(" Code='" & txtCode.Text & "'") Then
                    txtCode.Text = ""
                    txtCode.Focus()
                End If
            ...
            End If
            SetToolBarDefaults()
            SetToolBarPermission(Me, ClsInterval.ConnectionString, ClsInterval.DataBaseUserRelatedID, ClsInterval.GroupID, StrMode)
            SetToolBarRecordPermission(Me, ClsInterval.ConnectionString, ClsInterval.DataBaseUserRelatedID, ClsInterval.GroupID, ClsInterval.Table, IntId)
    ...
    Private Sub LoadDataUpdateSchedules(ByVal formName As String)
        ...
        ClsInterval = New Clshrs_Intervals(Me.Page)
        ClsInterval.Find(" code = '" & txtCode.Text & "'")
        Dim recordID As Integer = ClsInterval.ID
        If (recordID > 0) Then
    ...
    Private Function CreateOtherFields(ByVal IntRecordID As Integer)
        ...
        If clsMainOtherFields.CheckOtherFieldsButton(Me.Page, ClsInterval.Table) = True Then
            Dim StrTablename As String
            ClsInterval = New Clshrs_Intervals(Page)
            StrTablename = ClsInterval.Table
