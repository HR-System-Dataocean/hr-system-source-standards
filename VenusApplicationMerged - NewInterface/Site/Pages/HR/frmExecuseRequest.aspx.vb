...
#Region "Private Functions"
    Private Function SetNew() As Boolean
        ...
        Try
            ...
            DdlExecuseType.SelectedValue = 0
            DdlExecuseReason.SelectedValue = 0
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            Dim str1 As String = "select ID,VacationType, RequestSerial ,EmployeeID ," & EmpName & " as EmployeeName,RequestDate , " & RequestName & " as RequestType, FormCode from SS_VFollowup where  EmployeeID= " & ClsEmployees.ID & "  and FormCode='SS_0014' Order By RequestDate desc"
    ...
    Private Function SavePart() As Boolean
        ...
        If String.IsNullOrEmpty(TxtExecuseTime.Text) Then
            Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "The number of Excuse hours Can't be Empty /عفوا ... لا يمكن ترك حقل ساعات الاذن فارغا "))
            Return False
        End If
        Dim Sys_Companie As New Clssys_Companies(Page)
        Sys_Companie.Find("ID = " & Convert.ToInt32(HttpContext.Current.Session("CompanyID")))
        If Sys_Companie.ExecuseRequestHoursallowed > 0 Then
            Dim ExecuseTime As String = TxtExecuseTime.Text
            Dim hourAsInt As Integer = 0
            Dim minuts As Integer = 0
            If ExecuseTime.Contains(":") Then
                Dim time As TimeSpan = TimeSpan.Parse(ExecuseTime)
                hourAsInt = time.Hours
                minuts = time.Minutes
            Else
                hourAsInt = Integer.Parse(TxtExecuseTime.Text)
            End If
            If hourAsInt > Sys_Companie.ExecuseRequestHoursallowed Then
                Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "The number of authorized hours exceeded the number of allowed hours /عدد ساعات الاذن تخطي عدد الساعات المسوح "))
                Return False
            Else
                If hourAsInt = Sys_Companie.ExecuseRequestHoursallowed Then
                    If minuts > 0 Then
    ...
    Public Function SaveExecuseRequest() As Boolean
        ...
        Try
            ...
            strNoOfTimes = "select NoOfTimes from SS_RequestTypes where RequestCode='SS_0014'"
            ...
            strTimesPeriodPerMonth = "select TimesPeriodPerMonth from SS_RequestTypes where RequestCode='SS_0014'"
            ...
            If TimesPeriodPerMonth > 0 And NoOfTimes > 0 Then
                ...
                Dim firstDayOfCurrentMonth As Date = New Date(Today.Year, Today.Month, 1)
            ...
            End If
            ''''''''''''''''''''''''''''''''''''''''''''''
            If DdlExecuseType.SelectedValue = "0" Then
                Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "Sorry...Please select Excuse type /عفوا ... برجاء اختيار نوع الاذن       "))
                Exit Function
            End If
            If DdlExecuseReason.SelectedValue = "0" Then
                Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "Sorry...Please select Excuse reason /عفوا ... برجاء اختيار سبب الاذن       "))
                Exit Function
            End If

            If String.IsNullOrEmpty(TxtExecuseTime.Text) Then
                Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "Sorry...Please select Excuse time /عفوا ... برجاء اختيار مدة الاذن       "))
                Exit Function
            End If
            If DdlShift.SelectedValue = 0 Then
                Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "Sorry...Please select Excuse shift /عفوا ... برجاء اختيار الوردية        "))
                Exit Function
            End If
            ...
            If COuntNoOfimesPerDay > 0 Then
                Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "Sorry...you have an excuse request in this date /عفوا يوجد لديك طلب اذن مسجل في هذا التاريخ  "))
                Exit Function
            End If

            Dim strCOuntNoOfimesPerdayTrns As String
            strCOuntNoOfimesPerdayTrns = "Select count(id)  from hrs_EmployeesExcuses where excusedate = convert(datetime, '" & WebDateChooser1.Value & "',103)  and EmployeeID='" & ClsEmployees.ID & "' and CancelDate is null "
            Dim COuntNoOfimesPerDayTrns As Integer
            COuntNoOfimesPerDayTrns = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteScalar(CType(HttpContext.Current.Session("ConnectionString"), String), Data.CommandType.Text, strCOuntNoOfimesPerdayTrns)
            If COuntNoOfimesPerDayTrns > 0 Then
                Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, "Sorry...you have an excuse request in this date /عفوا يوجد لديك طلب اذن مسجل في هذا التاريخ  "))
                Exit Function
            End If
            ...
            Dim WebHandler As New Venus.Shared.Web.WebHandler
            WebHandler.GetCookies(Page, "UserID", User)
            InsertCommand = "Insert Into SS_ExecuseRequest Values(''," & ClsEmployees.ID & ",convert(datetime, '" & ddlRequestDate.Value & "',103),'" & DdlExecuseType.SelectedValue & "','" & DdlExecuseReason.SelectedValue & "',convert(datetime, '" & WebDateChooser1.Value & "',103) ,'" & TxtExecuseTime.Text & "', " & DdlShift.SelectedValue & " ,'" & TxtRemarks.Text & "','" & txtRequesterUser.Text & "',3)"
            ...
            Dim UpdateCommand = "Update  SS_ExecuseRequest  set Code ='" & GenerateRequestCode() & "' where ID=" & GetInertedID() & " "
    ...
    Public Function SaveFirstLeveNotification() As Boolean
        Try
            ...
            strselect = "select * from SS_Configuration where FormCode='SS_0014'and Rank=1"
            ...
            If DsFirstLevel.Tables(0).Rows.Count > 0 Then
                For Each Row As Data.DataRow In DsFirstLevel.Tables(0).Rows
                    ...
                    If Row("UserTypeID") = 1 Then
                        ...
                        strMaxRequestSerial = "select isnull(Max (ID),1) from SS_ExecuseRequest where EmployeeID=" & ClsEmployees.ID & " "
                        ...
                        SqlCommand.Connection.Close()
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & DirectManagerID & ",'SS_0014'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                    ...
                    If Row("UserTypeID") = 2 Then
                        ...
                        If DsPositionEmployees.Tables(0).Rows.Count > 0 Then
                            ...
                            For Each RW In DsPositionEmployees.Tables(0).Rows
                                ...
                                End If
                                strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & RW("EmployeeID") & ",'SS_0014'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                                SqlCommand = New SqlClient.SqlCommand
                                SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                                SqlCommand.CommandType = CommandType.Text
                                SqlCommand.CommandText = strinsert
                    ...
                    If Row("UserTypeID") = 3 Then
                        ...
                        strinsert = "Insert Into SS_RequestActions (RequestSerial,SS_EmployeeID,FormCode,EmployeeID,Seen,ConfigID)  values(" & MaxSerial & " , " & Row("EmployeeID") & ",'SS_0014'," & ClsEmployees.ID & ",0," & Row("ID") & ")"
                        SqlCommand = New SqlClient.SqlCommand
                        SqlCommand.Connection = New SqlClient.SqlConnection(ConnectionString)
                        SqlCommand.CommandType = CommandType.Text
                        SqlCommand.CommandText = strinsert
                    ...
                    End If
                Next
                ClsEmployees.SendEmail("SSRequests", Me.Page, 1, "SS_ExecuseRequest", PrintRequestSerial)
            End If
