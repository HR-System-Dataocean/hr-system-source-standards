...
Partial Class frmPreparedPayment
    Inherits MainPage
...
#Region "Protected Sub"
    ...
    Protected Sub Button_Click(sender As Object, e As System.EventArgs) Handles LinkButtonGLPost.Command, ImageButton_GLPost.Command, ImageButton_GLPostReview.Command, LinkButtonGLPostReview.Command
        ...
        Try
            ...
            Select Case DirectCast(e, System.Web.UI.WebControls.CommandEventArgs).CommandArgument
                ...
                Case "GLPost"
                    ...
                    For I As Integer = 0 To dsPatches.Tables(0).Rows.Count - 1
                        ...
                        Else
                            ...
                            ExecProfile = "Declare @Err	varchar(max)" &
                                         ...
                                         " Exec fcs_InsertProfilePayment " & dsPatches.Tables(0).Rows(I)(0)
                        End If
                    ...
                    Next
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, objNav.SetLanguage(Page, "Payment Done!/تم الصرف"))
            End Select
            GetData()
...
#Region "Private Function"
    ...
    Private Function GetData() As Boolean
        Try
            ...
            strCommand = " select et.ID,et.EmployeeID,et.PrepareType,e.Code as Code," & "dbo.fn_GetEmpName(e.Code," & ClsNavigationHandler.SetLanguage(Page, "0/1") & ") AS FullName,(select isnull(count(hrs_EmployeesTransactions.ID),0) from hrs_EmployeesTransactions where hrs_EmployeesTransactions.ID = et.ID and hrs_EmployeesTransactions.PaidDate is not null) as Paid  from hrs_EmployeesTransactions et inner join hrs_Employees e on e.ID = et.EmployeeID INNER JOIN sys_FiscalYearsPeriodsModules as m ON m.FiscalYearPeriodID=et.FiscalYearPeriodID where e.CancelDate is null and  m.ModuleID=" & GetModuleID("frmPrepareSalaries") & " And IsNull(m.CloseDate,'')='' And et.FiscalYearPeriodID=" & ClsFisicalPeriods.ID
            strCommand &= "  and (select isnull(count(hrs_EmployeesTransactions.ID),0) from hrs_EmployeesTransactions where hrs_EmployeesTransactions.ID = et.ID and hrs_EmployeesTransactions.PostDate is not null) >0 and (select isnull(count(hrs_HrsTrans.ID),0) from hrs_HrsTrans where hrs_HrsTrans.RegComputerID = et.ID and hrs_HrsTrans.ProfileHeadID is not null) > 0 "
