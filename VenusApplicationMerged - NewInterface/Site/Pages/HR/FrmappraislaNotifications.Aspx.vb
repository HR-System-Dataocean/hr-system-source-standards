    ...
    Private Sub GetAppraisalNotifications()
        Try
            ...
            If ProfileCls.CurrentLanguage = "Ar" Then
                ...
                AppraisalName = "APP_Appraisals.ArabName "
                AppraisalType = " App_AppraisalTypes.ArbName "
            Else
                EmpName = " dbo.fn_GetEmpName(hrs_employees.Code,0)"
                AppraisalName = "APP_Appraisals.EngName "
                AppraisalType = " App_AppraisalTypes.EngName "

            End If

            Dim str1 As String
            Dim Criteria As String = ""
            str1 = "select " & ClsEmployees.ID & " as UserEmployeeID,ConfigurationID,ConfigurationLevel,IsObjection, App_AppraisalNotifications.ID as NotificationID,App_AppraisalNotifications.AppraisalID,App_AppraisalNotifications.EmployeeID, APP_Appraisals.Code  as AppraisalCode, " & AppraisalType & " as AppraisalType, " & AppraisalName & " as AppraisalName," & EmpName & " as EmployeeName from App_AppraisalNotifications join APP_Appraisals on App_AppraisalNotifications.AppraisalID=APP_Appraisals.ID join App_AppraisalTypes on APP_Appraisals.AppraisalTypeID=App_AppraisalTypes.ID join Hrs_Employees on Hrs_Employees.ID=App_AppraisalNotifications.EmployeeID where (Completed is null or Completed=0) and APP_EmployeeID= " & ClsEmployees.ID & ""
            If DdlAppraisalType.SelectedValue > 0 Then
                Criteria &= " And  APP_Appraisals.AppraisalTypeID = " & DdlAppraisalType.SelectedValue & " "
            End If
            If ddlAppraisals.SelectedValue > 0 Then
                Criteria &= " And  APP_Appraisals.ID = " & ddlAppraisals.SelectedValue & " "

            End If
            If Not String.IsNullOrEmpty(txtEmpCode.Text) Then
                ClsEmployees.Find("Code = '" & txtEmpCode.Text & "'")
                If ClsEmployees.ID > 0 Then
                    Criteria &= " And App_AppraisalNotifications.EmployeeID=" & ClsEmployees.ID & " "
