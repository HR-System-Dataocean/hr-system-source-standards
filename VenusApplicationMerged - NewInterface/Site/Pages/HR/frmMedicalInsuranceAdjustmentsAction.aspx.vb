...
Partial Class frmMedicalInsuranceAdjustmentsAction
    Inherits MainPage
...
#Region "Protected Sub"
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            'lll
            Dim str1 As String
            str1 = "select " & EmpName & " as EmployeeName,( case when " & ActionName & " is not null then " & ActionName & " else 'Pending ...' end) As Action ,convert(varchar, ActionDate,103) as ActionDate,ActionRemarks  from SS_MedicalInsuranceAdjustments join SS_RequestActions on SS_MedicalInsuranceAdjustments.ID=SS_RequestActions.RequestSerial and SS_MedicalInsuranceAdjustments.EmployeeID=SS_RequestActions.EmployeeID join hrs_Employees on hrs_Employees.ID= SS_RequestActions.SS_EmployeeID left join SS_UserActions on SS_RequestActions.ActionID=SS_UserActions.ID where RequestSerial=" & RequestSerial & " and ( SS_RequestActions.IsHidden is null or SS_RequestActions.IsHidden=0 ) and FormCode='SS_001924' "
            ...
            strselect = "select SS_MedicalInsuranceAdjustments.ID as RequestSerial,SS_MedicalInsuranceAdjustments.EmployeeID,hrs_employees.Code as EmpCode," & EmpName & " as EmployeeName ,Convert(date,RequestDate) as RequestDate,SS_MedicalInsuranceAdjustments.Remarks, " & Position & " As Position , " & Department & " As Department FROM            dbo.SS_MedicalInsuranceAdjustments INNER JOIN dbo.hrs_Employees ON dbo.SS_MedicalInsuranceAdjustments.EmployeeID = dbo.hrs_Employees.ID INNER JOIN dbo.hrs_Contracts ON dbo.hrs_Employees.ID = dbo.hrs_Contracts.EmployeeID and( hrs_Contracts.EndDate>=getdate() or hrs_Contracts.EndDate is null)  INNER JOIN dbo.hrs_Positions ON dbo.hrs_Contracts.PositionID = dbo.hrs_Positions.ID INNER JOIN dbo.sys_Departments ON dbo.hrs_Employees.DepartmentID = dbo.sys_Departments.ID   where  SS_MedicalInsuranceAdjustments.ID=" & RequestSerial & ""
    ...
    Protected Sub btnSave_Click(sender As Object, e As Infragistics.WebUI.WebDataInput.ButtonEventArgs) Handles btnSave.Click
        ...
        If ddlAction.SelectedValue > 0 Then
            ...
            If ddlAction.SelectedValue = 2 Then   'رفض
                ...
                UpdateCommand = "update SS_RequestActions set  seen=1 , ActionID=" & ddlAction.SelectedValue & " ,ActionDate= GETDATE(),ActionRemarks='" + TxtRemarks.Text + "'  where ConfigID=" & ConfigID & " And FormCode='SS_001924' and RequestSerial=" & RequestSerial & " and SS_EmployeeID=" & ClsEmployees_SS.ID & " And ActionID Is Null "
                ...
                UpdateCommandRank = "UPDATE SS_MedicalInsuranceAdjustments SET [RequestStautsTypeID] = 2 WHERE ID=" & RequestSerial & ""
                ...
                ClsEmployees.SendEmail("frmMedicalInsuranceAdjustmentsAction", Me.Page, 1, "SS_RequestActions", actionSerial)
            ...
            If ddlAction.SelectedValue = 1 Then
                ...
                UpdateCommandRank1 = "UPDATE SS_MedicalInsuranceAdjustments SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
                ...
                SqlCommand.Connection.Close()
                        Dim SqlCommandRank As Data.SqlClient.SqlCommand
                        Dim UpdateCommandRank As String = ""
                        UpdateCommandRank = "UPDATE SS_MedicalInsuranceAdjustments SET [RequestStautsTypeID] = 1 WHERE ID=" & RequestSerial & ""
                        SqlCommandRank = New SqlClient.SqlCommand
                        SqlCommandRank.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                        SqlCommandRank.CommandType = CommandType.Text
                        SqlCommandRank.CommandText = UpdateCommandRank
...
#End Region
    ...
    Private Function ProcessForwardForReview(ByVal RequestSerial As Integer, ByVal ConfigID As Integer, ByVal ReviewerCode As String, ByVal ActionRemarks As String, ByVal ConfirmedNoOfDays As String) As Boolean
        Try
            ...
            Dim UpdateStatus As String = "UPDATE SS_MedicalInsuranceAdjustments SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
