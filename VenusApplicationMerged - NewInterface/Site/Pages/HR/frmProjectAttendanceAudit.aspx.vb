...
Partial Class frmProjectAttendanceAudit
    Inherits MainPage
...
#Region "Protected Sub"
    ...
    Protected Sub Page_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            ClsProjects.GetDropDownList(ddlProject, True, "IsLocked = 1 and isnull(IsStoped,0) = 0 and CancelDate is null and convert(Datetime,'" & txtDate.Text & "') between StartDate and EndDate and ID in (select ProjectID from hrs_ProjectChanges where ID in (select ProjectChangeID from hrs_ProjectLocations where Supervisor = " & ClsProjects.DataBaseUserRelatedID & "))")
            ...
            If clsprojectchange.Find("ProjectID = " & ddlProject.SelectedValue & " and RegComputerID = 1 and FromDate <= convert(Datetime,'" & txtDate.Text & "') order by ID DESC") Then
                ...
                cls_ProjectLocations.GetDropDownList(ddlLocation, True, "ProjectChangeID = " & clsprojectchange.ID & " and Supervisor = " & clsprojectchange.DataBaseUserRelatedID)
    ...
    Private Function Confirm() As Boolean
        Try
            ...
            dsAudit = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(AttendanceTransactions.ConnectionString, Data.CommandType.Text, "select * from hrs_ProjectLocationsAuditing where ProjectID = '" & ddlProject.SelectedValue & "' and LocationID = '" & ddlLocation.SelectedValue & "' and TrnsDate = '" & Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd") & "'")
            If dsAudit.Tables(0).Rows.Count > 0 Then
                If CheckBox_Confirmed.Checked Then
                    Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteNonQuery(AttendanceTransactions.ConnectionString, Data.CommandType.Text, "update hrs_ProjectLocationsAuditing set IsActive = 1 where ID = " & dsAudit.Tables(0).Rows(0)("ID"))
                Else
                    Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteNonQuery(AttendanceTransactions.ConnectionString, Data.CommandType.Text, "update hrs_ProjectLocationsAuditing set IsActive = 0 where ID = " & dsAudit.Tables(0).Rows(0)("ID"))
                End If
            Else
                If CheckBox_Confirmed.Checked Then
                    Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteNonQuery(AttendanceTransactions.ConnectionString, Data.CommandType.Text, "insert into hrs_ProjectLocationsAuditing values (" & ddlProject.SelectedValue & "," & ddlLocation.SelectedValue & ",'" & Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd") & "'," & AttendanceTransactions.DataBaseUserRelatedID & ",getdate(),1)")
                End If
            End If
            Return True
    ...
    Protected Sub Button_Click(sender As Object, e As System.EventArgs) Handles LinkButton_Prepare.Command, ImageButton_Prepare.Command
        Try
            Select Case DirectCast(e, System.Web.UI.WebControls.CommandEventArgs).CommandArgument
                Case "Prepare"
                    If ddlProject.SelectedValue > 0 And ddlLocation.SelectedValue > 0 Then
                        Dim AttendanceTransactions As New ClsAtt_AttendTransactions(Page)
...
#Region "Private Function"
    ...
    Private Function GetData() As Boolean
        Try
            ...
            If clsprojectchange.Find("ProjectID = " & ddlProject.SelectedValue & " and RegComputerID = 1 and FromDate <= convert(Datetime,'" & txtDate.Text & "') order by ID DESC") Then
                ...
                For Each DGRow As Infragistics.WebUI.UltraWebGrid.UltraGridRow In UwgSearchEmployees.Rows
                    If DGRow.Cells.FromKey("Marked").Value = 1 Then
                        DGRow.Style.BackColor = Drawing.Color.Wheat
                    End If
            ...
            End If
            Try
                CheckBox_Confirmed.Checked = False
                Dim AttendanceTransactions As New ClsAtt_AttendTransactions(Page)
                Dim dsAudit As New Data.DataSet
                ...
                If dsAudit.Tables(0).Rows.Count > 0 Then
                    CheckBox_Confirmed.Checked = True
                End If
            Catch ex As Exception
