...
#Region "Protected Sub"
    ...
    Protected Sub ImageButton_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs) Handles ImageButton_Save.Command, ImageButton_New.Command, ImageButton_Print.Command, ImageButton_Properties.Command, LinkButton_Properties.Command, ImageButton_Remarks.Command, LinkButton_Remarks.Command, ImageButton_Delete.Command
        ...
        hdDeletedStr.Value = ""
    End Sub
    Protected Sub ddlFormCode_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlFormCode.SelectedIndexChanged
        uwgSSConfiguration.Rows.Clear()
        'GetDropDownListGrid()
...
#Region "Private Functions"
    ...
    Public Function GetDropDownList() As Boolean
        ...
        strselect = "select * from SS_RequestTypes where NotActive =0 or NotActive  is null order by RequestEngName "
    ...
    Private Function SavePart() As Boolean
        If ddlFormCode.SelectedValue <> "0" Then
            SaveDG()
            Return True
        End If
    ...
    Private Function GetFormConfiguration()
        ...
        ConnectionString = ConfigurationManager.AppSettings("Connstring").ToString()
        Dim strselect As String
        strselect = "select ID, UserTypeID as UserType,Positionid as Position,EmployeeID as Employee,CanEdit,Rank,IsFinal,ApplyForAll,CanBeCanceledInThisLevel from [SS_Configuration] where FormCode='" & ddlFormCode.SelectedValue.Trim() & "'"
    ...
    Private Function SaveDG() As Boolean
        Try
            ...
            Dim CanBeCanceledInThisLevel As Boolean
            Dim FormConfigCount As Integer
            Try
                FormConfigCount = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteScalar(ConnectionString, Data.CommandType.Text, " Select count(ID) From SS_Configuration Where FormCode = '" & FormCode & "'")
            ...
            If FormConfigCount > 0 Then

                Dim str As String
                For Each DGRow As Infragistics.WebUI.UltraWebGrid.UltraGridRow In uwgSSConfiguration.Rows

                    CanEdit = CBool(DGRow.Cells("5").Value)
                    IsFinal = CBool(DGRow.Cells("6").Value)
                ...
                Next
                If hdDeletedStr.Value.ToString() <> "" Then
                    deletedString = hdDeletedStr.Value.ToString()
                    Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteNonQuery(ConnectionString, Data.CommandType.Text, deletedString)
                    deletedString = ""
                    hdDeletedStr.Value = ""
                End If
