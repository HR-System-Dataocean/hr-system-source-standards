...
#Region "Protected Sub"
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            'lll
            Dim str1 As String
            str1 = "select " & EmpName & " as EmployeeName,( case when " & ActionName & " is not null then " & ActionName & " else 'Pending ...' end) As Action ,convert(varchar, ActionDate,103) as ActionDate,ActionRemarks  from SS_AdvanceSalaryRequest join SS_RequestActions on SS_AdvanceSalaryRequest.ID=SS_RequestActions.RequestSerial and SS_AdvanceSalaryRequest.EmployeeID=SS_RequestActions.EmployeeID join hrs_Employees on hrs_Employees.ID= SS_RequestActions.SS_EmployeeID left join SS_UserActions on SS_RequestActions.ActionID=SS_UserActions.ID where RequestSerial=" & RequestSerial & " and ( SS_RequestActions.IsHidden is null or SS_RequestActions.IsHidden=0 ) and FormCode='SS_001914' "
            ...
            strselect = "select SS_AdvanceSalaryRequest.ID as RequestSerial,SS_AdvanceSalaryRequest.EmployeeID,hrs_employees.Code as EmpCode," & EmpName & " as EmployeeName ,Convert(date,RequestDate) as RequestDate,SS_AdvanceSalaryRequest.Remarks, " & Position & " As Position , " & Department & " As Department ,SS_AdvanceSalaryRequest.InstallmentDate,SS_AdvanceSalaryRequest.InstallmentsCount FROM            dbo.SS_AdvanceSalaryRequest INNER JOIN dbo.hrs_Employees ON dbo.SS_AdvanceSalaryRequest.EmployeeID = dbo.hrs_Employees.ID INNER JOIN dbo.hrs_Contracts ON dbo.hrs_Employees.ID = dbo.hrs_Contracts.EmployeeID and( hrs_Contracts.EndDate>=getdate() or hrs_Contracts.EndDate is null)  INNER JOIN dbo.hrs_Positions ON dbo.hrs_Contracts.PositionID = dbo.hrs_Positions.ID INNER JOIN dbo.sys_Departments ON dbo.hrs_Employees.DepartmentID = dbo.sys_Departments.ID   where  SS_AdvanceSalaryRequest.ID=" & RequestSerial & ""
            ...
            txtRequestDate.Text = CDate(DS2.Tables(0).Rows(0)("RequestDate").ToString()).ToShortDateString()
                ...
                requesttypestr = "Select " & SelectRequestType & " from SS_RequestTypes where RequestCode='SS_001914'"
    ...
    Protected Sub btnSave_Click(sender As Object, e As Infragistics.WebUI.WebDataInput.ButtonEventArgs) Handles btnSave.Click
        ...
        If ddlAction.SelectedValue > 0 Then
            ...
            If ddlAction.SelectedValue = 2 Then   'رفض
                ...
                UpdateCommandRank = "UPDATE SS_AdvanceSalaryRequest SET [RequestStautsTypeID] = 2 WHERE ID=" & RequestSerial & ""
                ...
                ClsEmployees.SendEmail("frmAdvanceSalaryRequestAction", Me.Page, 1, "SS_RequestActions", actionSerial)
            ...
            If ddlAction.SelectedValue = 1 Then
                ...
                If dsconfig.Tables(0).Rows.Count > 0 Then
                    ...
                    If CBool(dsconfig.Tables(0).Rows(0)("IsFinal")) Then
                        ...
                        Dim SqlCommandRank As Data.SqlClient.SqlCommand
                        Dim UpdateCommandRank As String = ""
                        UpdateCommandRank = "UPDATE SS_AdvanceSalaryRequest SET [RequestStautsTypeID] = 1 WHERE ID=" & RequestSerial & ""
                        SqlCommandRank = New SqlClient.SqlCommand
                        SqlCommandRank.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                        SqlCommandRank.CommandType = CommandType.Text
                        SqlCommandRank.CommandText = UpdateCommandRank
            ...
            If ddlAction.SelectedValue = 3 Then
                ...
                Dim SqlCommandRank As Data.SqlClient.SqlCommand
                Dim UpdateCommandRank As String = ""
                UpdateCommandRank = "UPDATE SS_AdvanceSalaryRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
                SqlCommandRank = New SqlClient.SqlCommand
                SqlCommandRank.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                SqlCommandRank.CommandType = CommandType.Text
                SqlCommandRank.CommandText = UpdateCommandRank
    ...
    Private Function SaveOverTimePayabilities(ByVal employeeId As Integer) As Boolean
        ...
        ClsTransactionsType.Find(" Code=246")
    ...
    Private Function SaveInstalment(ByVal IntId As Integer, ByVal empId As Integer) As Boolean
        ...
        Try
            ...
            Dim BasicSalary As Double ''''''''''11
            Dim Housing As Double  '''''''''''''13
            Dim Transportation As Double '''''''14
            Dim settlemntAmount As Double

            Dim ClsTransactionsType As New Clshrs_TransactionsTypes(Page)
            ClsTransactionsType.Find(" Code=11")
            ...
            BasicSalary = Convert.ToDouble(0)

            If ClsContractTransactions.ID > 0 Then
                BasicSalary = Math.Round(Convert.ToDouble(ClsContractTransactions.Amount), 2)
            End If
            ClsTransactionsType.Find(" Code=13")
            ClsContractTransactions.Find(" ContractID = " & ClsContracts.ID & " AND  TransactionTypeID = " & ClsTransactionsType.ID)
            Housing = Convert.ToDouble(0)
            If ClsContractTransactions.ID > 0 Then
                Housing = Math.Round(Convert.ToDouble(ClsContractTransactions.Amount), 2)
            End If
            ClsTransactionsType.Find(" Code=14")
            ClsContractTransactions.Find(" ContractID = " & ClsContracts.ID & " AND  TransactionTypeID = " & ClsTransactionsType.ID)
            Transportation = Convert.ToDouble(0)
            If ClsContractTransactions.ID > 0 Then
                Transportation = Math.Round(Convert.ToDouble(ClsContractTransactions.Amount), 2)
            End If

            settlemntAmount = BasicSalary + Housing + Transportation
            ...
            If LoanAmount > 0 Then
                ...
                Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteScalar(clsEmployeesPayability.ConnectionString, System.Data.CommandType.Text, StrSqlCommand)
            End If
            Dim StallCount As Integer = 1
            If txtInstallmentsNo.Text <> "" Then
                StallCount = CInt(txtInstallmentsNo.Text)
            End If
            settlemntAmount = settlemntAmount / StallCount
...
#End Region
    ...
    Private Function ProcessForwardForReview(ByVal RequestSerial As Integer, ByVal ConfigID As Integer, ByVal ReviewerCode As String, ByVal ActionRemarks As String, ByVal ConfirmedNoOfDays As String) As Boolean
        Try
            ...
            Dim UpdateStatus As String = "UPDATE SS_AdvanceSalaryRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
