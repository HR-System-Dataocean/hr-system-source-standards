...
#Region "Private Functions"
    Private Function SavePart() As Boolean
        ...
        Try
            ...
            Str &= " INSERT INTO [dbo].[sys_SystemConfig] ([UseCostCenter],[MinimumCostCentersCount],PreventChangeContractEndDate,RegUserID,MultiBranchedPosition,CompanyId,AllowDelayInstallmentPart,ShowVacationsNotifications,ShowPostingNotification,JournalEmployeeFields,LockJoinDate,showActingPopUpEndService,ApplyAutoDelegateWithAvnnualVacationRequest,DirectManagerIsMandatory,ReviewPreviousPeriodSalaries) VALUES(" & If(chkUseCostCenter.Checked, "1", "0") & "," & count & "," & If(ChkPreventChangeEndDate.Checked, "1", "0") & ",'" & User & "'," & If(chkMultiBranchedPosition.Checked, "1", "0") & "," & DdlCompany.SelectedValue & "," & If(chkAllowDelayInstallmentPart.Checked, "1", "0") & "," & If(chkShowVacationsNotifications.Checked, "1", "0") & "," & If(chkShowPostingNotification.Checked, "1", "0") & ",'" & journalEmployeeFields & "'," & If(ChkLockJoinDate.Checked, "1", "0") & "," & If(chkShowActingPopUpEndService.Checked, "1", "0") & "," & If(chkApplyAutoAdvanceDelegation.Checked, "1", "0") & "," & If(chkDirectManagerIsMandatory.Checked, 1, 0) & "," & If(chkReviewPreviousPeriodSalaries.Checked, "1", "0") & ")"
    ...
    Private Function GetValues() As Boolean
        ...
        Try
            ...
            For Each dr As DataRow In dsActions.Tables(0).Rows
                ...
                If Not IsDBNull(dr("ShowVacationsNotifications")) Then
                    chkShowVacationsNotifications.Checked = CBool(dr("ShowVacationsNotifications"))
                End If
                ...
                End If
                If dsActions.Tables(0).Columns.Contains("showActingPopUpEndService") AndAlso Not IsDBNull(dr("showActingPopUpEndService")) Then
                    chkShowActingPopUpEndService.Checked = CBool(dr("showActingPopUpEndService"))
                Else
                    chkShowActingPopUpEndService.Checked = False
                End If
                If dsActions.Tables(0).Columns.Contains("ApplyAutoDelegateWithAvnnualVacationRequest") AndAlso Not IsDBNull(dr("ApplyAutoDelegateWithAvnnualVacationRequest")) Then
                ...
                End If
                If dsActions.Tables(0).Columns.Contains("ReviewPreviousPeriodSalaries") AndAlso Not IsDBNull(dr("ReviewPreviousPeriodSalaries")) Then
                    chkReviewPreviousPeriodSalaries.Checked = CBool(dr("ReviewPreviousPeriodSalaries"))
                Else
                    chkReviewPreviousPeriodSalaries.Checked = False
                End If
            Next
            Return True
        Catch ex As Exception
