...
#Region "Protected Sub"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            txtExecuseType.Enabled = False
            txtRequestDate.Enabled = False
            txtDescEnglishName.Enabled = False
            txtExecuseDate.Enabled = False
            TxtExecuseReason.Enabled = False
            TxtExecuseShift.Enabled = False
            txtExecuseType.Enabled = False
            If CanEdit Then
                TxtExecuseTime.Enabled = True
            Else
                TxtExecuseTime.Enabled = False
    ...
    Private Sub FillEmployeeVacations()
        Try
            ...
            'lll
            Dim str1 As String
            str1 = "select " & EmpName & " as EmployeeName,( case when " & ActionName & " is not null then " & ActionName & " else 'Pending ...' end) As Action,ConfirmedNoOfdays ,convert(varchar, ActionDate,103) as ActionDate,ActionRemarks  from SS_ExecuseRequest join SS_RequestActions on SS_ExecuseRequest.ID=SS_RequestActions.RequestSerial and SS_ExecuseRequest.EmployeeID=SS_RequestActions.EmployeeID join hrs_Employees on hrs_Employees.ID= SS_RequestActions.SS_EmployeeID left join SS_UserActions on SS_RequestActions.ActionID=SS_UserActions.ID where RequestSerial=" & RequestSerial & " and ( SS_RequestActions.IsHidden is null or SS_RequestActions.IsHidden=0 ) and FormCode='SS_0014'"
            ...
            Dim firstshiftString As String
            If ProfileCls.CurrentLanguage = "Ar" Then
                firstshiftString = "وردية اولى"
            Else
                firstshiftString = "First Shift"
            End If

            Dim secondshiftString As String
            If ProfileCls.CurrentLanguage = "Ar" Then
                secondshiftString = "وردية ثانية"
            Else
                secondshiftString = "Second Shift"
            End If

            strselect = "select SS_ExecuseRequest.ID as RequestSerial,SS_ExecuseRequest.EmployeeID,hrs_employees.Code as EmpCode," & EmpName & " as EmployeeName ,Convert(date,RequestDate) as RequestDate,ExecuseDate,ExecuseType,ExecuseReason,ExecuseTime,Case when ExecuseShift=1 then '" & firstshiftString & "' else  '" & secondshiftString & "' End as ExecuseShift,ExecuseRemarks, " & Position & " As Position , " & Department & " As Department, CONVERT(varchar, hrs_Employees.JoinDate, 103) as JoinDate  FROM            dbo.SS_ExecuseRequest INNER JOIN dbo.hrs_Employees ON dbo.SS_ExecuseRequest.EmployeeID = dbo.hrs_Employees.ID INNER JOIN dbo.hrs_Contracts ON dbo.hrs_Employees.ID = dbo.hrs_Contracts.EmployeeID and( hrs_Contracts.EndDate>=getdate() or hrs_Contracts.EndDate is null)  INNER JOIN dbo.hrs_Positions ON dbo.hrs_Contracts.PositionID = dbo.hrs_Positions.ID INNER JOIN dbo.sys_Departments ON dbo.hrs_Employees.DepartmentID = dbo.sys_Departments.ID   where  SS_ExecuseRequest.ID=" & RequestSerial & ""
            ...
            Else
                ...
                txtExecuseDate.Text = CDate(DS2.Tables(0).Rows(0)("ExecuseDate").ToString()).ToShortDateString()
                TxtExecuseTime.Text = DS2.Tables(0).Rows(0)("ExecuseTime").ToString()
                TxtExecuseShift.Text = DS2.Tables(0).Rows(0)("ExecuseShift").ToString()
                txtExecuseType.Text = DS2.Tables(0).Rows(0)("ExecuseType").ToString()
                TxtExecuseReason.Text = DS2.Tables(0).Rows(0)("ExecuseReason").ToString()
                TxtRequestRemarks.Text = DS2.Tables(0).Rows(0)("ExecuseRemarks").ToString()
            ...
            requesttypestr = "Select " & SelectRequestType & " from SS_RequestTypes where RequestCode='SS_0014'"
        ...
        If ddlAction.SelectedValue > 0 Then
            ...
            If IsReviewer > 0 Then
                If ddlAction.SelectedValue = 1 Then
                    If ProcessReviewerDecision(RequestSerial, ConfigID, 1, TxtRemarks.Text, TxtExecuseTime.Text) Then
                        Page.ClientScript.RegisterStartupScript(Me.GetType(), "", "CloseMe()", True)
                    End If
                ElseIf ddlAction.SelectedValue = 2 Then
                    If ProcessReviewerDecision(RequestSerial, ConfigID, 2, TxtRemarks.Text, TxtExecuseTime.Text) Then
                        Page.ClientScript.RegisterStartupScript(Me.GetType(), "", "CloseMe()", True)
                    End If
                Else
            ...
            If ddlAction.SelectedValue = 2 Then   'رفض
                ...
                UpdateCommand = "update SS_RequestActions set  seen=1 , ActionID=" & ddlAction.SelectedValue & " ,ActionDate= GETDATE(),ConfirmedNoOfdays='" + TxtExecuseTime.Text + "',ActionRemarks='" + TxtRemarks.Text + "'  where ConfigID=" & ConfigID & " And FormCode='SS_0014' and RequestSerial=" & RequestSerial & " and SS_EmployeeID=" & ClsEmployees_SS.ID & " And ActionID Is Null "
                ...
                UpdateCommandRank = "UPDATE SS_ExecuseRequest SET [RequestStautsTypeID] = 2 WHERE ID=" & RequestSerial & ""
                ...
                ClsEmployees.SendEmail("FrmExceuseRequestAction", Me.Page, 1, "SS_RequestActions", actionSerial)
            ...
            If ddlAction.SelectedValue = 1 Then
                ...
                UpdateCommandRank1 = "UPDATE SS_ExecuseRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
                ...
                SqlCommand.Connection.Close()
                        Dim SqlCommandRank As Data.SqlClient.SqlCommand
                        Dim UpdateCommandRank As String = ""
                        UpdateCommandRank = "UPDATE SS_ExecuseRequest SET [RequestStautsTypeID] = 1 WHERE ID=" & RequestSerial & ""
                        SqlCommandRank = New SqlClient.SqlCommand
                        SqlCommandRank.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
                        SqlCommandRank.CommandType = CommandType.Text
                        SqlCommandRank.CommandText = UpdateCommandRank
            ...
            If ddlAction.SelectedValue = 3 Then
                ...
                If dsconfig.Tables(0).Rows.Count > 0 Then
                    ...
                    UpdateCommand = "update SS_RequestActions set  seen=1 , ActionID=" & ddlAction.SelectedValue & " ,ActionDate= GETDATE(),ActionRemarks='" + TxtRemarks.Text + "'  where ConfigID=" & ConfigID & " And FormCode='SS_0014' and RequestSerial=" & RequestSerial & " and SS_EmployeeID=" & ClsEmployees_SS.ID & " And ActionID Is Null "
                    SqlCommand = New SqlClient.SqlCommand
                    SqlCommand.Connection = New SqlClient.SqlConnection(ClsEmployees_SS.ConnectionString)
                    SqlCommand.CommandType = CommandType.Text
                    SqlCommand.CommandText = UpdateCommand
            ...
            If ddlAction.SelectedValue = 5 Then
                ...
                If ProcessForwardForReview(RequestSerial, ConfigID, txtReviewer.Text, TxtRemarks.Text, TxtExecuseTime.Text) Then
                    Page.ClientScript.RegisterStartupScript(Me.GetType(), "", "CloseMe()", True)
                End If
                Exit Sub
            End If
    ...
    Private Function AssignValues() As Boolean
        Try
            ...
            With ClsEmployeesExcuses
                ...
                .ExcuseCalcType = 1
                .SRC = "SS_0014"
                .RequestID = TxtRequestSerial.Text
...
#End Region
    ...
    Private Function ProcessForwardForReview(ByVal RequestSerial As Integer, ByVal ConfigID As Integer, ByVal ReviewerCode As String, ByVal ActionRemarks As String, ByVal ConfirmedNoOfDays As String) As Boolean
        Try
            ...
            Dim UpdateStatus As String = "UPDATE SS_ExecuseRequest SET [RequestStautsTypeID] = 4 WHERE ID=" & RequestSerial & ""
            ...
            Dim SqlCommand As Data.SqlClient.SqlCommand
            Dim UpdateCommand As String = "update SS_RequestActions set seen=1, ActionID=5, ActionDate=GETDATE(), ActionRemarks='" & ActionRemarks & "', ConfirmedNoOfDays='" & ConfirmedNoOfDays & "' where ConfigID=" & ConfigID & " and RequestSerial=" & RequestSerial & " and SS_EmployeeID=" & ClsEmployees2.ID & " and ActionID is null and seen<>1"
            SqlCommand = New SqlClient.SqlCommand
            SqlCommand.Connection = New SqlClient.SqlConnection(ClsEmployees.ConnectionString)
            SqlCommand.CommandType = CommandType.Text
            SqlCommand.CommandText = UpdateCommand
            ...
            Dim strinsert As String
            strinsert = "Insert Into SS_RequestActions (RequestSerial, SS_EmployeeID, FormCode, EmployeeID, Seen, ConfigID, IsForwarded, ForwardedBy, ConfirmedNoOfDays) " &
                        "values(" & RequestSerial & ", " & clsReviewerEmp.ID & ", '" & dsconfig.Tables(0).Rows(0)("FormCode") & "', " & ClsRequestEmployee.ID & ", 0, " & ConfigID & ", 1, " & _sys_User.RelEmployee & ", '" & ConfirmedNoOfDays & "')"
    ...
    Private Function ProcessReviewerDecision(ByVal RequestSerial As Integer, ByVal ConfigID As Integer, ByVal ActionID As Integer, ByVal ActionRemarks As String, ByVal ConfirmedNoOfDays As String) As Boolean
        Try
            ...
            Dim UpdateReviewer As String = "update SS_RequestActions set seen=1, ActionID=" & ActionID & ", ActionDate=GETDATE(), ActionRemarks='" & ActionRemarks & "', ConfirmedNoOfDays='" & ConfirmedNoOfDays & "' where ConfigID=" & ConfigID & " and RequestSerial=" & RequestSerial & " and SS_EmployeeID=" & _sys_User.RelEmployee & " and IsForwarded=1"
            ...
            Dim InsertManager As String = "Insert Into SS_RequestActions (RequestSerial, SS_EmployeeID, FormCode, EmployeeID, Seen, ConfigID, ConfirmedNoOfDays) " &
                                          "values(" & RequestSerial & ", " & ForwardedBy & ", '" & FormCode & "', " & EmployeeID & ", 0, " & ConfigID & ", '" & ConfirmedNoOfDays & "')"
