...
#Region "Public Decleration"
    Dim clsDwfDocumentTypes As ClsDwf_DocumentTypes
...
#Region "Protected Sub"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        clsDwfDocumentTypes = New ClsDwf_DocumentTypes(Me)
        Dim ClsObjects As New Clssys_Objects(Page)
        Dim ClsSearchs As New Clssys_Searchs(Page)
        Dim SearchID As Integer = 0
        Try
            Dim clsSearchsColumns As New Clssys_SearchsColumns(Page)
            If ClsObjects.Find(" Code='" & clsDwfDocumentTypes.Table.Trim & "'") Then
            ...
            If Not IsPostBack Then
                Page.Session.Add("ConnectionString", clsDwfDocumentTypes.ConnectionString)
                clsDwfDocumentTypes.AddOnChangeEventToControls("DocumentType", Page, UltraWebTab1)
            ...
            If (txtCode.Text <> "") Then
                clsDwfDocumentTypes.Find(" Code='" & txtCode.Text & "'")
                IntrecordID = clsDwfDocumentTypes.ID
        ...
        Catch ex As Exception
            ...
            mErrorHandler.RecordExceptions_DataBase("", ex, Err.Number, clsDwfDocumentTypes.RegUserID, Venus.Shared.ErrorsHandler.eRecordingType.System_DataBase)
    ...
    Protected Sub ImageButton_Command(sender As Object, e As System.Web.UI.WebControls.CommandEventArgs) Handles ImageButton_Save.Command, ImageButton_SaveN.Command, LinkButton_SaveN.Command, ImageButton_New.Command, ImageButton_Print.Command, ImageButton_Properties.Command, LinkButton_Properties.Command, ImageButton_Remarks.Command, LinkButton_Remarks.Command, ImageButton_Last.Command, ImageButton_Next.Command, ImageButton_Back.Command, ImageButton_First.Command, ImageButton_Delete.Command
        ...
        Dim ObjNavigationHandler As New Venus.Shared.Web.NavigationHandler(clsDwfDocumentTypes.ConnectionString)
        Select Case e.CommandArgument
            Case "SaveNew"
                ...
                If Not AssignValues() Then
                    Exit Sub
                End If
                If clsDwfDocumentTypes.ID > 0 Then
                    clsDwfDocumentTypes.Update("code='" & txtCode.Text & "'")
                Else
                    clsDwfDocumentTypes.Save()
                End If
                clsDwfDocumentTypes.Find("Code='" & txtCode.Text & "'")
                clsMainOtherFields = New clsSys_MainOtherFields(Me.Page)
                clsMainOtherFields.CollectDataAndSave(value.Text, clsDwfDocumentTypes.Table, clsDwfDocumentTypes.ID)
                value.Text = ""
                AfterOperation()
            Case "Save"
                If txtCode.Text = "" Then
            ...
            Case "New"
                AfterOperation()
            Case "Delete"
                clsDwfDocumentTypes.Delete("Code='" & txtCode.Text & "'")
                AfterOperation()
            Case "Property"
                clsDwfDocumentTypes.Find("Code='" & txtCode.Text & "'")
                Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmPropertyScreen.aspx?ID=" & clsDwfDocumentTypes.ID & "&TableName=" & clsDwfDocumentTypes.Table, 477, 313, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Property", False)
            Case "Remarks"
                clsDwfDocumentTypes.Find("Code='" & txtCode.Text & "'")
                Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmRemarks.aspx?ID=" & clsDwfDocumentTypes.ID & "&TableName=" & clsDwfDocumentTypes.Table, 410, 210, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Remarks", False)
            Case "Print"
                Venus.Shared.Web.ClientSideActions.PrintWindow(Me)
            Case "Other Fields"
                Dim clsObjOtherFields As New ClsSys_OtherFields(Page)
                Dim clsSysObjects As New Clssys_Objects(Page)
                Dim tablename As String = clsDwfDocumentTypes.Table
                clsDwfDocumentTypes.Find(" code = '" & txtCode.Text & "'")
                Dim recordID As Integer = clsDwfDocumentTypes.ID
                ...
                With clsObjOtherFields
                    If .Find(" sys_OtherFields.ObjectID = " & clsSysObjects.ID) = True Then
                        ...
                        Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmOtherFieldsDynamic.aspx?tableName=" & tablename & "&ObjectId=" & .ObjectID & "&RecordId=" & clsDwfDocumentTypes.ID, 602, 306, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "OtherFields", False)
                    End If
                End With
            Case "Exit"
                clsDwfDocumentTypes.Find(" Code= '" & txtCode.Text & "'")
                If clsDwfDocumentTypes.ID > 0 Then
                    Dim Ds As Data.DataSet = clsDwfDocumentTypes.DataSet
                    If Not AssignValues() Then
                        Exit Sub
                    End If
                    If clsDwfDocumentTypes.CheckDiff(clsDwfDocumentTypes, Ds, "") Then
                ...
                End If
            Case "First"
                clsDwfDocumentTypes.FirstRecord()
                GetValues()
            Case "Previous"
                clsDwfDocumentTypes.Find("Code='" & txtCode.Text & "'")
                If Not clsDwfDocumentTypes.previousRecord() Then
                    clsDwfDocumentTypes.Find("Code='" & txtCode.Text & "'")
            ...
            Case "Next"
                clsDwfDocumentTypes.Find("Code='" & txtCode.Text & "'")
                If Not clsDwfDocumentTypes.NextRecord() Then
                    clsDwfDocumentTypes.Find("Code='" & txtCode.Text & "'")
                    Venus.Shared.Web.ClientSideActions.MsgBoxBasic(Page, ObjNavigationHandler.SetLanguage(Page, " This is the last page /هذه أخر صفحة"))
                End If
                GetValues()
            Case "Last"
                clsDwfDocumentTypes.LastRecord()
                GetValues()
        End Select
    End Sub
...
#Region "Private Functions"
    ...
    Private Function GetValues() As Boolean
        ...
        Try
            SetToolBarDefaults()
            txtCode.Text = clsDwfDocumentTypes.Code
            txtEngName.Text = clsDwfDocumentTypes.EngName
            txtArbName.Text = clsDwfDocumentTypes.ArbName
         
            If Not clsDwfDocumentTypes.RegUserID = Nothing Then
                ClsUser.Find("ID=" & clsDwfDocumentTypes.RegUserID)
            End If
            ...
            End If
            If Convert.ToDateTime(clsDwfDocumentTypes.RegDate).Date = Nothing Then
                lblRegDateValue.Text = ""
            Else
                lblRegDateValue.Text = Convert.ToDateTime(clsDwfDocumentTypes.RegDate).Date
            End If
            If clsDwfDocumentTypes.CancelDate = Nothing Then
                lblCancelDateValue.Text = ""
            Else
                lblCancelDateValue.Text = Convert.ToDateTime(clsDwfDocumentTypes.CancelDate).Date
            End If
            If Not clsDwfDocumentTypes.CancelDate = Nothing Then
            ...
            Else
                StrMode = "N"
            End If
            SetToolBarPermission(Me, clsDwfDocumentTypes.ConnectionString, clsDwfDocumentTypes.DataBaseUserRelatedID, clsDwfDocumentTypes.GroupID, StrMode)
            SetToolBarRecordPermission(Me, clsDwfDocumentTypes.ConnectionString, clsDwfDocumentTypes.DataBaseUserRelatedID, clsDwfDocumentTypes.GroupID, clsDwfDocumentTypes.Table, clsDwfDocumentTypes.ID)
            If Not clsDwfDocumentTypes.CancelDate = Nothing Then
                ImageButton_Delete.Enabled = False
            End If
            If Page.IsPostBack Then
                CreateOtherFields(clsDwfDocumentTypes.ID)
            End If
            Return True
        Catch ex As Exception
        End Try
    ...
    Private Function SetToolbarSetting(ByVal ptrType As String, ByVal ClsClass As Object, ByVal intID As Integer) As Boolean
        Try
            Select Case ptrType
                ...
                Case "D"
                    clsDwfDocumentTypes.Find("ID=" & intID)
                    GetValues()
    ...
    Private Function CheckCode() As Boolean
        ...
        Try
            clsDwfDocumentTypes.Find("Code='" & txtCode.Text & "'")
            IntId = clsDwfDocumentTypes.ID
            txtEngName.Focus()
            If clsDwfDocumentTypes.ID > 0 Then
                GetValues()
                StrMode = "E"
            Else
                If clsDwfDocumentTypes.CheckRecordExistance(" Code='" & txtCode.Text & "'") Then
                    txtCode.Text = ""
                    txtCode.Focus()
                End If
                Clear()
            ...
            SetToolBarDefaults()
            SetToolBarPermission(Me, clsDwfDocumentTypes.ConnectionString, clsDwfDocumentTypes.DataBaseUserRelatedID, clsDwfDocumentTypes.GroupID, StrMode)
            SetToolBarRecordPermission(Me, clsDwfDocumentTypes.ConnectionString, clsDwfDocumentTypes.DataBaseUserRelatedID, clsDwfDocumentTypes.GroupID, clsDwfDocumentTypes.Table, IntId)
    ...
    Private Sub LoadDataUpdateSchedules(ByVal formName As String)
        ...
        clsDwfDocumentTypes = New ClsDwf_DocumentTypes(Page)
        clsDwfDocumentTypes.Find(" code = '" & txtCode.Text & "'")
        Dim recordID As Integer = clsDwfDocumentTypes.ID
        If (recordID > 0) Then
    ...
    Private Function CreateOtherFields(ByVal IntRecordID As Integer)
        ...
        clsMainOtherFields = New clsSys_MainOtherFields(Me.Page)
        If clsMainOtherFields.CheckOtherFieldsButton(Me.Page, clsDwfDocumentTypes.Table) = True Then
            Dim StrTablename As String
            clsDwfDocumentTypes = New ClsDwf_DocumentTypes(Me)
            StrTablename = clsDwfDocumentTypes.Table
