...
#Region "Protected Methods"
        ...
        If txtPreparedDays.Text <> "" Then
            If ClsEmployees.Find("Code='" & txtCode.Text.Trim & "'") Then
                ...
                For i As Integer = 0 To 10000
                    ...
                    Dim ClsContracts As New Clshrs_Contracts2(Page)
                    ...
                    If ClsContracts.ID > 0 Then
                        ClsContracts.CheckAnnualVacationDays(ClsContracts.ID, VacTypeID, True, dblVacationDays, totalDays, unpaidDays, NetDays, dtVacInfo, datecheck)
                ...
                'If ClsContracts.ID > 0 Then
                '    Dim ds As DataSet
                '    ClsContracts.GetContractVacation(ClsContracts.ID, datecheck, 0, ds)
                '    Dim day As Double = Convert.ToInt32(ds.Tables(0).Rows(0)("RequiredWorkingMonths")) / Convert.ToInt32(ds.Tables(0).Rows(0)("DurationDays"))
                '    Dim duedays As Integer = day * Convert.ToDouble(txtPreparedDays.Text)
                '    'Dim newdate As Date = datecheck.AddDays(duedays)



                '    'ClsContracts.GetContractVacation(ClsContracts.ID, newdate, 0, ds)
                '    'day = Convert.ToInt32(ds.Tables(0).Rows(0)("RequiredWorkingMonths")) / Convert.ToInt32(ds.Tables(0).Rows(0)("DurationDays"))
                '    'duedays = day * Convert.ToDouble(txtPreparedDays.Text)
                '    wdtPaymentDate.Value = datecheck.AddDays(duedays)
                'End If
    ...
    Protected Sub btnSave_Command(sender As Object, e As System.Web.UI.WebControls.CommandEventArgs) Handles btnSave.Command, btnDelete.Command
        ...
        Dim vactionTransactionInfo As New Clshrs_VactionTransactionInfo(Page)
        ...
        Select Case e.CommandName
            Case "Save"
                ...
                Dim clscontracts As New Clshrs_Contracts2(Me.Page)
                ...
                If Val(txtPreparedDays.Text) > 0 Or uwgEmployeeTransaction.Rows.Count > 0 Or uwgPayabilities.Rows.Count > 0 Then
                    ...
                    ClsEmployeesTransactions.FinancialWorkingUnits = Convert.ToDouble(txtPreparedDays.Text)
                    'If Request.QueryString.Item("TrnsID") <> 0 Then
                    ...
                    '------------------------------=============-----------------------------------------
                    ' Saving Vaction Info Details
                    '------------------------------=============-----------------------------------------

                    For Each objVaction As Infragistics.WebUI.UltraWebGrid.UltraGridRow In uwgVactionInfo.Rows
                        vactionTransactionInfo = New Clshrs_VactionTransactionInfo(Page)
                        vactionTransactionInfo.VacTrnsID = IntEmployeeTransactionID
                        vactionTransactionInfo.RequiredWorkingDays = objVaction.Cells.FromKey("RequiredWorkingDays").Value
                        vactionTransactionInfo.Duration = objVaction.Cells.FromKey("Duration").Value
                        vactionTransactionInfo.FromDate = objVaction.Cells.FromKey("FromDate").Value
                        vactionTransactionInfo.ToDate = objVaction.Cells.FromKey("ToDate").Value
                        vactionTransactionInfo.TotalDays = objVaction.Cells.FromKey("TotalDays").Value
                        vactionTransactionInfo.DeductionDays = objVaction.Cells.FromKey("DeductionDays").Value
                        vactionTransactionInfo.NetDays = objVaction.Cells.FromKey("NetDays").Value
                        vactionTransactionInfo.VactionBalance = objVaction.Cells.FromKey("VactionBalance").Value
                        vactionTransactionInfo.Amount = objVaction.Cells.FromKey("Amount").Value
                        vactionTransactionInfo.Save()
                    ...
                    If chkWithSalary.Checked Then
                        ...
                        Dim strcommand As String = "set dateformat dmy; insert into hrs_EmployeeExtraItems values ((select Code from hrs_Employees where ID = " & IntEmployeeID & "),''," & transactiontype.Code & "," & lblNetSalary.Value & "," & DdlPeriodsForSalary.SelectedValue & ",1,'" & DateTime.Now.ToString("dd/MM/yyyy") & "',5,'" & IntEmployeeTransactionID & "','','')"
...
#Region "Private Methods"
    ...
    Private ClsEmployeesContracts As Clshrs_Contracts2
    ...
    Private Function Load_ClsLayers() As Boolean
        Try
            ...
            ClsEmployeesContracts = New Clshrs_Contracts2(Page)
    ...
    Private Function CheckEmployee(ByVal IntInComingEmployeeID As Integer, ByVal FiscalPeriodID As Integer) As Integer
        ClsEmployees = New Clshrs_Employees(Page)
        Dim ClsContract As New Clshrs_Contracts2(Page)
    ...
    Private Function Calculate_SalaryPerHour(ByRef Amount As Double, empID As Integer, ByRef TrnsDate As DateTime) As Boolean
        Try
            ...
            ClsEmployeesContracts = New Clshrs_Contracts2(Me.Page)
    ...
    Private Sub SetData(Optional ByVal EmployeeID As Integer = 0, Optional ByVal intEmpTransID As Integer = 0)
        ...
        dtVacInfo = New DataTable
        If ClsContracts.ID > 0 Then
            Dim lastDays As Double = ClsContracts.CheckAnnualVacationDays(ClsContracts.ID, VacTypeID, True, dblVacationDays, totalDays, unpaidDays, NetDays, dtVacInfo, DteVacationStartDate)
            If lastDays > 0 Then
            ...
            Dim TempDtBenefitsCurr As New Data.DataTable
            Dim TempDtDeductionsCurr As New Data.DataTable
            Dim vacSalaryToDate As Date

            For Each row In dtVacInfo.Rows
                vacSalaryToDate = DteVacationStartDate
                If row("ToDate") <> "" Then
                    vacSalaryToDate = row("ToDate")
                End If
                ClsFisicalPeriod.GetFisicalperiodInfo(vacSalaryToDate, IntPeriodId, DteFromDate, DteToDate)
                ClsEmployees.CollectEmployeesTransactions("", ToDate, intEmployeeID, IntPeriodId, TempDtBenefitsCurr, TempDtDeductionsCurr, dblBenefits, dblDeduct, row("VactionBalance"), 0, row("VactionBalance"), 0, 0, 0, 0, 0, 0, Clshrs_EmployeesBase.ePrepareType.EndOfContract, Clshrs_EmployeesBase.ePrepareStage.Vacation)
                If DtBenefitsCurr.Rows.Count = 0 Then
                    DtBenefitsCurr = TempDtBenefitsCurr.Clone()

                End If
                Dim totalAmount As Double = 0
                For Each r In TempDtBenefitsCurr.Rows
                    DtBenefitsCurr.ImportRow(r)
                    If r.Item("DescriptionSign") = "Paid" Or r.Item("DescriptionSign") = "Paid By Days" Then
                        totalAmount += r("Amount")
                    End If
                Next
                row("Amount") = totalAmount
            Next
            uwgVactionInfo.DataSource = dtVacInfo
            uwgVactionInfo.DataBind()
            ...
            If intEmpTransID > 0 Then
                If ClsEmployeesTransactions.Find("ID=" & intEmpTransID) Then
                    ...
                    ClsEmployeesTransactionsDetOtherBenfites.FindRelatedToProjects("EmployeeTransactionID = " & ClsEmployeesTransactionsOtherBenfites.ID & " And (Select sign From hrs_TransactionsTypes Where ID=hrs_EmployeesTransactionsDetails.TransactionTypeID)=1 And (Select ispaid From hrs_TransactionsTypes Where ID=hrs_EmployeesTransactionsDetails.TransactionTypeID)=1")
