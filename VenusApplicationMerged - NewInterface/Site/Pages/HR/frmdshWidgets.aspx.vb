...
Partial Class frmdshWidgets
    Inherits MainPage
#Region "Public Decleration"
    Private ClsdshWidgets As Clsdsh_Widgets
...
#Region "Protected Sub"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        ClsdshWidgets = New Clsdsh_Widgets(Me.Page)
        Dim clsPages As New Clsdsh_Pages(Me.Page)
        Dim ClsObjects As New Clssys_Objects(Page)
        Dim ClsSearchs As New Clssys_Searchs(Page)
        Dim SearchID As Integer = 0
        Try
            Dim clsSearchsColumns As New Clssys_SearchsColumns(Page)
            If ClsObjects.Find(" Code='" & ClsdshWidgets.Table.Trim & "'") Then
            ...
            If Not IsPostBack Then
                Page.Session.Add("ConnectionString", ClsdshWidgets.ConnectionString)
                ClsdshWidgets.AddOnChangeEventToControls("frmdshWidgets", Page, UltraWebTab1)
                ...
                clsPages.GetDropDownList(DdlPage, True)
            End If
            '================================== Add DateUpdateSchedules [Start]
            Dim IntrecordID As Integer
            If (txtCode.Text <> "") Then
                ClsdshWidgets.Find(" Code='" & txtCode.Text & "'")
                IntrecordID = ClsdshWidgets.ID
        ...
        Catch ex As Exception
            ...
            mErrorHandler.RecordExceptions_DataBase("", ex, Err.Number, ClsdshWidgets.RegUserID, Venus.Shared.ErrorsHandler.eRecordingType.System_DataBase)
    ...
    Protected Sub ImageButton_Command(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.CommandEventArgs) Handles ImageButton_Save.Command, ImageButton_SaveN.Command, LinkButton_SaveN.Command, ImageButton_New.Command, ImageButton_Print.Command, ImageButton_Properties.Command, LinkButton_Properties.Command, ImageButton_Remarks.Command, LinkButton_Remarks.Command, ImageButton_Last.Command, ImageButton_Next.Command, ImageButton_Back.Command, ImageButton_First.Command, ImageButton_Delete.Command
        ...
        Dim ObjNavigationHandler As New Venus.Shared.Web.NavigationHandler(ClsdshWidgets.ConnectionString)
        Select Case e.CommandArgument
            ...
            Case "New"
                AfterOperation()
            Case "Delete"
                ClsdshWidgets.Delete("Code='" & txtCode.Text & "'")
                AfterOperation()
            Case "Property"
                ClsdshWidgets.Find("Code='" & txtCode.Text & "'")
                Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmPropertyScreen.aspx?ID=" & ClsdshWidgets.ID & "&TableName=" & ClsdshWidgets.Table, 477, 313, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Property", False)
            Case "Remarks"
                ClsdshWidgets.Find("Code='" & txtCode.Text & "'")
                Venus.Shared.Web.ClientSideActions.OpenWindowAdv(Page, "frmRemarks.aspx?ID=" & ClsdshWidgets.ID & "&TableName=" & ClsdshWidgets.Table, 410, 210, False, Venus.Shared.Web.ClientSideActions.WINDOW_TARGET._Blank, "Remarks", False)
            Case "Print"
            ...
            Case "First"
                ClsdshWidgets.Find("Code='" & txtCode.Text & "'")
                ClsdshWidgets.FirstRecord()
                GetValues(ClsdshWidgets)
            Case "Previous"
                ClsdshWidgets.Find("Code='" & txtCode.Text & "'")
                If Not ClsdshWidgets.previousRecord() Then
                    ClsdshWidgets.Find("Code='" & txtCode.Text & "'")
            ...
            Case "Next"
                ClsdshWidgets.Find("Code='" & txtCode.Text & "'")
                If Not ClsdshWidgets.NextRecord() Then
                    ClsdshWidgets.Find("Code='" & txtCode.Text & "'")
...
#Region "Private Functions"
    Private Function SavePart() As Boolean
        Dim StrMode As String = Request.QueryString.Item("Mode")
        ClsdshWidgets = New Clsdsh_Widgets(Page)

        Try
            ClsdshWidgets.Find("Code='" & txtCode.Text & "'")

            If ClsdshWidgets.ID > 0 Then
                If Not AssignValue(ClsdshWidgets) Then
                    Exit Function
                End If
                ClsdshWidgets.Update("Code='" & txtCode.Text & "'")
            ...
            clsMainOtherFields.CollectDataAndSave(value.Text, ClsdshWidgets.Table, ClsdshWidgets.ID)
            value.Text = ""
            Return True
        Catch ex As Exception
            Return False
        End Try
    End Function
    Private Function AssignValue(ByRef ClsdshWidgets As Clsdsh_Widgets) As Boolean
        Try
            With ClsdshWidgets
                .Code = txtCode.Text
                ...
                .AlocationCol = txtCol.Value
                .AlocationRw = txtRws.Value
                .Src = txtSrc.Text
                .Remarks = IIf(rbHorizontal.Checked, "H", IIf(rbVertical.Checked, "V", ""))
                .ViewType = IIf(rbchart.Checked, 2, 1)
                .PageID = DdlPage.SelectedIndex
            End With
        ...
        End Try
    End Function
    Private Function GetValues(ByVal ClsdshWidgets As Clsdsh_Widgets) As Boolean
        ...
        Try
            ...
            With ClsdshWidgets
                ...
                txtCol.Value = .AlocationCol
                txtRws.Value = .AlocationRw
                txtSrc.Text = .Src
                If .Remarks = "H" Then
                    rbHorizontal.Checked = True
                ElseIf .Remarks = "V" Then
                ...
                If Page.IsPostBack Then
                    CreateOtherFields(ClsdshWidgets.ID)
                End If
            End With
            Return True
        Catch ex As Exception
    ...
    Private Function GetChildBranches(ByVal BranchID As Integer, ByVal Connection As String, ByRef RetStr As String) As Boolean
        Dim strSelect As String = " Select ID from sys_Branches where ParentID = " & BranchID
    ...
    Private Function CheckCode() As Boolean
        ...
        Try
            ClsdshWidgets.Find("Code='" & txtCode.Text & "'")
            IntId = ClsdshWidgets.ID
            txtEngName.Focus()
            If ClsdshWidgets.ID > 0 Then
                GetValues(ClsdshWidgets)
                StrMode = "E"
            Else
                If ClsdshWidgets.CheckRecordExistance(" Code='" & txtCode.Text & "'") Then
                    txtCode.Text = ""
                    txtCode.Focus()
                End If
            ...
            End If
            SetToolBarDefaults()
            SetToolBarPermission(Me, ClsdshWidgets.ConnectionString, ClsdshWidgets.DataBaseUserRelatedID, ClsdshWidgets.GroupID, StrMode)
            SetToolBarRecordPermission(Me, ClsdshWidgets.ConnectionString, ClsdshWidgets.DataBaseUserRelatedID, ClsdshWidgets.GroupID, ClsdshWidgets.Table, IntId)
    ...
    Private Sub LoadDataUpdateSchedules(ByVal formName As String)
        ...
        ClsdshWidgets = New Clsdsh_Widgets(Me.Page)
        ClsdshWidgets.Find(" code = '" & txtCode.Text & "'")
        Dim recordID As Integer = ClsdshWidgets.ID
        If (recordID > 0) Then
    ...
    Private Function CreateOtherFields(ByVal IntRecordID As Integer)
        ...
        If clsMainOtherFields.CheckOtherFieldsButton(Me.Page, ClsdshWidgets.Table) = True Then
            Dim StrTablename As String
            ClsdshWidgets = New Clsdsh_Widgets(Me)
            StrTablename = ClsdshWidgets.Table
