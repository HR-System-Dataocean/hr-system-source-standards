...
function UltraWebTree1ReportTree_NodeClick(treeId, nodeId, button)
{
     ...
     if (startvalue == 'r=')
     {
         if (targetValue.substring(2).includes('Sti')) {
             CallStiReport(targetValue.substring(2)); // <-- replace with your actual method name
         }
         else {
...
function CallStiReport(ReportCode) {
    var win = window.open("frmReportsViewerCriteriasSti.aspx?Code=" + ReportCode, "ReportScreen", "height=500,width=730,resizable=0,menubar=0,toolbar=0,location=0,directories=0,scrollbars=0,status=0,center=1");
    win.focus();
}
