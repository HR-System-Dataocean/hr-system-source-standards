<%@ Page Language="VB" AutoEventWireup="false" CodeFile="frmReportsViewerCriteriasSti.aspx.vb"
    Inherits="Interfaces_frmReportsViewerCriteriasSti" Culture="auto" meta:resourcekey="PageResource1"
    UICulture="auto" %>
...
<head runat="server">
    ...
    <script id="igClientScript" type="text/javascript">
        ...
        function ShowStiOnly(oButton, oEvent) {
            // Open only the Stimulsoft viewer; skip legacy C1 flow
            OpenStiViewerFromCriteria();
            return false;
        }

        function OpenStiViewerFromCriteria() {
            ...
            var arr = ctrTxtRealNames.value.split("/");
            var params = {};
            for (var i = 0; i < arr.length; i++) {
                var kind = arr[i].substring(0, 3);
                var realName = arr[i].substring(3);
                var value = "";
                if (kind === "txt" || kind === "Num" || kind === "Cur" || kind === "Drd" || kind === "Drl") {
                    var c1 = igtab_getElementById(arr[i], ultraTab.element);
                    if (c1) value = c1.value;
                } else if (kind === "Msk") {
                    var c2 = igtab_getElementById("UltraWebTab1__ctl0_" + arr[i]);
                    if (c2) value = c2.value;
                } else if (kind === "Dte") {
                    var c3 = igdrp_getComboById("UltraWebTab1__ctl0_" + arr[i]);
                    if (c3) value = c3.getText();
                }
                if (value !== "" && realName) params[realName] = value;
            }
            var chk = igtab_getElementById("ChkArabicView", ultraTab.element);
            if (chk) params["ArabicView"] = !!chk.checked;
            var code = ctrTxtReportCode.value || "";
            var reportPath = resolveStiReportPath(code);
            var url = "/Pages/Reports/StiViewer.aspx?report=" + encodeURIComponent(reportPath) + "&params=" + encodeURIComponent(JSON.stringify(params));
            window.open(url, "_blank");
        }

        function resolveStiReportPath(code) {
            if (!code) return "SSReport/Emp.mrt";
            return "SSReport/" + code + ".mrt";
        }
    </script>
</head>
<body>
    ...
    <div style="left: 0px; width: 732px; position: absolute; top: 0px; height: 499px;
        ...
        <igtxt:WebImageButton ID="btnPreview" runat="server" meta:resourcekey="btnPreviewResource1"
            ...
            <RoundedCorners HeightOfBottomEdge="0" HoverImageUrl="ig_butMac2.gif" ImageUrl="ig_butMac1.gif"
                MaxHeight="23" MaxWidth="300" PressedImageUrl="ig_butMac4.gif" RenderingType="FileImages"
                WidthOfRightEdge="13" />
            <ClientSideEvents Click="ShowStiOnly" />
        </igtxt:WebImageButton>
