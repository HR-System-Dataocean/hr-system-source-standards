...
'Developer         : [0257]
'Description       : 1-Implement Data Acess Layer of hrs_GradesSteps table with fields code,English name,
'                       Arabic name,Grade ID and Step
'                    2-Allow searching
...
Public Class Clshrs_GradesSteps
...
#Region "Class Constructors"
    ...
    Public Sub New(ByVal Page As Web.UI.Page)
        MyBase.New(Page)
        mTable = " hrs_GradesSteps "
        mInsertParameter = " Code,EngName,ArbName,ArbName4S,GradeID,Step,Remarks,RegUserID,RegComputerID "
        mInsertParameterValues = " @Code,@EngName,@ArbName,@SearchArbName,@GradeID,@Step,@Remarks,@RegUserID,@RegComputerID "
        mUpdateParameter = " Code=@Code,EngName=@EngName,ArbName=@ArbName,ArbName4S=@SearchArbName,GradeID=@GradeID,Step=@Step,Remarks=@Remarks,RegUserID=@RegUserID,RegComputerID=@RegComputerID "
        mSelectCommand = " Select hrs_GradesSteps.ID,hrs_GradesSteps.Code,hrs_GradesSteps.EngName,hrs_GradesSteps.ArbName,hrs_GradesSteps.ArbName4S,hrs_GradesSteps.GradeID,hrs_GradesSteps.Step,hrs_GradesSteps.Remarks,hrs_GradesSteps.RegUserID,hrs_GradesSteps.RegComputerID,hrs_GradesSteps.RegDate,hrs_GradesSteps.CancelDate From  " & mTable & " INNER JOIN hrs_Grades ON hrs_GradesSteps.GradeID = hrs_Grades.ID"
...
#Region "Public Function"
    ...
    'Modifacations  :
    'Calls          :
    'From           :(frmGradesSteps.aspx.vb)TlbMainToolbar_ButtonClicked()
    '               :(frmGradesSteps.aspx.vb)TlbMainNavigation_ButtonClicked()
    '               :(frmGradesSteps.aspx.vb)GetValues()
    '               :(frmGradesSteps.aspx.vb)SavePart()
    '               :(frmGradesSteps.aspx.vb)SetToolbarSetting()
    '               :(frmGradesSteps.aspx.vb)Setsetting()
    '               :(frmGradesSteps.aspx.vb)CheckCode()
    'To             :GetParameter()
    'fn. Arguments  :
    '---------------------------------------------------------
    'Parmeter Name      : Data Type : Description
    ...
    Public Function Find(ByVal Filter As String) As Boolean
        ...
        Try
            ...
            If Filter.ToLower.IndexOf("order by") = -1 Then
                orderByStr = " Order By hrs_GradesSteps.Code "
            End If
            ...
            StrSelectCommand = mSelectCommand & IIf(Len(Filter) > 0, " Where IsNull(dbo.hrs_GetRecordViewStatus(hrs_GradesSteps.ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 And " & Filter, "  And IsNull(dbo.hrs_GetRecordViewStatus(hrs_GradesSteps.ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 ")
            '==================== Order By Modification [Start]
    ...
    Public Function SaveUpdate(ByVal Filter As String) As Boolean
        ...
        Try
            strSQL = "Select ID From hrs_Professions Where " & Filter
    ...
    'Modifacations  : 
    'Calls          :
    'From           :(frmGradesSteps.aspx.vb)SavePart()
    'To             :SetParameter()
    'fn. Arguments  :
    '---------------------------------------------------------
    'Parmeter Name      : Data Type : Description
    ...
    Public Function GetDropDownList(ByVal DdlValues As Global.System.Web.UI.WebControls.DropDownList, ByVal NullNode As Boolean, Optional ByVal Filter As String = "") As Boolean
        ...
        Try
            ...
            StrCommandString = " Select (Select EngName From hrs_Grades Where hrs_Grades.id= GradeID) as GradeEngName,(Select ArbName From hrs_Grades Where hrs_Grades.id= GradeID) as GradeArbName ,* from " & mTable & IIf(Len(Filter) > 0, " Where IsNull(CancelDate,'')=''  And IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 And " & Filter, "  Where IsNull(CancelDate,'')=''  And IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 ")
            ObjDataset = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(mConnectionString, CommandType.Text, StrCommandString)
            DdlValues.Items.Clear()
            ...
            For Each ObjDataRow In ObjDataset.Tables(0).Rows
                Item = New Global.System.Web.UI.WebControls.ListItem
                'Item.Text = _
                'mDataHandler.DataValue(ObjDataRow(IIf(Me.mLangauge = Language.English, "GradeEngName", "GradeArbName")), SqlDbType.VarChar) _
                '& " ~ " & _
                'mDataHandler.DataValue(ObjDataRow(IIf(Me.mLangauge = Language.English, "EngName", "ArbName")), SqlDbType.VarChar)

                Item.Text = _
                mDataHandler.DataValue(ObjDataRow(ObjNavigationHandler.SetLanguage(mPage, "GradeEngName/GradeArbName")), SqlDbType.VarChar) _
                & " ~ " & _
...
#Region "Class Private Function"
    ...
    Private Function GetParameter(ByVal Ds As DataSet) As Boolean
        Try
            With Ds.Tables(0).Rows(0)
                ...
                mStep = mDataHandler.DataValue_Out(.Item("Step"), SqlDbType.Int)
    ...
    Private Function SetParameter(ByRef Sqlcommand As SqlClient.SqlCommand) As Boolean
        Try
            ...
            Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@Step", SqlDbType.Int)).Value = mDataHandler.DataValue_In(mStep, SqlDbType.Int)
