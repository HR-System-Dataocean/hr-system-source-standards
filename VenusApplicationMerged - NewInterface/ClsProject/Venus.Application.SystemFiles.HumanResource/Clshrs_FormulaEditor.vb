            ...
            If StrSubLevelFormula = "" Or mLevels <= mCurrentLevel Or StrSubLevelFormula.IndexOf(Statment) <> -1 Then
                If mSalarySrc.Contains("T") And mFiscalPeriodID > 0 Then
                    ...
                    If Not mSalarySrc.Contains("E") And dtChanges.Rows.Count > 0 And FromDate = FiscFromDate And ToDate = FiscToDate And Not mBolBeginOfContract Then
                        ...
                        'Rabie 05-08-2025 Make   Change Date = BAse Date
                        daydiff = (ToDate.Subtract(FromDate).Days + 1)
                        'End Of Modification 05-08-2025
                        CDblValue = CoDblValue / (ToDate.Subtract(FromDate).Days + 1) * daydiff
                        DblValue = DblValue + CDblValue
                    Else
                        StrSqlCommand = " Set Dateformat DMY; Select top(1) IsNull(Amount,0) From hrs_employees Inner Join hrs_contracts	On hrs_employees.id	= hrs_contracts.employeeid Inner Join hrs_contractsTransactions	On hrs_contracts.id	= hrs_contractsTransactions.Contractid  Inner Join hrs_TransactionsTypes On hrs_TransactionsTypes.id = hrs_ContractsTransactions.TransactionTypeid Where hrs_Transactionstypes.Code = '" & StrColumnName & "' And hrs_contracts.id=" & IntContractID & " And hrs_contractsTransactions.CancelDate Is Null "
    ...
    Private Function EvaluateSubField2(ByVal Statment As String) As Double
        ...
        Try
            ...
            Select Case StrColumnName
                ...
                Case "RUPP"
                    ...
                    End If
                Case "AVDPP"
                    'this is to calc AnnualVacationDays =====Rabie
                    If FormulaCalculated <> "N" Then
                        Dim AVDays As Int32 = 0
                        Dim ClsFisicalPeriods As New Clssys_FiscalYearsPeriods(mPage)
                        ClsFisicalPeriods.Find("ID = " & mFiscalPeriodID)

                        '=========Rabie 23-11-2023   to calculate ANVPP from preparDay=======
                        'FiscFromDate = FromDate
                        'FiscToDate = ToDate
                        '===========
                        Dim clsCompanies As New Clssys_Companies(mPage)
                        ...
                        clsCompanies.Find("ID = " & ClsFisicalPeriods.MainCompanyID)
                        If Not clsCompanies.VacationFromPrepareDay Then


                            If FiscToDate.Day = 30 Then
                                CntDays = FiscToDate.Subtract(FiscFromDate).Days + 2
                            End If

                            If FiscToDate.Day = 31 Then
                                CntDays = FiscToDate.Subtract(FiscFromDate).Days + 1
                            End If
                            If FiscToDate.Day = 28 Then
                                CntDays = FiscToDate.Subtract(FiscFromDate).Days + 4
                            End If
                            If FiscToDate.Day = 29 Then
                                CntDays = FiscToDate.Subtract(FiscFromDate).Days + 1
                            End If

                            If FiscToDate.Day <> 33 Then
                        ...
                        Else
                            FiscFromDate = FromDate
                            FiscToDate = ToDate
                            CntDays = FiscToDate.Subtract(FiscFromDate).Days
                            For CounDays As Integer = 0 To CntDays
                                Dim OperDate As DateTime = FiscFromDate.AddDays(CounDays)
                                Dim hrsEmployeesVacations As New Clshrs_EmployeesVacations(mPage)
                                hrsEmployeesVacations.Find("EmployeeID = " & mEmployeeID & " and CONVERT(date,'" & OperDate.ToString("dd/MM/yyyy") & "',103) >= CONVERT(date,ActualStartDate,103) and CONVERT(date,'" & OperDate.ToString("dd/MM/yyyy") & "',103) < CONVERT(date,isnull(ActualEndDate,'01/01/2050'),103) and VacationTypeID in (select ID from hrs_VacationsTypes where IsAnnual = 1)")
                                If (hrsEmployeesVacations.DataSet.Tables(0).Rows.Count > 0) Then
                                    ...
                                    Else
                                        ...
                                        If Not (DateTime.DaysInMonth(FromDate.Year, FromDate.Month) = 31 And OperDate >= ToDate) Then
                                            AVDays = AVDays + 1
                ...
                Case "MaxTIV"
                    If FormulaCalculated <> "N" Then
                        ...
                        If mNoOfWorkingDays > mDurationDays Then
                            Return (OneCost + (OneCost * mCnt)) '* (mNoOfWorkingDays / mDurationDays)
                        End If
                        Return (OneCost + (OneCost * mCnt)) * (mNoOfWorkingDays / mDurationDays)
                ...
                Case "HIC"
                    Try
                        ...
                        Else
                            IntContractID = ClsContracts.ContractValidatoinId(mEmployeeID, Date.Now)
                        End If
                        StrSqlCommand = "Set DateFormat DMY; Select Sum(IsNull(CompanyAmt,0)) From hrs_HIPolicyContract where contractID = " & IntContractID & " and ActiveDate <=  '" & ToDate.ToString("dd/MM/yyyy") & "' and (CancelDate is null)"
                    Catch ex As Exception
                        Return 0
                    End Try
                Case "AVC"
                    Try
                        ...
                        'Edited by: Hassan Kurdi
                        'Date: 2021-11-04
                        'Purose: Calculate Annual Vacation Cost from formula
                        Dim mCost As Double
                        ...
                        Dim ClsSolver = New Clshrs_FormulaSolver(mStrConnectionString, Page)


                        ClsContract.ContractValidatoinId(EmployeeID, FiscalPeriodID)
                        ClsEmployeeClass.Find("Id = " & ClsContract.EmployeeClassID)

                        ClsSolver.EmployeeID = EmployeeID
                        ClsSolver.FiscalPeriodID = FiscalPeriodID
                        ClsSolver.NoOfDaysPerPeriod = ClsEmployeeClass.NoOfDaysPerPeriod
                        ClsSolver.Executedate = ToDate
                        ClsSolver.EvaluateExpression(ClsEmployeeClass.VacCostFormula)

                        ClsEmployees.Find("ID=" & mEmployeeID & "")
                        Dim EmpJoindate As Date
                        EmpJoindate = ClsEmployees.JoinDate
                        '»‰Õ”» ⁄œœ «Ì«„ «·⁄„· „‰  «—ÌŒ «·«· Õ«ﬁ ·€«Ì… «Œ— «·‘Â— «··Ì »‰ÃÂ“Â
                        Dim NoOfTotalWorkingDays = (ToDate - EmpJoindate).TotalDays
                        '»‰ÃÌ» ⁄œœ «Ì«„ «·«Ã«“… «··Ì Ì” ÕﬁÂ« œ·Êﬁ Ì ⁄·Ì Õ”» „œ… «·Œœ„…
        ...
        Try
                ...
                Case "WDPP"
                    ...
                    End If
                Case "ANVPP"
                    'This Is to Calc No Of days For Not Paied Vacations--Rabie
                    Dim AVDays As Int32 = 0
                    Dim ClsFisicalPeriods As New Clssys_FiscalYearsPeriods(mPage)
                    ClsFisicalPeriods.Find("ID = " & mFiscalPeriodID)
                    '=========Rabie 23-11-2023   to calculate ANVPP from preparDay=======
                    ...
                    If Not clsCompanies.VacationFromPrepareDay Then
                        ...
                        For CounDays As Integer = 0 To CntDays
                            Dim OperDate As DateTime = FiscFromDate.AddDays(CounDays)
                            Dim hrsEmployeesVacations As New Clshrs_EmployeesVacations(mPage)
                            hrsEmployeesVacations.Find("EmployeeID = " & mEmployeeID & " AND CONVERT(date,'" & OperDate.ToString("dd/MM/yyyy") & "',103) >= CONVERT(date,ActualStartDate,103) AND CONVERT(date,'" & OperDate.ToString("dd/MM/yyyy") & "',103) < CONVERT(date,ISNULL(ActualEndDate,'01/01/2050'),103) AND VacationTypeID IN (SELECT ID FROM hrs_VacationsTypes WHERE IsPaid = -1)")
                            If (hrsEmployeesVacations.DataSet.Tables(0).Rows.Count > 0) Then
                                If OperDate > FiscToDate Then
                                    IsToEnd = True
                                    Continue For
                ...
                Case "SVPP"
                    ...
                    For CounDays As Integer = 0 To CntDays
                        Dim OperDate As DateTime = FiscFromDate.AddDays(CounDays)
                        Dim hrsEmployeesVacations As New Clshrs_EmployeesVacations(mPage)
                        hrsEmployeesVacations.Find("EmployeeID = " & mEmployeeID & " AND CONVERT(date,'" & OperDate.ToString("dd/MM/yyyy") & "',103) >= CONVERT(date,ActualStartDate,103) AND CONVERT(date,'" & OperDate.ToString("dd/MM/yyyy") & "',103) < CONVERT(date,ISNULL(ActualEndDate,'01/01/2050'),103) AND VacationTypeID IN (SELECT ID FROM hrs_VacationsTypes WHERE IsSickVacation  = 1)")
                        If (hrsEmployeesVacations.DataSet.Tables(0).Rows.Count > 0) Then
                            If OperDate > FiscToDate Then
                                IsToEnd = True
                                Continue For
