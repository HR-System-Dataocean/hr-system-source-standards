...
'Description    : 1-Implement class of Data Acess Layer of evaluation main types (hrs_EvaluationsMainElements table)
'                   ,Every type has percentage 
'                     from total evaluation types that used in total evaluation of employee in final 
'                     total evaluation 
'                    2-Each main type has  code,English name,Arabic name,Search Arab Name,Percent from total elements and Remarks
'                    3-Allow search in Evaluation main types using critra
'                    4-Get list with all codes of Evaluation main types records
...
'                       records
'Modification   : Convert data type of PercentFromTotalElement (field in hrs_EvaluationsMainElements) from mony to real 
'
'==========================================================================
Imports Venus.Application.SystemFiles.System
Public Class Clshrs_EvaluationsMainTypesBases
...
#Region "Class Constructors"
    ...
    Public Sub New(ByVal Page As Web.UI.Page)
        ...
        mInsertParameter = " Code,EngName,ArbName,ArbName4S,PercentFromTotalElements,EvaluationTypeID,Remarks,RegUserID,RegComputerID "
        mInsertParameterValues = " @Code,@EngName,@ArbName,@ArbName4S,@PercentFromTotalElements,@EvaluationTypeID,@Remarks,@RegUserID,@RegComputerID"
        mUpdateParameter = " Code=@Code,EngName=@EngName,ArbName=@ArbName,ArbName4S=@ArbName4S,PercentFromTotalElements=@PercentFromTotalElements,EvaluationTypeID=@EvaluationTypeID,Remarks=@Remarks,RegUserID=@RegUserID,RegComputerID=@RegComputerID "

        ' mSelectCommand = " Select * From  " & mTable
        mInsertCommand = " insert into " & mTable & "( " & mInsertParameter & ")Values(" & mInsertParameterValues & ")"
        mUpdateCommand = " Update " & mTable & " Set " & mUpdateParameter
        mUpdateCommandPercent = " Update " & mTable & " Set " & mUpdateParameter
        mDeleteCommand = " Update " & mTable & " Set CancelDate=GetDate()"

        mSelectCommand = "Select hrs_EvaluationsMainElements.ID,hrs_EvaluationsMainElements.Code,hrs_EvaluationsMainElements.EngName,hrs_EvaluationsMainElements.ArbName,hrs_EvaluationsMainElements.ArbName4S,hrs_EvaluationsMainElements.PercentFromTotalElements,hrs_EvaluationsMainElements.EvaluationTypeID,hrs_EvaluationsMainElements.Remarks,hrs_EvaluationsMainElements.RegUserID,hrs_EvaluationsMainElements.RegComputerID,hrs_EvaluationsMainElements.RegDate,hrs_EvaluationsMainElements.CancelDate From " & Me.mTable & " INNER JOIN hrs_EvaluationsTypes on hrs_EvaluationsMainElements.EvaluationTypeID=hrs_EvaluationsTypes.ID "
...
#Region "Private Members"
    ...
    Private mUpdateCommandPercent As String
    Private mUpdateCommandParameter As String
#End Region
...
#Region "Public Function"
    ...
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  Find all rows in hrs_EvaluationsMainElements that match criteria or filter 
    '                   and canceldate = null
    '               Steps:
    ...
    'Date Created   :  [22-07-2007]
    'Modifications  : 
    '               : [0256] 240707 E# 00001 Remove Select String From Find() Method ad make the selction strig be at class constructor procedure 
    '               : [0256] 270707 E# Add Filter Syntax 
    'Calls          :
    'From           : (frmEvaluationMainElements.aspx.vb) TlbMainNavigation_ButtonClicked() 
    '               : (frmEvaluationMainElements.aspx.vb) SetToolbarSetting()
    '               : (frmEvaluationMainElements.aspx.vb) Page_Load()
    '               : (frmEvaluationMainElements.aspx.vb) TlbMainToolbar_ButtonClicked()
    '               : (frmEvaluationMainElements.aspx.vb) btnRefCode_Click()
    '               : (frmEvaluationMainElements.aspx.vb) SavePart()
    '               : (frmEvaluationMainElements.aspx.vb) CheckMode()
    'To             : GetParameter()
    'fn. Arguments  :
    '---------------------------------------------------------
    ...
    Public Function Find(ByVal Filter As String) As Boolean
        ...
        Try
            ...
            StrSelectCommand = mSelectCommand & IIf(Len(Filter) > 0, " Where IsNull(hrs_EvaluationsMainElements.CancelDate,'')='' And IsNull(dbo.hrs_GetRecordViewStatus(hrs_EvaluationsMainElements.ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 And " & Filter, "  Where IsNull(hrs_EvaluationsMainElements.CancelDate,'')='' And IsNull(dbo.hrs_GetRecordViewStatus(hrs_EvaluationsMainElements.ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 ")
    ...
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  Save Or Update Row that match with critera in hrs_EvaluationsMainElements table 
    ...
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  Save new row into hrs_EvaluationsMainElements table
    '                  and return true if operation done otherwise report errors in ErrorPage
    'Developer      :  [MAE] 
    'Date Created   :  [22-07-2007]
    'Modifacations  : 
    'Calls          :
    'From           : (frmEvaluationMainElements.aspx.vb)SavePart()
    '               :
    'To             : SetParameter()
    'fn. Arguments  :
    '---------------------------------------------------------
    ...
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  update row that match with critera into hrs_EvaluationsMainElements table
    '                  and return true if operation done otherwise report errors in ErrorPage
    'Developer      :  [MAE] 
    'Date Created   :  [22-07-2007]
    'Modifacations  : 
    'Calls          :
    'From           : (frmEvaluationMainTypes.aspx.vb)SavePart() 
    'To             : SetParameter()
    'fn. Arguments  :
    '---------------------------------------------------------
    'Parmeter Name      : Data Type : Description
    ...
    '========================================================================
    'ProcedureName  :  UpdatePercent
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  update percent field into hrs_EvaluationsMainElements table
    '                  and return true if operation done otherwise report errors in ErrorPage
    'Developer      :  [MAE] 
    'Date Created   :  [19-07-2007]
    'Modifacations  : 
    'Calls          :
    'From           : (frmEvaluationMainTypes.aspx.vb)AssignValues() 
    'To             : SetParameter()
    'fn. Arguments  :
    '---------------------------------------------------------
    'Parmeter Name      : Data Type : Description
    ...
    'Filter             :String     :used as critera in select statment like 'ID=2'
    '========================================================================
    Public Function UpdatePercent(ByVal Filter As String) As Boolean
        Dim StrUpdateCommand As String
        Try
            StrUpdateCommand = " Update " & mTable & " Set " & " PercentFromTotalElements=" & mPercentFromTotalElements.ToString() & IIf(Len(Filter) > 0, " Where " & Filter, "")
            mSqlCommand = New SqlClient.SqlCommand
            mSqlCommand.Connection = New SqlClient.SqlConnection(mConnectionString)
            mSqlCommand.CommandType = CommandType.Text
            mSqlCommand.CommandText = StrUpdateCommand
    ...
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  Delete new row that match critera into hrs_EvaluationsMainElements table
    '                  and return true if operation done otherwise report errors in ErrorPage                    
    'Developer      :  [MAE] 
    'Date Created   :  [16-07-2007]
    'Modifacations  : 
    'Calls          :
    'From           : (frmEvaluationMainElements.aspx.vb) TlbMainToolbar_ButtonClicked() 
    'To             : SetParameter()
    'fn. Arguments  :
    '---------------------------------------------------------
    'Parmeter Name      : Data Type : Description
    ...
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  Get first record in hrs_EvaluationsMainElements table
    '                  and return true if operation done otherwise report errors in ErrorPage
    'Developer      :  [MAE] 
    'Date Created   :  [16-07-2007]
    'Modifacations  : 
    ...
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  Get last record in hrs_EvaluationsMainElements table
    '                  and return true if operation done otherwise report errors in ErrorPage
    'Developer      :  [MAE] 
    'Date Created   :  [16-07-2007]
    'Modifacations  : 
    ...
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  Get next record  from hrs_EvaluationsMainElements table
    '                  and return true if operation done otherwise report errors in ErrorPage
    'Developer      :  [MAE] 
    'Date Created   :  [16-07-2007]
    'Modifacations  : 
    ...
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  Get previous record from hrs_EvaluationsMainElements table
    '                  and return true if operation done otherwise report errors in ErrorPage
    'Developer      :  [MAE] 
    'Date Created   :  [16-07-2007]
    'Modifacations  : 
    ...
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  1-Get sum of all evaluation types percentages from hrs_EvaluationsMainElements table
    ...
    'Modifacations  : 
    'Calls          :
    'From           : (frmEvaluationMainElements.aspx.vb) fnGetAvailablePercent() 
    '               : 
    'fn. Arguments  :
    '---------------------------------------------------------
    'Parmeter Name      : Data Type : Description
...
#Region "Class Private Function"
    ...
    Protected Function GetParameter(ByVal Ds As DataSet) As Boolean
        Try
            With Ds.Tables(0).Rows(0)
                ...
                mEvaluationTypeID = mDataHandler.DataValue_Out(.Item("EvaluationTypeID"), SqlDbType.Int)
    ...
    Private Function SetParameter(ByRef Sqlcommand As SqlClient.SqlCommand) As Boolean
        Try
            ...
            Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@EvaluationTypeID", SqlDbType.Int)).Value = mDataHandler.DataValue_In(mEvaluationTypeID, SqlDbType.Int)
...
Public Class Clshrs_EvaluationsMainElements
    Inherits Clshrs_EvaluationsMainTypesBases
    ...
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  Fill DropDownList with English name column from hrs_EvaluationsMainElements table
    '                  and return true if operation done otherwise report errors in ErrorPage
    'Developer      :  [MAE]  
    'Date Created   :  [16-07-2007]
    'Modifacations  : 
    ...
    ''========================================================================
    ''ProcedureName  :  GetListWithFilter
    ''Module         :  Hrs (Human Resource Module)
    ''Project        :  Venus V.
    ''Description    :  Fill DropDownList with English name column from hrs_EvaluationsMainElements table
    ''                  records that match critera
    ''                  and return true if operation done otherwise report errors in ErrorPage
    ''Developer      :  [MAE] 
    ...
    ''---------------------------------------------------------
    ''DdlValues             :DropDownList     :used to fill it with English name column from hrs_TimeIntervals table
    ''Filter                :String           :used as critera to filter selection 
    ''========================================================================
    'Public Function GetListWithFilterDG(ByRef DdlValues As Global.System.Web.UI.WebControls.DropDownList, ByVal Filter As String) As Boolean
    '    Dim ObjDataRow As DataRow
    '    Dim StrCommandString As String
    '    Dim ObjDataset As New DataSet
    '    Dim Item As Global.System.Web.UI.WebControls.ListItem
    ...
    '        StrCommandString = "Select hrs_EvaluationsMainElements.ID,hrs_EvaluationsMainElements.EngName From " & Me.mTable & " INNER JOIN hrs_EvaluationsTypes on hrs_EvaluationsMainElements.EvaluationTypeID=hrs_EvaluationsTypes.ID " & IIf(Len(Filter) > 0, " Where IsNull(hrs_EvaluationsMainElements.CancelDate,'')='' And IsNull(dbo.hrs_GetRecordViewStatus(hrs_EvaluationsMainElements.ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 And " & Filter, "  Where IsNull(hrs_EvaluationsMainElements.CancelDate,'')='' And IsNull(dbo.hrs_GetRecordViewStatus(hrs_EvaluationsMainElements.ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 ")
    ...
    Public Function GetList(ByRef DdlValues As Infragistics.WebUI.UltraWebGrid.ValueList, ByVal Filter As String) As Boolean
        Dim ObjDataRow As DataRow
        Dim StrCommandString As String
        Dim ObjDataset As New DataSet
        Dim Item As Infragistics.WebUI.UltraWebGrid.ValueListItem
    ...
    '========================================================================
    'ProcedureName  :  GetAllEvaluations
    'Module         :  Hrs (Human Resource Module)
    'Project        :  Venus V.
    'Description    :  1-Fill Datagrid with ID,EngName,PercentFromTotalElements and PercentFromTotalElements 
    '
    'Developer      :  [MAE] 
    'Date Created   :  [16-07-2007]
    'Modifacations  : 
    ...
    'From           : (frmEvaluationMainElements.aspx.vb) TlbMainToolbar_ButtonClicked() 
    '               : (frmEvaluationMainElements.aspx.vb) SavePart() 
    '               : (frmEvaluationMainElements.aspx.vb) Load()
    'To             : Clear()
    '               : 
    'fn. Arguments  :
    '---------------------------------------------------------
    'Parmeter Name      : Data Type : Description
    '---------------------------------------------------------
    'DdlValues             :DropDownList     :used to fill it with English name column from 
    '                                         frmEvaluationMainElements table
    '========================================================================
    Public Function GetAllEvaluations(ByRef DG As Infragistics.WebUI.UltraWebGrid.UltraWebGrid) As Boolean

        Dim StrCommandString As String
        Dim ObjDataset As New DataSet
        Try

            StrCommandString = " Select  ID,EngName,PercentFromTotalElements From  " & Me.mTable & " Where IsNull(CancelDate,'')='' And IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1"
            mSqlDataAdapter = New SqlClient.SqlDataAdapter(StrCommandString, mConnectionString)
            mDataSet = New DataSet
            mSqlDataAdapter.Fill(mDataSet)
            If mDataHandler.CheckValidDataObject(mDataSet) Then
                DG.DataSource = mDataSet
                DG.DataBind()
