...
Public Class Clshrs_LocationGeoPoints
    Inherits ClsDataAcessLayer
#Region "Class Constructors"
    Public Sub New(ByVal Page As Web.UI.Page)
        MyBase.New(Page)
        mTable = " hrs_LocationGeoPoints "
        mInsertParameter = "" &
          "LocationID," &
          "EmployeeID," &
          "LineNum," &
...
#Region "Private Members"
    ...
    Private mLatitude As Decimal
    Private mLongitude As Decimal
    Private mAllowedRadius As Decimal
...
#Region "Public property"
    ...
    Public Property Longitude() As Decimal
        ...
        End Set
    End Property
    Public Property AllowedRadius() As Decimal
        Get
            Return mAllowedRadius
        End Get
        Set(ByVal Value As Decimal)
            mAllowedRadius = Value
        End Set
    End Property
...
#Region "Public Function"
    ...
    '========================================================================
    Public Function Find(ByVal Filter As String) As DataSet
        Dim StrSelectCommand As String
        Try
...
#Region "Class Private Function"
    ...
    Private Function GetParameter(ByVal Ds As DataSet) As Boolean
        Try
            With Ds.Tables(0).Rows(0)
                ...
                mLineNum = [Shared].DataHandler.DataValue_Out(.Item("LineNum"), SqlDbType.Int, True)
                ...
                mLatitude = [Shared].DataHandler.DataValue_Out(.Item("Latitude"), SqlDbType.Decimal)
                mLongitude = [Shared].DataHandler.DataValue_Out(.Item("Longitude"), SqlDbType.Decimal)
                mAllowedRadius = [Shared].DataHandler.DataValue_Out(.Item("AllowedRadius"), SqlDbType.Decimal)
                mAddress = [Shared].DataHandler.DataValue_Out(.Item("Address"), SqlDbType.VarChar)
                mActive = [Shared].DataHandler.DataValue_Out(.Item("Active"), SqlDbType.Bit)
    ...
    Private Function SetParameter(ByRef Sqlcommand As SqlClient.SqlCommand, ByVal strMode As String) As Boolean
        Try
            ...
            Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@EmployeeID", SqlDbType.Int)).Value = [Shared].DataHandler.DataValue_In(mEmployeeID, SqlDbType.Int, True)
            Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@LineNum", SqlDbType.Int)).Value = [Shared].DataHandler.DataValue_In(mLineNum, SqlDbType.Int, True)
            ...
            Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@Latitude", SqlDbType.Decimal)).Value = [Shared].DataHandler.DataValue_In(mLatitude, SqlDbType.Decimal)
            Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@Longitude", SqlDbType.Decimal)).Value = [Shared].DataHandler.DataValue_In(mLongitude, SqlDbType.Decimal)
            Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@AllowedRadius", SqlDbType.Decimal)).Value = [Shared].DataHandler.DataValue_In(mAllowedRadius, SqlDbType.Decimal)
            Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@Address", SqlDbType.VarChar)).Value = [Shared].DataHandler.DataValue_In(mAddress, SqlDbType.VarChar)
            Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@Active", SqlDbType.Bit)).Value = [Shared].DataHandler.DataValue_In(mActive, SqlDbType.Bit)
            If (strMode.Trim.ToUpper = "SAVE") Then
                ...
                Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@RegDate", SqlDbType.DateTime)).Value = [Shared].DataHandler.DataValue_In(mRegDate, SqlDbType.DateTime)
            End If
        Catch ex As Exception
            mPage.Session.Add("ErrorValue", ex)
            mErrorHandler.RecordExceptions_DataBase("", ex, Err.Number, mDataBaseUserID, Venus.Shared.ErrorsHandler.eRecordingType.System_ErrorsLog)
...
#Region "Navigation Functions"
    ...
    Public Function FirstRecord(Optional ByVal Filter As String = "") As Boolean
        Dim StrSelectCommand As String
        Try
            StrSelectCommand = "SELECT TOP 1 * FROM " & mTable & " Where  IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 " & IIf(Filter.Trim = "", " ", " AND " & Filter & " ") & " ORDER BY LocationID ASC"
            mSqlDataAdapter = New SqlClient.SqlDataAdapter(StrSelectCommand, mConnectionString)
            mDataSet = New DataSet
            mSqlDataAdapter.Fill(mDataSet)
    ...
    Public Function LastRecord(Optional ByVal Filter As String = "") As Boolean
        Dim StrSelectCommand As String
        Try
            StrSelectCommand = "SELECT TOP 1 * FROM " & mTable & " Where  IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 " & IIf(Filter.Trim = "", " ", " AND " & Filter & " ") & " ORDER BY LocationID DESC"
            mSqlDataAdapter = New SqlClient.SqlDataAdapter(StrSelectCommand, mConnectionString)
            mDataSet = New DataSet
            mSqlDataAdapter.Fill(mDataSet)
            If mDataHandler.CheckValidDataObject(mDataSet) Then
    ...
    Public Function NextRecord(Optional ByVal Filter As String = "") As Boolean
        Dim StrSelectCommand As String
        Try
            StrSelectCommand = "SELECT TOP 1 * FROM " & mTable & " WHERE LocationID >'" & mLocationID & "' And  IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 " & IIf(Filter.Trim = "", " ", " AND " & Filter & " ") & " ORDER BY LocationID ASC"
            mSqlDataAdapter = New SqlClient.SqlDataAdapter(StrSelectCommand, mConnectionString)
            mDataSet = New DataSet
            mSqlDataAdapter.Fill(mDataSet)
            If mDataHandler.CheckValidDataObject(mDataSet) Then
    ...
    Public Function previousRecord(Optional ByVal Filter As String = "") As Boolean
        Dim StrSelectCommand As String
        Try
            StrSelectCommand = "SELECT TOP 1 * FROM " & mTable & " WHERE LocationID <'" & mLocationID & "' And  IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 " & IIf(Filter.Trim = "", " ", " AND " & Filter & " ") & " ORDER BY LocationID DESC"
            mSqlDataAdapter = New SqlClient.SqlDataAdapter(StrSelectCommand, mConnectionString)
            mDataSet = New DataSet
            mSqlDataAdapter.Fill(mDataSet)
            If mDataHandler.CheckValidDataObject(mDataSet) Then
