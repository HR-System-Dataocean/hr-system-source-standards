...
'Developer         : [0257]
'Description       : 1-Implement Data Acess Layer of hrs_GradesVactions table with fields Grade ID,Vcation Type ID,
'                       Duration Days and RequiredWorkingMonths
'                    2-Allow searching
...
Public Class Clshrs_GradesStepsTransactions
...
#Region "Class Constructors"
    ...
    'Date Created   :09-08-2007
    'Modifications  :
    '               : [0256] 04-09-2007 Add TransactionTypeID to select statement 
    'fn. Arguments  :
    '---------------------------------------------------------
    ...
    Public Sub New(ByVal Page As Web.UI.Page)
        MyBase.New(Page)
        mTable = " hrs_GradesStepsTransactions "
        mInsertParameter = " GradeStepID,GradeTransactionID,Amount,Active,Remarks,RegUserID,RegComputerID "
        mInsertParameterValues = " @GradeStepID,@GradeTransactionID,@Amount,@Active,@Remarks,@RegUserID,@RegComputerID "
        mUpdateParameter = " GradeStepID=@GradeStepID,GradeTransactionID=@GradeTransactionID,Amount=@Amount,Active=@Active,Remarks=@Remarks,RegUserID=@RegUserID,RegComputerID=@RegComputerID "
        mSelectCommand = " Select hrs_GradesStepsTransactions.ID,hrs_GradesStepsTransactions.GradeStepID,hrs_GradesStepsTransactions.GradeTransactionID,hrs_GradesStepsTransactions.Amount,hrs_GradesStepsTransactions.Active,hrs_GradesStepsTransactions.Remarks,hrs_GradesStepsTransactions.RegUserID,hrs_GradesStepsTransactions.RegComputerID,hrs_GradesStepsTransactions.RegDate,hrs_GradesStepsTransactions.CancelDate ,hrs_GradesTransactions.TransactionTypeID,hrs_GradesTransactions.PaidAtVacation,hrs_GradesTransactions.OnceatPeriod From  " & mTable & "INNER JOIN hrs_GradesSteps ON hrs_GradesStepsTransactions.GradeStepID=hrs_GradesSteps.ID INNER JOIN hrs_GradesTransactions ON hrs_GradesStepsTransactions.GradeTransactionID=hrs_GradesTransactions.ID INNER JOIN hrs_Grades ON  hrs_GradesTransactions.GradeID=hrs_Grades.ID AND hrs_GradesSteps.GradeID=hrs_Grades.ID "
...
#Region "Private Members"
    ...
    Private mGradeStepID As Integer
    Private mGradeTransactionID As Integer
...
#Region "Public property"
    Public Property ID() As Object
        ...
        End Set
    End Property
    Public Property GradeStepID() As Integer
        Get
            Return mGradeStepID
        End Get
        ...
        End Set
    End Property
    Public Property GradeTransactionID() As Integer
        Get
            Return mGradeTransactionID
        End Get
        Set(ByVal Value As Integer)
            mGradeTransactionID = Value
        End Set
    End Property
...
#Region "Public Function"
    ...
    'Modifacations  :
    'Calls          :
    'From           :(frmGrades.aspx.vb)GetValues()
    'To             :GetParameter()
    'fn. Arguments  :
    '---------------------------------------------------------
    'Parmeter Name      : Data Type : Description
    ...
    Public Function Find(ByVal Filter As String) As Boolean
        ...
        Try
            ...
            StrSelectCommand = mSelectCommand & IIf(Len(Filter) > 0, " Where IsNull(dbo.hrs_GetRecordViewStatus(hrs_GradesStepsTransactions.ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 And " & Filter, "  And IsNull(dbo.hrs_GetRecordViewStatus(hrs_GradesStepsTransactions.ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 ")
    ...
    Public Function SaveUpdate(ByVal Filter As String) As Boolean
        ...
        Try
            strSQL = "Select ID From hrs_GradesStepsTransactions Where " & Filter
    ...
    Public Function GetGradesStepsTransactions(ByVal IntGradesStepID As Integer, ByVal IntLanguage As Integer, ByRef DsTransactions As DataSet) As Boolean
        Try
            DsTransactions = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(mConnectionString, "GetDefaultGradesStepsTransactions", IntGradesStepID, IntLanguage)
            Return True
        Catch ex As Exception
            mPage.Session.Add("ErrorValue", ex)
            mErrorHandler.RecordExceptions_DataBase("", ex, Err.Number, mDataBaseUserID, Venus.Shared.ErrorsHandler.eRecordingType.System_DataBase)
        ...
        End Try
    End Function
    Public Function GetLastIDFromContractTransaction() As Integer
        Dim StrSelectCommand As String
        Dim retInt As Object
        Try
            StrSelectCommand = " Select max(ID) From hrs_ContractsTransactions "
            retInt = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteScalar(mConnectionString, CommandType.Text, StrSelectCommand)
            If Not IsDBNull(retInt) Then
                Return retInt
            Else
                Return 0
            End If
...
#Region "Class Private Function"
    ...
    Private Function GetParameter(ByVal Ds As DataSet) As Boolean
        Try
            With Ds.Tables(0).Rows(0)
                ...
                mGradeTransactionID = mDataHandler.DataValue_Out(.Item("GradeTransactionID"), SqlDbType.Int, True)
    ...
    Private Function SetParameter(ByRef Sqlcommand As SqlClient.SqlCommand) As Boolean
        Try
            ...
            Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@GradeTransactionID", SqlDbType.Int)).Value = mDataHandler.DataValue_In(mGradeTransactionID, SqlDbType.Int, True)
