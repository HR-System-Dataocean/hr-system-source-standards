...
Public Class Clshrs_VactionTypeCalculation
    Inherits ClsDataAcessLayer
#Region "Class Constructors"
    Public Sub New(ByVal Page As Web.UI.Page)
        MyBase.New(Page)
        mTable = " hrs_VactionTypeCalculation "
        mInsertParameter = "" &
                                                                                "Code," &
                                                                                "EngName," &
                                                                                "ArbName," &
...
#Region "Public Function"
    ...
    Public Function FindVactionCalculation() As Boolean
        Dim StrSelectCommand As String
        Try
            StrSelectCommand = "select * from hrs_VactionTypeCalculation where id in(select VactionTypeCaculation from hrs_VacationsTypes where IsAnnual=1)"
            'StrSelectCommand = mSelectCommand & IIf(Len(Filter) > 0, " Where IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 And " & Filter(), " And IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 ")
            mSqlDataAdapter = New SqlClient.SqlDataAdapter(StrSelectCommand, mConnectionString)
            mDataSet = New DataSet
            mSqlDataAdapter.Fill(mDataSet)
    ...
    'Developer      :  DataOcean   
    'Date Created   :06/10/2018 9:48:52 PM
    'Modifacations  :
    'fn. Arguments  :
    '---------------------------------------------------------
    'Parmeter Name      : Data Type : Description
