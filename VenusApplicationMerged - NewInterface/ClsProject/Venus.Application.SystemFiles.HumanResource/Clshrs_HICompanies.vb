...
'Developer         : [0257]
'Description       : 1-Implement Data Acess Layer of hrs_HICompanies table with fields code,English name,
'                       Arabic name
'                    2-Allow searching
...
Public Class Clshrs_HICompanies
...
#Region "Class Constructors"
    ...
    Public Sub New(ByVal Page As Web.UI.Page)
        MyBase.New(Page)
        mTable = " hrs_HICompanies "
        mInsertParameter = " Code,EngName,ArbName,ArbName4S,Remarks,CompanyID,RegUserID,RegComputerID "
        mInsertParameterValues = " @Code,@EngName,@ArbName,@SearchArbName,@Remarks,@CompanyID,@RegUserID,@RegComputerID "
        mUpdateParameter = " Code=@Code,EngName=@EngName,ArbName=@ArbName,ArbName4S=@SearchArbName,Remarks=@Remarks "
...
#Region "Public Function"
    ...
    Public Function GetList(ByVal DdlValues As Infragistics.WebUI.UltraWebGrid.ValueList, ByVal cTableName As String, ByVal Filter As String) As Boolean
        Dim ObjDataRow As DataRow
        Dim StrCommandString As String
        Dim ObjDataset As New DataSet
        Dim Item As Infragistics.WebUI.UltraWebGrid.ValueListItem
        Dim ObjNavigationHandler As New Venus.Shared.Web.NavigationHandler(mConnectionString)
        Try
            Dim str As String = " IsNull(CancelDate,'')='' And IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & cTableName & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 "

            StrCommandString = "Select * From " & cTableName & " Where " & IIf(Filter.Length > 0, Filter & " And " & str, str)
    ...
    Public Function SaveUpdate(ByVal Filter As String) As Boolean
        ...
        Try
            strSQL = "Select ID From hrs_HICompanies Where " & Filter
    ...
    Public Function Get_HICompaniesClass(ByVal HICompaniesID As Integer, ByRef DsHICompaniesClass As DataSet) As Boolean
        Try
            DsHICompaniesClass = New DataSet
            DsHICompaniesClass = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(ConnectionString, CommandType.Text, "Select ID,HICompanyID,EngName,ArbName,CompanyAmount,EmployeeAmount From hrs_HICompanyClasses Where HICompanyID=" & HICompaniesID)

            If DsHICompaniesClass.Tables(0).Rows.Count > 0 Then
                Return True
            End If
        ...
        End Try
    End Function
    Public Function Set_HICompaniesClass(ByVal HICompaniesID As Integer, ByRef DsHICompaniesClass As DataSet) As Boolean
        Try
