...
Public Class Clshrs_ActingPositionAssignments
    Inherits ClsDataAcessLayer

    Public Sub New(ByVal Page As Web.UI.Page)
        MyBase.New(Page)
        mTable = " hrs_ActingPositionAssignments "
        mSelectCommand = CONFIG_DATEFORMAT & " SELECT * FROM " & mTable
        mInsertCommand = CONFIG_DATEFORMAT & " INSERT INTO " & mTable &
            " (Code, OriginalPositionID, ActingEmployeeID, EffectiveFrom, EffectiveTo, Reason, Remarks, SourceForm, SourceID, RegUserID, RegDate)" &
            " VALUES (@Code,@OriginalPositionID,@ActingEmployeeID,@EffectiveFrom,@EffectiveTo,@Reason,@Remarks,@SourceForm,@SourceID,@RegUserID,GETDATE())"
        mUpdateCommand = CONFIG_DATEFORMAT & " UPDATE " & mTable &
            " SET OriginalPositionID=@OriginalPositionID, ActingEmployeeID=@ActingEmployeeID," &
    ...
    Public Property OriginalPositionID As Integer
    ...
    Public Function FirstRecord() As Boolean
        Return Find("ID=(SELECT MIN(ID) FROM hrs_ActingPositionAssignments)")
    End Function

    Public Function LastRecord() As Boolean
        Return Find("ID=(SELECT MAX(ID) FROM hrs_ActingPositionAssignments)")
    End Function

    Public Function NextRecord() As Boolean
        Return Find("ID=(SELECT MIN(ID) FROM hrs_ActingPositionAssignments WHERE ID>" & ID & ")")
    End Function

    Public Function PreviousRecord() As Boolean
        Return Find("ID=(SELECT MAX(ID) FROM hrs_ActingPositionAssignments WHERE ID<" & ID & ")")
    End Function

    Public Function Clear() As Boolean
        ID = 0
        Code = ""
        OriginalPositionID = 0
        ActingEmployeeID = 0
    ...
    Private Sub SetParameters(ByVal command As SqlClient.SqlCommand, ByVal includeCode As Boolean)
        ...
        command.Parameters.Add("@OriginalPositionID", SqlDbType.Int).Value = OriginalPositionID
    ...
    Private Sub LoadRow(ByVal row As DataRow)
        ...
        OriginalPositionID = Convert.ToInt32(row("OriginalPositionID"))
