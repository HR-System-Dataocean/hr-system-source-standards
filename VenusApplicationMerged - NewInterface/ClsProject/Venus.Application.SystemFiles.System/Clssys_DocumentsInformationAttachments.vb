Public Class Clssys_DocumentsInformationAttachmentsBases
...
#Region "Class Constructors"
    Public Sub New(ByVal Page As Web.UI.Page)
        ...
        mInsertParameter = " ObjectID,RecordId,EngName,ArbName,ArbName4S,FolderName,FileName,ExpiryDate,Remarks,RegUserID,RegComputerID,CombID "
        mInsertParameterValues = " @ObjectID,@RecordId,@EngName,@ArbName,@ArbName4S,@FolderName,@FileName,@ExpiryDate,@Remarks,@RegUserID,@RegComputerID,@CombID "
        mUpdateParameter = " ObjectID=@ObjectID,RecordId=@RecordId,EngName=@EngName,ArbName=@ArbName,ArbName4S=@ArbName4S,FolderName=@FolderName,FileName=@FileName,ExpiryDate=@ExpiryDate,Remarks=@Remarks,CombID=@CombID "
...
#Region "Public Function"
    ...
    Public Function SaveUpdate(ByVal Filter As String) As Boolean
        ...
        Try
            strSQL = "Select ID From sys_DocumentsInformationAttachments Where " & Filter
...
#Region "Class Private Function"
    Private Function GetParameter(ByVal Ds As DataSet) As Boolean
        ...
        Try
            With Ds.Tables(0).Rows(0)
                ...
                mObjectID = mDataHandler.DataValue_Out(.Item("ObjectID"), SqlDbType.VarChar)
                ...
                If clsCompanies.IsHigry Then
                    mExpiryDate = mDataHandler.DataValue_Out(ClsGHCalender.GetRelativeDate(.Item("ExpiryDate"), Clssys_GHCalendar.DateType.Hijri, Clssys_GHCalendar.Directions.Output), SqlDbType.DateTime)
                Else
                    mExpiryDate = mDataHandler.DataValue_Out(ClsGHCalender.GetRelativeDate(.Item("ExpiryDate"), Clssys_GHCalendar.DateType.Gregorian, Clssys_GHCalendar.Directions.Output), SqlDbType.DateTime)
                End If
    ...
    Protected Function SetParameter(ByRef Sqlcommand As SqlClient.SqlCommand) As Boolean
        Dim clsCompanies As New Clssys_Companies(mPage)
        clsCompanies.Find("ID = " & Me.MainCompanyID)
        Dim ClsGHCalender As New Clssys_GHCalendar(mPage)
        Try
            ...
            If clsCompanies.IsHigry Then
                Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@ExpiryDate", SqlDbType.DateTime)).Value = mDataHandler.DataValue_In(ClsGHCalender.GetRelativeDate(mExpiryDate, Clssys_GHCalendar.DateType.Hijri, Clssys_GHCalendar.Directions.Input), SqlDbType.DateTime)
            Else
                Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@ExpiryDate", SqlDbType.DateTime)).Value = mDataHandler.DataValue_In(ClsGHCalender.GetRelativeDate(mExpiryDate, Clssys_GHCalendar.DateType.Gregorian, Clssys_GHCalendar.Directions.Input), SqlDbType.DateTime)
            End If
...
Public Class Clssys_DocumentsInformationAttachments
    Inherits Clssys_DocumentsInformationAttachmentsBases
    ...
    Public Function UpdateCanceled(ByVal Filter As String) As Boolean
        Dim StrUpdateCommand As String
        Dim CLsWebHandlar As New Venus.Shared.Web.WebHandler()
        Try
            StrUpdateCommand = mUpdateCommand & ", CancelDate = Null " & IIf(Len(Filter) > 0, " Where " & Filter, "")
            mSqlCommand = New SqlClient.SqlCommand
            mSqlCommand.Connection = New SqlClient.SqlConnection(mConnectionString)
            mSqlCommand.CommandType = CommandType.Text
            mSqlCommand.CommandText = StrUpdateCommand
