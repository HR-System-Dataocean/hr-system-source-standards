...
Public Class ClsDataAcessLayer
...
#End Region
    ...
    Public Sub AddOnChangeEventToControls(ByVal formName As String, ByRef currPage As Global.System.Web.UI.Page, ByRef TabCtrl As Infragistics.WebUI.UltraWebTab.UltraWebTab, Optional ByVal frmInterface As String = "")
        ...
        If clsForms.ID > 0 Then
            ...
            For Each row As Data.DataRow In tab.Rows
                ...
                ElseIf TypeOf (currCtrl) Is Infragistics.WebUI.WebDataInput.WebMaskEdit Then
                    CType(currCtrl, Infragistics.WebUI.WebDataInput.WebMaskEdit).ClientSideEvents.TextChanged = "DataChanged"
                    If CType(currCtrl, Infragistics.WebUI.WebDataInput.WebMaskEdit).InputMask = "##/##/####" And _
                       frmInterface <> "" Then
                        CType(currCtrl, Infragistics.WebUI.WebDataInput.WebMaskEdit).ClientSideEvents.ValueChange = _
                            ...
                            If StrReturnDate Is Nothing Then
                                ...
                                Catch ex As SqlClient.SqlException
                                    If ex.Number = 2601 Then
                                        Venus.Shared.Web.ClientSideActions.MsgBoxBasic(mPage, nav.SetLanguage(mPage, "Found invalid saved Dates/ÌÊÃœ  Ê«—ÌŒ „Õ›ÊŸ… Œÿ√"))
                                        Return Nothing
                                    End If
    ...
    'Created by : Moussa
    'Date : 06/11/2024
    'Description: Send Email After Operations 1:Save,2:Update,3:DELETE
    '==================================================================
    Public Sub SendEmail(ByVal formName As String, ByRef currPage As Global.System.Web.UI.Page, ByVal Operation As Integer, ByVal formTable As String, ByVal RecordID As Integer)
        Dim setting As New Clssys_SendEmailSetting(currPage)
        Dim filter As String = " FormName='" & formName & "' "

        If Operation = 1 Then
            filter += " and SendAfterSave=1"
        ElseIf Operation = 2 Then
            filter += " and SendAfterUpdate=1"
        ElseIf Operation = 3 Then
            filter += " and SendAfterDelete=1"
        End If

        setting.Find(filter)

        Dim condition As String = setting.ToFormCondition
        Dim body As String = setting.EmailBody

        If setting.ID > 0 Then
            Try
                If condition.Contains("@") Or body.Contains("@") Then
                    'condition = " and " + condition

                    Dim StrCon As String = "Select * from " & formTable & " Where ID= " & RecordID
                    If formTable = "SS_RequestActions" Then
                        StrCon = "Select * from " & formTable & " Where ActionSerial = " & RecordID
                    End If

                    Dim SqlCon As New SqlClient.SqlDataAdapter(StrCon, ConnectionString)
                    Dim dtCon As New DataTable
                    Try
                        SqlCon.Fill(dtCon)
                        For i = 0 To dtCon.Columns.Count - 1
                            condition = condition.Replace("@" & dtCon.Columns(i).ColumnName, "'" & dtCon.Rows(0)(dtCon.Columns(i).ColumnName).ToString() & "'")
                            body = body.Replace("@" & dtCon.Columns(i).ColumnName, dtCon.Rows(0)(dtCon.Columns(i).ColumnName).ToString())
                            'body = body.Replace("@" & dtCon.Columns(i).ColumnName, dtCon.Rows(0)(dtCon.Columns(i).ColumnName).ToString())
                        Next
                    Catch ex As Exception
                        ' Optional: Log error
                ...
                If setting.ToFixedCondition <> "" Then
                    condition += " and " + setting.ToFixedCondition
                End If
                If condition.ToLower().StartsWith("and ") Or condition.ToLower().StartsWith(" and ") Then
                    condition = condition
                Else
                    condition = " and " & condition
                End If
                ...
                If body.Contains("@") Then
                    Dim StrCon As String = "Select * from " & setting.ToTable & " Where 1=1 " & condition
                    ...
                    End Try
                End If
                Dim StrCmd As String = "Select " & setting.ToEmailField & " from " & setting.ToTable & " Where 1=1 " & condition
                Dim SqlDa As New SqlClient.SqlDataAdapter(StrCmd, ConnectionString)
                Dim dt As New DataTable
                Try
                    SqlDa.Fill(dt)
                ...
                Dim StrFixedEmail As String = "Select FixedEmail from sys_SendEmailSettingFixedEmails Where SendEmailSettingId=" & setting.ID
                Dim SqlFixedEmail As New SqlClient.SqlDataAdapter(StrFixedEmail, ConnectionString)
                Dim dtFixedEmail As New DataTable
                Try
                    SqlFixedEmail.Fill(dtFixedEmail)
                Catch ex As Exception
                    ' Optional: Log error
                End Try

                Dim fromEmail = setting.FromEmail
                If String.IsNullOrWhiteSpace(setting.FromEmail) Then
                ...
                ' Add dynamic email addresses
                For index = 0 To dt.Rows.Count - 1
                    If Not String.IsNullOrEmpty(Convert.ToString(dt.Rows(index)(0))) Then
                        mail.To.Add(dt.Rows(index)(0).ToString())
                    End If
                Next

                ' Add fixed emails
                For i = 0 To dtFixedEmail.Rows.Count - 1
                    If Not String.IsNullOrEmpty(Convert.ToString(dtFixedEmail.Rows(i)(0))) Then
                        mail.To.Add(dtFixedEmail.Rows(i)(0).ToString())
                    End If
                Next

                mail.Subject = setting.EmailTitle
                mail.Body = body
