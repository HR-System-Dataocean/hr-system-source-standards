...
'Program name  : Clssys_Searchs.vb 
'Date Created  : DatOcean  
'Date Modified : 16-07-2007  
'Issue #       :       
'Developer     : DataOcean 
'              : [0256]
'Description   : Acts with cls_sysSearcsColumns as Class Tier for frmSearchSetting.aspx.vb and frmSearchCustmaziation.aspx.vb Pages    
'Modifacations: 
'======================================================================
Public Class Clssys_Searchs
...
#Region "Class Constructors"
    Public Sub New(ByVal Page As Web.UI.Page)
        MyBase.New(Page)
        mTable = " sys_Searchs "
        mInsertParameter = " Code,EngName,ArbName,ObjectID,RegUserID "
        mInsertParameterValues = " @Code,@EngName,@ArbName,@ObjectID,@RegUserID "
        mUpdateParameter = " Code=@Code,EngName=@EngName,ArbName=@ArbName,ObjectID=@ObjectID,RegUserID=@RegUserID "

        'mInsertParameter = " Code,EngDescription,ArbDescription,ObjectID,RegUserID "
        'mInsertParameterValues = " @Code,@EngDescription,@ArbDescription,@ObjectID,@RegUserID "
        'mUpdateParameter = " Code=@Code,EngDescription=@EngDescription,ArbDescription=@ArbDescription,ObjectID=@ObjectID,RegUserID=@RegUserID "
...
#Region "Public Function"
    ...
    '               : True if the searched data exisit or false if not 
    'Developer      : [0256]
    'Date Created   : DataOcean 
    'Date Modified  : 16-07-2007 
    'Modifacations  : 
    'Calling       *:*
    'Function Calls : GetParameter() : To create and set values for msqlcommand parameters 
    '               : mDataHandler.CheckValidDataObject(mDataSet) 
    'Called From    : frmSearchSetting.aspx.vb  
    '               :      TbrMainToolbar_ButtonClicked 
    '               :      CheckView 
    '               : frmSearchCustomization.aspx.vb 
    '               :       Page_Load()
    '               :       uwgNavigation_ActiveRowChange
    '               :       TbrMainToolbar_ButtonClicked
    '               :       
    '==================================================================================
    ...
    Public Function GetList(ByRef DdlValues As Infragistics.WebUI.UltraWebGrid.ValueList, ByVal NullNode As Boolean, Optional ByVal Filter As String = "") As Boolean
        ...
        Try
            ...
            'StrCommandString = Me.mSelectCommand & IIf(Len(Filter) > 0, " Where IsNull(CancelDate,'')='' And ID=" & Me.ID & " And IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 And " & Filter, "  Where IsNull(CancelDate,'')='' And Id=" & Me.ID & " And IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 ")
...
#Region "New Functions"
    Public Function fnGetTables() As DataSet
        Try
            mSqlCommand = New SqlClient.SqlCommand
            mSqlCommand.CommandType = CommandType.Text
            mSqlCommand.CommandText = " SELECT ID,code FROM SYS_OBJECTS where canceldate is null order by code "
            mSqlCommand.Connection = New SqlClient.SqlConnection(mConnectionString)
            mSqlDataAdapter = New SqlClient.SqlDataAdapter(mSqlCommand)
            mDataSet = New DataSet
            mSqlDataAdapter.Fill(mDataSet)
            'GetParameter(mDataSet)
        Catch ex As Exception
