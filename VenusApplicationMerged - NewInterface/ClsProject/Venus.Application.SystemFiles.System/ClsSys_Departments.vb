...
Public Class Clssys_DepartmentsBase
...
#Region "Class Constructors"
    ...
    Public Sub New(ByVal Page As Web.UI.Page)
        MyBase.New(Page)
        mTable = " sys_Departments "
        mInsertParameter = " Code,EngName,ArbName,ArbName4S,ParentID,CostCenterCode,Remarks,CompanyID,RegUserID,RegComputerID "
        mInsertParameterValues = " @Code,@EngName,@ArbName,@ArbName4S,@ParentID,@CostCenterCode,@Remarks,@CompanyID,@RegUserID,@RegComputerID "
        mUpdateParameter = " Code=@Code,EngName=@EngName,ArbName=@ArbName,ArbName4S=@ArbName4S,ParentID=@ParentID,CostCenterCode=@CostCenterCode,Remarks=@Remarks "
...
#Region "Private Members"
    ...
    Private mCostCenterCode As String
...
#Region "Public Function"
    ...
    Public Function Find(ByVal Filter As String) As Boolean
        ...
        Try
            ...
            StrSelectCommand = mSelectCommand & IIf(Len(Filter) > 0, " Where IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 And " & Filter, "  Where IsNull(dbo.hrs_GetRecordViewStatus(ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 And CancelDate = null")
            '==================== Order By Modification [Start]
...
#Region "Class Private Function"
    ...
    Private Function GetParameter(ByVal Ds As DataSet) As Boolean
        Try
            With Ds.Tables(0).Rows(0)
                ...
                mCostCenterCode = mDataHandler.DataValue_Out(.Item("CostCenterCode"), SqlDbType.VarChar)
    ...
    Private Function SetParameter(ByRef Sqlcommand As SqlClient.SqlCommand, ByVal operationtype As OperationType) As Boolean
        Try
            ...
            Sqlcommand.Parameters.Add(New SqlClient.SqlParameter("@CostCenterCode", SqlDbType.VarChar)).Value = mDataHandler.DataValue_In(mCostCenterCode, SqlDbType.VarChar)
...
Public Class Clssys_Departments
    Inherits Clssys_DepartmentsBase
    ...
    Public Function GetDropDownListWithBranches(ByVal DdlValues As Global.System.Web.UI.WebControls.DropDownList, ByVal NullNode As Boolean, Optional ByVal Filter As String = "") As Boolean
        Dim ObjDataRow As DataRow
        Dim StrSelectCommand As String
        Dim ObjDataset As New DataSet
        Dim Item As Global.System.Web.UI.WebControls.ListItem
        Dim ObjNavigationHandler As New Venus.Shared.Web.NavigationHandler(mConnectionString)
        Try
            mSelectCommand = "SELECT sys_Departments.* FROM sys_Departments INNER JOIN sys_DepartmentsBranches ON sys_Departments.ID = sys_DepartmentsBranches.DepartmentID"
            StrSelectCommand = mSelectCommand & IIf(Len(Filter) > 0, " Where IsNull(sys_Departments.CancelDate,'') = '' And IsNull(dbo.hrs_GetRecordViewStatus(sys_Departments.ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 And " & Filter, "  Where IsNull(sys_Departments.CancelDate,'')='' And IsNull(dbo.hrs_GetRecordViewStatus(sys_Departments.ID,'" & Me.mTable & "'," & Me.mDataBaseUserRelatedID & "," & Me.GroupID & "),0) <> 1 ")
            ObjDataset = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(mConnectionString, CommandType.Text, StrSelectCommand)
            DdlValues.Items.Clear()
    ...
    End Function
    Public Function GetAllBranches(ByVal intDepartmentID As Integer) As DataSet
        Dim ObjDataRow As DataRow
        Dim StrCommandString As String
        Dim ObjDataset As New DataSet
        Dim Item As Global.System.Web.UI.WebControls.ListItem
        ...
        Try
            ...
            StrCommandString = "sys_GetDepartmentBranches"
            ObjDataset = Microsoft.ApplicationBlocks.Data.SqlHelper.ExecuteDataset(mConnectionString, StrCommandString, intDepartmentID, Me.DataBaseUserRelatedID, clsDAL.MainCompanyID)
            Return ObjDataset
    ...
    Public Function SaveAllBranches(ByVal intDepartmentID As Integer, ByRef DG As Infragistics.WebUI.UltraWebGrid.UltraWebGrid) As Boolean
        Dim strCmd As String = " Delete From sys_DepartmentsBranches where DepartmentID = " & intDepartmentID & " ;" & vbNewLine
        Dim bFirst As Boolean = True
        Dim intRows As Integer
        strCmd &= " Insert Into sys_DepartmentsBranches (DepartmentID,BranchID,Checked) "

        For Each row As Infragistics.WebUI.UltraWebGrid.UltraGridRow In DG.Rows
            strCmd &= IIf(bFirst, " select ", " union all select ") & intDepartmentID & "," & row.Cells.FromKey("BranchID").Value & "," & row.Cells.FromKey("Checked").Value & vbNewLine
            bFirst = False
        Next
